...
// These are the types of blob fields we can create with the standard bcreatekey
// or bcreateval these correspond 1:1 with the SEM_TYPE so they need to stay in
// sync.  In fact these can never change because this is an external format. The
// first 4 types are fixed length encoded in an int64_t
...
// This is the general shape for cursor metadata, it can describe the contents
// of any cursor this is useful for generic cursor functions like "hash"
...
CQL_EXPORT void cql_copyoutrow(
...
CQL_EXPORT void cql_multifetch_meta(
  char *_Nonnull data,
  cql_fetch_info *_Nonnull info);

CQL_EXPORT cql_code cql_fetch_all_results(
  cql_fetch_info *_Nonnull info,
  cql_result_set_ref _Nullable *_Nonnull result_set);

CQL_EXPORT cql_code cql_one_row_result(
  cql_fetch_info *_Nonnull info,
  char *_Nullable data,
  cql_int32 count,
  cql_result_set_ref _Nullable *_Nonnull result_set);

CQL_EXPORT void cql_set_blob_ref(
  cql_blob_ref _Nullable *_Nonnull target,
  cql_blob_ref _Nullable source);

CQL_EXPORT void cql_set_string_ref(
  cql_string_ref _Nullable *_Nonnull target,
  cql_string_ref _Nullable source);

CQL_EXPORT void cql_set_object_ref(
  cql_object_ref _Nullable *_Nonnull target,
  cql_object_ref _Nullable source);

CQL_EXPORT void cql_set_created_blob_ref(
  cql_blob_ref _Nullable *_Nonnull target,
  cql_blob_ref _Nullable source);

CQL_EXPORT void cql_set_created_string_ref(
  cql_string_ref _Nullable *_Nonnull target,
  cql_string_ref _Nullable source);

CQL_EXPORT void cql_set_created_object_ref(
  cql_object_ref _Nullable *_Nonnull target,
  cql_object_ref _Nullable source);

CQL_EXPORT cql_code cql_prepare(
  sqlite3 *_Nonnull db,
  sqlite3_stmt *_Nullable *_Nonnull pstmt,
...
CQL_EXPORT cql_code cql_prepare_frags(
  sqlite3 *_Nonnull db,
  sqlite3_stmt *_Nullable *_Nonnull pstmt,
  const char *_Nonnull base,
  const char *_Nonnull frags);

CQL_EXPORT cql_code cql_exec_frags(
  sqlite3 *_Nonnull db,
  const char *_Nonnull base,
  const char *_Nonnull frags);

CQL_EXPORT void cql_finalize_on_error(
  cql_code rc,
  sqlite3_stmt *_Nullable *_Nonnull pstmt);

CQL_EXPORT void cql_finalize_stmt(sqlite3_stmt *_Nullable *_Nonnull pstmt);

CQL_EXPORT void cql_column_nullable_bool(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
  cql_nullable_bool *_Nonnull data);

CQL_EXPORT void cql_column_nullable_int32(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
  cql_nullable_int32 *_Nonnull data);

CQL_EXPORT void cql_column_nullable_int64(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
  cql_nullable_int64 *_Nonnull data);

CQL_EXPORT void cql_column_nullable_double(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
  cql_nullable_double *_Nonnull data);

CQL_EXPORT void cql_column_nullable_string_ref(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
  cql_string_ref _Nullable *_Nonnull data);

CQL_EXPORT void cql_column_nullable_blob_ref(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
  cql_blob_ref _Nullable *_Nonnull data);

CQL_EXPORT void cql_column_string_ref(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
  cql_string_ref _Nonnull *_Nonnull data);

CQL_EXPORT void cql_column_blob_ref(
  sqlite3_stmt *_Nonnull stmt,
  cql_int32 index,
...
CQL_EXPORT void cql_contract_argument_notnull(
...
CQL_EXPORT void cql_retain_offsets(
  void *_Nonnull pv, cql_uint16 refs_count,
  cql_uint16 refs_offset);

CQL_EXPORT void cql_release_offsets(
  void *_Nonnull pv,
...
CQL_EXPORT cql_bool cql_rows_equal(
...
CQL_EXPORT cql_bool cql_rows_same(
  cql_result_set_ref _Nonnull rs1,
  cql_int32 row1,
  cql_result_set_ref _Nonnull rs2,
  cql_int32 row2);
...
CQL_EXPORT void cql_rowset_copy(
  cql_result_set_ref _Nonnull result_set,
...
CQL_EXPORT void cql_initialize_meta(
...
// The indicated cql_bytebuf has in it exactly the raw row data that we need for
// a rowset we've accumulated it one row at a time.  It becomes the guts of a
// result set with the given metadata.  The incoming "return" code tells the
// helper if it needs to tear down because a failure is in progress so it's an
// input.
...
// stream over a child result set, hash it by the key columns and then lash each
// part of the result to the parent.  The rows could be in any order. The
// dynamic cursor for the key defines the partitioning columns and the value is
// the full child row.  When the partition is extracted the accumulated matching
// rows are provided.  Once extraction begins no more rows can be added.
...
// For internal use by the schema upgrader only, subject to change and generally
// uninteresting because of its unusual matching rules.
CQL_EXPORT cql_bool _cql_contains_column_def(
...
// option and the cql_compressed primitive.  This helper function gives us a
// normal string from the compressed fragments.  Note that --compress also makes
// fragments for the text of SQL statements.
CQL_EXPORT cql_string_ref _Nonnull cql_uncompress(
  const char *_Nonnull base,
  const char *_Nonnull frags);

// A recreate group can possibly be updated if all we did is add a new table to
// it.  If we have to do something more complicated then we have to drop the
// whole thing and and rebuild it from scratch. This code handles the fallback
// case.
...
// When emitting code for to generate a query plan we cannot expect that code to
// run in a context where all the UDFs that were mentioned are actually
// initialized. This turns out to be ok because UDFS won't affect the plan
// anyway.  This code creates a stub UDF that does nothing so that the code
// compiles as written allowing a plan to be created.  The query plan code calls
// this function once for each required UDF.
CQL_EXPORT cql_code cql_create_udf_stub(
  sqlite3 *_Nonnull db,
