...
# Importing necessary modules for JSON handling and system operations
import json
import sys
...
# Constants to represent nullability of data types
IS_NOT_NULL = 1
IS_NULLABLE = 0

# Dictionary to determine if a type is a reference type
is_ref_type = {}
...
# Mapping of CQL types to their corresponding SQLite data types
cql_row_types = {}
...
# Functions to retrieve values from SQLite based on CQL types and nullability
...
# Functions to set SQLite results based on CQL types and nullability
...
# State management for code indentation during code generation
indentation_state = {'value': 0, 'pending_line': False}

# Utility functions for code generation with verbosity control
...
# Function to emit necessary headers for the generated C code
...
# Function to emit the initializer for the SQLite extension
...
# Function to emit all procedures defined in the input JSON
...
# Function to emit the C function body for a specific procedure
...
# Function to normalize the JSON output for consistent processing
...
# Main function to parse arguments, process input JSON, and generate C code
def main():
    cmd_args = {
        "cql_header": "cqlrt.h",
        "namespace": "",
    ...
    with open(jfile) as json_file:
        ...
        emit_headers(cmd_args, json_file)
