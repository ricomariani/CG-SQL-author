...
if ! ${CQL} --sem --ast --dev --in "out/qp_in" >"out/__temp" 2>"out/cg_test_query_plan.err"; then
...
if ! ${CQL} --test --dev --cg "out/cg_test_query_plan.out" --in "out/qp_in" --rt query_plan 2>"out/cg_test_query_plan.err"; then
...
if ! ${CQL} --sem --ast --dev --test --in "out/cg_test_query_plan.out" >"out/__temp" 2>"out/cg_test_query_plan.err"; then
...
if ! ${CQL} --test --dev --cg "out/query_plan.h" "out/query_plan.c" --in "out/cg_test_query_plan.out" 2>"out/query_plan_print.err"; then
...
if ! make query_plan_test >"out/make.out" 2>"out/make.err"; then
