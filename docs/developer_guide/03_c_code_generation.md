...
information.  See [Chapter 2](./02_semantic_analysis.md) for those details.
...
`pri` and `pri_new` work exactly like they did in the echoing code (see [Chapter 1](./01_building_the_ast.md))
...
* the second string "b" is a C formatted string literal
  ...
  * as discussed in [Chapter 1](./01_building_the_ast.md), it is decoded to plain text, then re-encoded as a SQL escaped string
...
talked about `gen_statement_with_callbacks` in [Chapter 1](./01_building_the_ast.md).
...
These forms are described more properly in [Chapter 5](./05_cql_runtime.md) of the Guide but they
