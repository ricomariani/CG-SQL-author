...
begin
  ...
  EXAMPLE_NOTE!(3,    "-- An integer");
  EXAMPLE_NOTE!(3L,   "-- A long integer");
  EXAMPLE_NOTE!(3.5,  "-- A real literal");
  EXAMPLE_NOTE!(0x10, "-- 16 in hex");
  _!("");
  ...
  EXAMPLE_NOTE!(2 | 4 & 1,   "-- (Expected LTR: 0, RTL/Standard: 2)");
  EXAMPLE_NOTE!(2 | (4 & 1), "-- (Standard Evaluation)");
  EXAMPLE_NOTE!((2 | 4) & 1, "-- (Left-to-Right Evaluation)");
  _!("");
  ...
  EXAMPLE!(not true);
  EXAMPLE!(not false);
  EXAMPLE_NOTE!(not null, "-- not unknown is unknown");
  _!("");
  ...
  EXAMPLE!(1 and 0);
  EXAMPLE!(0 or 1);
  EXAMPLE_NOTE!(0 and x, "-- x not evaluated");
  EXAMPLE_NOTE!(1 or x,  "-- x not evaluated");
  _!("");
  ...
  _!("### Equality is == or =\n");
  EXAMPLE!(1 == 1);
  EXAMPLE_NOTE!(1 = 1, "-- = and == are the same thing");
  EXAMPLE!(2 == 1);
  _!("");
  ...
  EXAMPLE!(2 IS 1);
  EXAMPLE!(null IS 1);
  EXAMPLE_NOTE!(null IS null, "-- Unknown is Unknown? Yes it is!");
  EXAMPLE!("x" IS "x");
  _!("");
...
begin
  ...
  set X := 123;
  EXAMPLE_NOTE!(X, "-- Keywords and identifiers are not case sensitive");
  EXAMPLE_NOTE!(x, "-- x is the same as X");
  _!("");
  ...
  if y == 0 then
    EXAMPLE_NOTE!(y, "-- Yes, this will run.");
  end if;
  _!("");
  ...
  declare z real;
  if z is null then
    EXAMPLE_NOTE!(z, "-- Yes, this will run.");
  end if;
  _!("");


  _!("### The various types\n");
  declare a_real real;       EXAMPLE_NOTE!(a_real, '-- null real');
  declare an_int integer;    EXAMPLE_NOTE!(an_int, '-- null integer');
  declare a_long long;       EXAMPLE_NOTE!(a_long, '-- null long');
  declare a_string text;     EXAMPLE_NOTE!(a_string, "-- null string");
  declare an_object object;  EXAMPLE_NOTE!(an_object, "-- null object");
  declare a_blob blob;       EXAMPLE_NOTE!(a_blob, "-- null blob");
  _!("");
  ...
  let text_not_null   := "x";
  EXAMPLE_NOTE!(int_not_null,  "-- integer NOT NULL");
  EXAMPLE_NOTE!(long_not_null, "-- long NOT NULL");
  EXAMPLE_NOTE!(text_not_null, "-- text NOT NULL");
  ...
  EXAMPLE_NOTE!(_int_not_null IS _int_not_null, "-- `IS` never returns `NULL`");
  EXAMPLE_NOTE!(_int_not_null IS _int_null,     "-- `IS` never returns `NULL`");
  _!("");

  _!("### `=` returns `NULL` if either side is `NULL`");
  EXAMPLE_NOTE!(_int_not_null = _int_not_null, "-- (NOT NULL = NOT NULL) -> NOT NULL, ");
  EXAMPLE_NOTE!(_int_not_null = _int_null    , "-- (NOT NULL = NULL) -> NULL");
  _!("");

  _!("### The pseudo function `nullable` converts the type of its arg to the nullable version of the same thing");
  let n_i := nullable(1);
  let l_i := nullable(1L);
  EXAMPLE_NOTE!(l_i, "-- nullable integer variable initialized to 1");
...
begin
  ...
  EXAMPLE_NOTE!(IFNULL(_null, -1), "-- use -1 if x is null");
  _!("");
  ...
  EXAMPLE_NOTE!(case when 1 == 2 then 1 else 0 end, "    -- true if y = z and neither is null");
  EXAMPLE_NOTE!(case when 2 == 2 then 1 else 0 end, "    -- true if y = z and neither is null");
  EXAMPLE_NOTE!(case when 2 == _null then 1 else 0 end, "-- true if y = z and neither is null");
  _!("");


  _!("### `case` can be used to give you a default value after various tests\n");
  EXAMPLE_NOTE!((case when _null > 0 then "pos" when _null < 0 then "neg" else "other" end), "-- never null, \"other\" is returned if _null is null");
  _!("");
...
begin
  ...
  EXAMPLE_NOTE!(1+1, "-- this is evaluated in generated C code");
  EXAMPLE_NOTE!((select 1+1), "-- this expression goes to SQLite; SQLite does the addition");
  _!("");
  ...
  _!("### Use IF NOTHING THEN forms to handle no rows or null\n");
  create table T3(id integer primary key, t text, r real);
  insert into T3 values (1, "foo", 3.14);
  declare _null integer;

  EXAMPLE!((select r from T3 where id = 1));
  EXAMPLE!((select r from T3 where id = 2 if nothing then -1));
  EXAMPLE!((select _null from T3 where id = 1 if nothing then -1));
  EXAMPLE!((select _null from T3 where id = 1 if nothing or null then -1));
  _!("");
  ERROR!((select r from T3 where id = 2), "-- This throws an exception");
  ERROR!((select r from T3 where id = 2 if nothing then throw), "-- Merely makes it explicit");
  _!("");
END;
