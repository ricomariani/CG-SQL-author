...
`if nothing then throw` (the old default) `if nothing then [value]` or `if nothing or null then [value]`.  This options exists
...
### CQL0369: (SELECT ... IF NOTHING THEN) construct is for use in top level expressions, not inside of other DML
