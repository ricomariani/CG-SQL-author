...
cql_noexport void sem_main(CS, ast_node *head) {}
...
#define BEGIN_BACKING_REWRITE() \
  ...
  if (!CS->sem.in_backing_rewrite) { \
    CS->sem.backed_tables_list = NULL; \
    CS->sem.in_backing_rewrite = true; \
...
#define END_BACKING_REWRITE() \
  if (began_backing_rewrite) { \
    CS->sem.backed_tables_list = NULL; \
    CS->sem.in_backing_rewrite = false; \
  }
...
#define BACKING_REWRITE_NEEDED(ast, table_ast) \
  (!is_error(ast) && began_backing_rewrite && \
   (CS->sem.backed_tables_list || (table_ast != NULL && is_backed(table_ast->sem->sem_type))))
...
//static enforcement_stack_record *enforcement_stack;
#define enforcement_stack_lv CS->sem.enforcement_stack
#define enforcement_stack_rv ((enforcement_stack_record*)enforcement_stack_lv)
...
#define current_joinscope_lv CS->sem.current_joinscope
#define current_joinscope_rv ((sem_joinscope*)current_joinscope_lv)
...
//static in_shared_fragment_callnt_call;
...
#define global_notnull_improvements_lv CS->sem.global_notnull_improvements
#define global_notnull_improvements_rv ((global_notnull_improvement_item*)global_notnull_improvements_lv)
...
#define PUSH_JOIN(name, x)  \
  sem_joinscope name;  \
  name.parent = current_joinscope_rv; \
  name.jptr = x; \
  current_joinscope_lv = &name;

#define POP_JOIN() \
  current_joinscope_lv = current_joinscope_rv->parent;
...
#define PUSH_EXPR_CONTEXT(x) \
  uint32_t saved_expr_context = CS->sem.current_expr_context; \
  CS->sem.current_expr_context = (x); \
  Contract(0 == (CS->sem.current_expr_context & (CS->sem.current_expr_context - 1)))

#define POP_EXPR_CONTEXT() \
  CS->sem.current_expr_context = saved_expr_context
...
#define PUSH_MONITOR_SYMTAB() \
  symtab *monitor_symbtab_saved = CS->sem.monitor_symtab; \
  sem_join *monitor_jptr_saved = CS->sem.monitor_jptr; \
  if (CS->sem.select_level != 1) { \
    CS->sem.monitor_jptr = NULL; \
    used_symbols = NULL; \
    CS->sem.monitor_symtab = NULL; \
  } \
  else { \
    CS->sem.monitor_jptr = current_joinscope_rv->jptr; \
    used_symbols = CS->sem.monitor_symtab = symtab_new(); \
    add_pending_symtab_free(CS, used_symbols); \
  }

// put it all back unconditionally.
#define POP_MONITOR_SYMTAB() \
  CS->sem.monitor_jptr = monitor_jptr_saved; \
  CS->sem.monitor_symtab = monitor_symbtab_saved;
...
static void enqueue_pending_table_validation(CqlState* _Nonnull CS, pending_table_validation *pending) {
  ...
  *v = *pending;
  ...
  pending_table_validations_head_lv = v;
}
...
static void run_pending_table_validations(CqlState* _Nonnull CS) {
  ...
  for (; v; v = v->next) {
    if (v->fk) {
      ...
      Invariant(v->check);
      sem_validate_check_expr_for_table(CS, v->table_ast, v->check, "CHECK");
      if (is_error(v->check)) {
        goto error;
      }
    }
  ...
  pending_table_validations_head_lv = NULL;  // the minipool will free these
  return;

error:
  record_error(CS, v->table_ast);
  record_error(CS, v->def);
  pending_table_validations_head_lv = NULL;  // the minipool will free these
}
...
static void sem_validate_check_expr_for_table(CqlState* _Nonnull CS, ast_node *table, ast_node *expr, CSTR context) {
  ...
  if (!is_error(table)) {
    ...
    symtab *saved_locals = CS->sem.locals;
    symtab *saved_globals = CS->sem.globals;

    // hide variables for this expression
    CS->sem.locals = CS->sem.globals = NULL;

    PUSH_JOIN(expr_scope, jptr);
    sem_numeric_expr(CS, expr, NULL, context, SEM_EXPR_CONTEXT_CONSTRAINT);
    POP_JOIN();

    CS->sem.locals = saved_locals;
    CS->sem.globals = saved_globals;
...
static void sem_validate_index_expr_for_jptr(CqlState* _Nonnull CS, sem_join *jptr, ast_node *expr) {
  ...
  PUSH_JOIN(expr_scope, jptr);
  sem_root_expr(CS, expr, SEM_EXPR_CONTEXT_CONSTRAINT);
  POP_JOIN();
...
static bool_t is_name_list_equal(CqlState* _Nonnull CS, ast_node *name_list1, ast_node *name_list2) {
  ...
  for (ast_node *name_list = name_list1; name_list; name_list = name_list->right) {
    CSTR name = string_from_name_list_item(CS, name_list);
    symtab_add(CS, cache, name, NULL);
    count1++;
  }
...
static bool_t is_either_list_a_subset(CqlState* _Nonnull CS, ast_node *indexed_columns1, ast_node *indexed_columns2) {
  ...
  for (ast_node *names = indexed_columns2; names; names = names->right) {
    ...
    EXTRACT_ANY_NOTNULL(expr, indexed_column->left);
    symtab_add(CS, cache, expr_as_text(CS, expr), NULL);
  }

  for (ast_node *names = indexed_columns1; names; names = names->right) {
    EXTRACT(indexed_column, names->left);
    EXTRACT_ANY_NOTNULL(expr, indexed_column->left);
    if (!symtab_find(cache, expr_as_text(CS, expr))) {
      included = false;
      break;
    }
  }
...
static bool_t is_unique_key_valid(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *uk) {
  ...
  for (ast_node *unq_def = find_first_unique_key(table_ast); unq_def; unq_def = find_next_unique_key(unq_def)) {
    ...
    if (is_either_list_a_subset(CS, indexed_columns1, indexed_columns2)) {
      return false;
    }
  }
  return true;
...
static ast_node *find_usable_index(CqlState* _Nonnull CS, CSTR name, ast_node *err_target, CSTR msg) {
  ...
  if (!sem_validate_object_ast_in_current_region(CS, name, index_ast, err_target, msg)) {
    return NULL;
  }
...
cql_noexport bool_t is_proc_private(CqlState* _Nonnull CS, ast_node *_Nonnull proc_stmt) {
  ...
  return misc_attrs && exists_attribute_str(CS, misc_attrs, "private");
}
...
cql_noexport bool_t is_proc_suppress_result_set(CqlState* _Nonnull CS, ast_node *_Nonnull proc_stmt) {
  ...
  return misc_attrs && exists_attribute_str(CS, misc_attrs, "suppress_result_set");
}
...
cql_noexport bool_t is_proc_suppress_getters(CqlState* _Nonnull CS, ast_node *_Nonnull proc_stmt) {
  ...
  return misc_attrs && exists_attribute_str(CS, misc_attrs, "suppress_getters");
}
...
cql_noexport bool_t is_proc_emit_setters(CqlState* _Nonnull CS, ast_node *_Nonnull proc_stmt) {
  ...
  return misc_attrs && exists_attribute_str(CS, misc_attrs, "emit_setters");
}
...
cql_noexport bool_t is_proc_shared_fragment(CqlState* _Nonnull CS, ast_node *_Nonnull proc_stmt) {
  ...
  return misc_attrs && exists_attribute_str(CS, misc_attrs, "shared_fragment");
}
...
static void sem_column_name_exist_in_result_set(CqlState* _Nonnull CS, CSTR name, ast_node *misc_attr_value, void *context) {
  ...
  if (sptr) {
    ...
    if (icol < 0) {
      CHARBUF_OPEN(msg);
      bprintf(&msg, "CQL0239: %s column does not exist in result set", CS->sem.annotation_target);
      report_error(CS, misc_attrs, msg.ptr, name);
      record_error(CS, misc_attrs);
      CHARBUF_CLOSE(msg);
      return;
    }
  }

  record_ok(CS, misc_attr_value);
}
...
static void sem_column_name_match_encode_context_column_type(CqlState* _Nonnull CS, CSTR name, ast_node *misc_attrs)
{
  ...
  for (uint32_t i = 0; i < count; i++) {
    CSTR col = sptr->names[i];
    if (!strcmp(name, col)) {
      if (core_type_of(sptr->semtypes[i]) != CS->sem.encode_context_type) {
        report_error(CS, misc_attrs, "CQL0402: vault context column in vault_senstive attribute must match the specified type in strict mode", "vault_sensitive");
        record_error(CS, misc_attrs);
      }
    }
  }
...
static bool_t vault_sensitive_encode_column_valid_string(CqlState* _Nonnull CS, ast_node *misc_attr_value, ast_node *misc_attrs)
{
  if (!is_id(misc_attr_value)) {
    ...
    record_error(CS, misc_attr_value);
    if (misc_attrs) {
      record_error(CS, misc_attrs);
    }
    return false;
...
static void vault_sensitive_encode_columns(CqlState* _Nonnull CS, ast_node *ast_misc_attrs, symtab *vault_column_list, ast_node *misc_attrs) {
  for (ast_node *list = ast_misc_attrs; list; list = list->right) {
    ...
    if (is_ast_str(list->left)) {
      ...
      if (misc_attrs && CS->sem.current_proc) {
        sem_column_name_exist_in_result_set(CS, name, list->left, misc_attrs);
      }
      if (vault_column_list) {
        symtab_add(CS, vault_column_list, name, NULL);
      }
    } else {
      report_error(CS, list->left, "CQL0363: all arguments must be names", "vault_sensitive");
      record_error(CS, list->left);
      if (misc_attrs) {
        record_error(CS, misc_attrs);
      }
    }
  ...
  void *context)
{
  ...
  if (ast_misc_attr_value_list == NULL) {
    ...
    enforce_encode_context_column_with_strict_mode(CS, misc_attrs);
    return;
  }
  ...
  if (!is_ast_misc_attr_value_list(ast_misc_attr_value_list)) {
    ...
    if (!vault_sensitive_encode_column_valid_string(CS, ast_misc_attr_value_list, misc_attrs)) {
      return;
    }
    // ensure column exists in result set
    EXTRACT_STRING(name, ast_misc_attr_value_list);
    if (misc_attrs && CS->sem.current_proc) {
      sem_column_name_exist_in_result_set(CS, name, ast_misc_attr_value_list, misc_attrs);
    }
    // extract the single to-be-encoded column
    if (info->encode_columns) {
      symtab_add(CS, info->encode_columns, name, NULL);
    }
    // report error if strict mode is on for encode context column
    enforce_encode_context_column_with_strict_mode(CS, misc_attrs);
    return;
  ...
  if (!has_nested_columns) {
    vault_sensitive_encode_columns(CS, ast_misc_attr_value_list, info->encode_columns, misc_attrs);
    // report error if strict mode is on for encode context column
enforce_encode_context_column_with_strict_mode(CS, misc_attrs);
    return;
  }
  ...
  for (ast_node *list = ast_misc_attr_value_list; list; list = list->right) {
    ...
    if (is_ast_misc_attr_value_list(misc_attr_value)) {
      vault_sensitive_encode_columns(CS, misc_attr_value, info->encode_columns, misc_attrs);
    }
    else {
      // we found encode context column and ensure it is valid string
      if (!vault_sensitive_encode_column_valid_string(CS, misc_attr_value, misc_attrs)) {
        return;
      }
      EXTRACT_STRING(context_col, misc_attr_value);
      if (misc_attrs && CS->sem.current_proc) {
        enforce_non_sensitive_context_column(CS, context_col, misc_attrs);
        sem_column_name_exist_in_result_set(CS, context_col, misc_attr_value, misc_attrs);
        sem_column_name_match_encode_context_column_type(CS, context_col, misc_attrs);
      }
      ...
      if (context_col_count > 1 && misc_attrs) {
        report_error(CS, misc_attrs, "CQL0408: encode context column can be only specified once", context_col);
        record_error(CS, misc_attrs);
      }
    }
  }
...
cql_noexport void init_encode_info(CqlState* _Nonnull CS, ast_node *misc_attrs, bool_t *use_encode_arg, CSTR *encode_context_column_arg, symtab *encode_columns_arg) {
  *use_encode_arg = misc_attrs && exists_attribute_str(CS, misc_attrs, "vault_sensitive");
  if (*use_encode_arg) {
    ...
    find_misc_attrs(CS, misc_attrs, sem_find_ast_misc_attr_vault_sensitive_callback, &info);
    record_ok(CS, misc_attrs);
  }
}
...
static bool_t sem_validate_object_ast_in_current_region(CqlState* _Nonnull CS, CSTR name,
  ...
  if (table_ast->sem && table_ast->sem->region) {
    ...
    Invariant(CS->sem.current_region_image);
    if (!symtab_find(CS->sem.current_region_image, table_ast->sem->region)) {
      ...
      if (err_target) {
        ...
        bprintf(&err_msg, "%s (object is in schema region '%s' not accessible from region '%s')",
          ...
          CS->sem.current_region);
        report_error(CS, err_target, err_msg.ptr, name);
...
static ast_node *find_usable_table_or_view_even_deleted(CqlState* _Nonnull CS, CSTR name, ast_node *err_target, CSTR msg) {
  ...
  ast_node *table_ast = find_table_or_view_even_deleted(CS, name);
  if (!table_ast) {
    if (err_target) {
      report_error(CS, err_target, msg, name);
    }
  ...
  if (!sem_validate_object_ast_in_current_region(CS, name, table_ast, err_target, msg)) {
    return NULL;
  }
...
cql_noexport ast_node *find_usable_and_not_deleted_table_or_view(CqlState* _Nonnull CS, CSTR name, ast_node *err_target, CSTR msg) {
  ...
  ast_node *table_ast = find_usable_table_or_view_even_deleted(CS, name, err_target, msg);
  if (!table_ast) {
    return NULL;
  }
  ...
  if (CS->sem.schema_upgrade_version > 0 && !is_ast_create_table_stmt(table_ast)) {
  ...
  if (is_deleted(table_ast)) {
    if (err_target) {
      CHARBUF_OPEN(err_msg);
      if (CS->sem.schema_upgrade_version > 0) {
        bprintf(&err_msg, "%s (not visible in schema version %d)", msg, CS->sem.schema_upgrade_version);
      }
      else {
...
static ast_node *find_usable_trigger(CqlState* _Nonnull CS, CSTR name, ast_node *err_target, CSTR msg) {
  ...
  if (!sem_validate_object_ast_in_current_region(CS, name, trigger_ast, err_target, msg)) {
    return NULL;
  }
...
cql_noexport ast_node *find_arg_bundle(CqlState* _Nonnull CS, CSTR name) {
  ...
  symtab_entry *entry = symtab_find(CS->sem.arg_bundles, name);
  return entry ? (ast_node*)(entry->val) : NULL;
}
...
static bool_t add_named_type(CqlState* _Nonnull CS, CSTR name, ast_node *ast) {
  ...
  if (existing_type) {
    bool_t matching = sem_validate_identical_text(CS, existing_type, ast, gen_one_stmt, NULL);

    if (!matching) {
      report_error(CS, ast, "CQL0359: conflicting type declaration", name);
      record_error(CS, ast);
      return false;
    }
    else {
      // this will let us find the duplicate later, it marks this as not authoritative
      sem_add_flags(CS, ast, SEM_TYPE_ALIAS);
    }
  }
  else {
    bool_t added = symtab_add(CS, tab, name, ast);
    Invariant(added);
  }
...
cql_noexport ast_node *find_named_type(CqlState* _Nonnull CS, CSTR name) {
  ...
  if (CS->sem.local_types) {
    entry = symtab_find(CS->sem.local_types, name);
  }
  if (!entry) {
    entry = symtab_find(CS->sem.global_types, name);
  }
...
CSTR create_group_id(CqlState* _Nonnull CS, CSTR group_name, CSTR table_name) {
  // These names are used as keys in symbol tables and were selected to match the facet names
   ...
   if (group_name && group_name[0]) {
     return dup_printf(CS, "%s_group", group_name);
   }
   else {
     return dup_printf(CS, "%s_table", table_name);
   }
}
...
static bool_t walk_recreate_group_deps (CqlState* _Nonnull CS, CSTR start, CSTR end) {
  ...
  bytebuf *buf = symtab_ensure_bytebuf(CS, CS->sem.recreate_group_deps, start);
  size_t count = buf->used / sizeof(CSTR);
  CSTR *neighbors = (CSTR *) (buf->ptr);

  for (size_t i = 0; i < count; i++) {
    if (walk_recreate_group_deps(CS, neighbors[i], end)) {
      return true;
    }
  }
  return false;
...
static bool_t add_group_dependency_to_recreate_group(CqlState* _Nonnull CS, CSTR group_name, CSTR new_dependent_group_name) {
  ...
  // DFS.
  if (walk_recreate_group_deps(CS, new_dependent_group_name, group_name)) {
    // Adding the edge introduces a cycle because we found a path from v to u
    return false;
  }
  symtab_append_bytes(CS, CS->sem.recreate_group_deps, group_name, &new_dependent_group_name, sizeof(CSTR));
  return true;
}
...
static void print_sem_struct(CqlState* _Nonnull CS, sem_struct *sptr) {
  ...
  for (uint32_t i = 0; i < sptr->count; i++) {
    ...
    if (sptr->semtypes[i] & SEM_TYPE_QID) {
      CHARBUF_OPEN(tmp);
      cg_decode_qstr(CS, &tmp, sptr->names[i]);
      cql_output(CS, "%s", tmp.ptr);
      CHARBUF_CLOSE(tmp);
    }
    else {
      cql_output(CS, "%s", sptr->names[i]);
    }

    cql_output(CS, ": %s", temp.ptr);
    CHARBUF_CLOSE(temp);
  }
  cql_output(CS, " }");
}
...
static void print_sem_join(CqlState* _Nonnull CS, sem_join *jptr) {
  ...
  for (uint32_t i = 0; i < jptr->count; i++) {
    ...
    cql_output(CS, "%s: %s", jptr->names[i], jptr->tables[i]->struct_name);
  }
  cql_output(CS, " }");
}
...
cql_noexport void print_sem_type(CqlState* _Nonnull CS, sem_node *sem) {
  ...
  if (sem->name) {
    cql_output(CS, "%s: ", sem->name);
  }

  sem_t sem_type = sem->sem_type;

  if (is_struct(sem_type)) {
     print_sem_struct(CS, sem->sptr);
  }
  else if (is_join(sem_type)) {
    print_sem_join(CS, sem->jptr);
  }
  else {
    CHARBUF_OPEN(temp);
  ...
  if (sem->kind) {
     cql_output(CS, "<%s>", sem->kind);
  }

  if (sem->value) {
    CHARBUF_OPEN(temp);
    eval_format_number(CS, sem->value, EVAL_FORMAT_NORMAL, &temp);
    cql_output(CS, " = %s", temp.ptr);
    CHARBUF_CLOSE(temp);
  }
  ...
  if (sem->create_version > 0) {
    cql_output(CS, " @create(%d)", sem->create_version);
  }
  if (sem->delete_version > 0) {
    cql_output(CS, " @delete(%d)", sem->delete_version);
  }

  if (sem->recreate) {
    cql_output(CS, " @recreate");
  }

  if (sem->recreate_group_name) {
    cql_output(CS, "(%s)", sem->recreate_group_name);
  }
}
...
cql_noexport void report_error(CqlState* _Nonnull CS, ast_node *ast, CSTR msg, CSTR subject) {
  ...
  if (subject) {
    ...
    if (subject[0] == 'X' && subject[1] == '_' && strchr(subject+2, 'X')) {
       ...
       CHARBUF_OPEN(tmp);
       cg_decode_qstr(CS, &tmp, subject);
       subject = Strdup(CS, tmp.ptr);
       CHARBUF_CLOSE(tmp);
    }
    subj1 = " '";
    subj2 = subject;
...
static void cql_attach_captured_errors(CqlState* _Nonnull CS, ast_node *stmt) {
   if (is_error(stmt)) {
      ...
      stmt->sem->error = Strdup(CS, CS->sem.error_capture->ptr);
   }
}
...
static void report_and_capture_error(CqlState* _Nonnull CS, ast_node *root, ast_node *ast, CSTR err_msg, CSTR name) {
  ...
  if (CS->options.print_ast) {
    CS->sem.error_capture = &errbuf;
  }

  report_error(CS, ast, err_msg, name);
  record_error(CS, ast);
  record_error(CS, root);

  if (CS->sem.error_capture) {
    cql_attach_captured_errors(CS, ast);
    CS->sem.error_capture = NULL;
  }
...
static void report_sem_type_mismatch(CqlState* _Nonnull CS,
  ...
  report_error(CS, node, temp.ptr, sem_name);
  CHARBUF_CLOSE(temp);
}
...
static sem_node *ok_sentinel(CqlState* _Nonnull CS) {
  if (CS->sem.sem_ok) {
    ...
    return CS->sem.sem_ok = new_sem(CS, SEM_TYPE_OK);
  }
}
...
static void sem_reject_real(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  if (!is_error(ast)) {
    ...
    if (core_type == SEM_TYPE_REAL) {
      report_error(CS, ast, "CQL0001: operands must be an integer type, not real", op);
      record_error(CS, ast);
    }
  }
}
...
static sem_struct *new_sem_struct_strip_table_flags(CqlState* _Nonnull CS, sem_struct *sptr) {
  ...
  sem_struct *result = sem_clone_struct_strip_flags(CS, sptr, sem_not(allowed_flags));
...
static bool_t error_any_object(CqlState* _Nonnull CS, ast_node *ast, sem_t core_type_left, sem_t core_type_right, CSTR op) {
  ...
  if (is_object(core_type_right)) {
    report_error(CS, ast->right, "CQL0003: right operand cannot be an object in", op);
    record_error(CS, ast);
    return true;
  }
...
static bool_t error_any_blob_types(CqlState* _Nonnull CS, ast_node *ast, sem_t core_type_left, sem_t core_type_right, CSTR op) {
  ...
  if (is_blob(core_type_right)) {
    report_error(CS, ast->right, "CQL0005: right operand cannot be a blob in", op);
    record_error(CS, ast);
    return true;
  }
...
static bool_t error_any_text_types(CqlState* _Nonnull CS, ast_node *ast, sem_t core_type_left, sem_t core_type_right, CSTR op) {
  ...
  if (is_text(core_type_right)) {
    report_error(CS, ast->right, "CQL0008: right operand cannot be a string in", op);
    record_error(CS, ast);
    return true;
  }
  ...
  switch (core_type_needed) {
    case SEM_TYPE_TEXT:
      if (!is_string_compat(core_type_found)) {
        report_error(CS, ast, "CQL0009: incompatible types in expression", subject);
        record_error(CS, ast);
        return false;
      }
      break;
    ...
    case SEM_TYPE_OBJECT:
      if (!is_object_compat(core_type_found)) {
        report_error(CS, ast, "CQL0010: incompatible types in expression", subject);
        record_error(CS, ast);
        return false;
      }
      break;
    ...
    case SEM_TYPE_BLOB:
      if (!is_blob_compat(core_type_found)) {
        report_error(CS, ast, "CQL0011: incompatible types in expression", subject);
        record_error(CS, ast);
        return false;
      }
      break;
    ...
    case SEM_TYPE_REAL:
      if (!is_numeric_compat(core_type_found)) {
        report_error(CS, ast, "CQL0012: incompatible types in expression", subject);
        record_error(CS, ast);
        return false;
      }
      break;
...
error:
  ...
  report_error(CS, ast, tmp.ptr, NULL);
  CHARBUF_CLOSE(tmp);
...
cql_noexport bool_t sem_verify_assignment(CqlState* _Nonnull CS, ast_node *ast, sem_t sem_type_needed, sem_t sem_type_found, CSTR var_name) {
  ...
  if (!sem_verify_compat(CS, ast, sem_type_needed, sem_type_found, var_name)) {
    return false;
  }

  if (!sem_verify_safe_assign(CS, ast, sem_type_needed, sem_type_found, var_name)) {
    return false;
  }

  if (is_nullable(sem_type_found) && is_not_nullable(sem_type_needed)) {
    report_error(CS, ast, "CQL0013: cannot assign/copy possibly null expression to not null target", var_name);
    return false;
  }

  if (sensitive_flag(sem_type_found) && !sensitive_flag(sem_type_needed)) {
    report_error(CS, ast, "CQL0014: cannot assign/copy sensitive expression to non-sensitive target", var_name);
    return false;
  }
...
static void sem_numeric_expr(CqlState* _Nonnull CS, ast_node *expr, ast_node *context, CSTR subject, uint32_t expr_context) {
  ...
  if (!is_error(expr) && !is_numeric_expr(expr)) {
    ...
    report_error(CS, best, "CQL0015: expected numeric expression", subject);
    record_error(CS, expr);
  }
...
static void join_tables(CqlState* _Nonnull CS, ast_node *t1, ast_node *t2, ast_node *result, int32_t join_type) {
  ...
  if (CS->sem.enforcement.strict_table_function) {
     ...
     if (error) {
       report_error(CS, result, "CQL0371: table valued function used in a left/right/cross context; this would hit a SQLite bug.  Wrap it in a CTE instead.", NULL);
       record_error(CS, result);
       return;
     }
  }
  ...
  for (uint32_t i = 0; i < j1->count; i++) {
    for (uint32_t j = 0; j < j2->count; j++) {
      ...
      if (!Strcasecmp(n1, n2)) {
        report_error(CS, t2, "CQL0016: duplicate table name in join", n1);
        record_error(CS, result);
        return;
      }
    }
  ...
  sem_join *jptr = new_sem_join(CS, j1->count + j2->count);
  ...
  for (uint32_t i = 0; i < j1->count; i++, j++) {
    ...
    jptr->tables[j] = sem_clone_struct_strip_flags(CS, j1->tables[i], strip_left);
  }
  for (uint32_t i = 0; i < j2->count; i++, j++) {
    jptr->names[j] = j2->names[i];
    jptr->tables[j] = sem_clone_struct_strip_flags(CS, j2->tables[i], strip_right);
  }

  result->sem = new_sem(CS, SEM_TYPE_JOIN);
...
static void sem_data_type_column(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_type_int(ast)) {
    ...
    ast->sem = new_sem(CS, SEM_TYPE_TEXT);
  } else if (is_ast_type_blob(ast)) {
    ast->sem = new_sem(CS, SEM_TYPE_BLOB);
  } else if (is_ast_type_object(ast)) {
    ast->sem = new_sem(CS, SEM_TYPE_OBJECT);
  } else if (is_ast_type_long(ast)) {
    ast->sem = new_sem(CS, SEM_TYPE_LONG_INTEGER);
  } else if (is_ast_type_real(ast)) {
    ast->sem = new_sem(CS, SEM_TYPE_REAL);
  } else if (is_ast_type_cursor(ast)) {
    ast->sem = new_sem(CS, SEM_TYPE_CURSOR_FORMAL);
  } else {
    Contract(is_ast_type_bool(ast));
    ast->sem = new_sem(CS, SEM_TYPE_BOOL);
  }
  ...
  if (ast->left) {
    ...
    if (is_set || is_cursor) {
      ...
      ast_node *type_node = new_ast_str(CS, tmp.ptr);
      ast_node *like_node = new_ast_like(CS, type_node, NULL);
      AST_REWRITE_INFO_RESET();

      CHARBUF_CLOSE(tmp);

      // the indicated type must be a valid shape name (one we could use in LIKE T)
      ast_node *like_target = sem_find_shape_def_base(CS, like_node, LIKEABLE_FOR_VALUES);
      if (!like_target) {
        record_error(CS, ast);
        return;
      }
      ...
      if (is_set && !is_ast_create_proc_stmt(like_target) && !is_ast_declare_proc_stmt(like_target)) {
        report_error(CS, ast, "CQL0090: object<T SET> has a T that is not a procedure with a result set", kind);
        record_error(CS, ast);
        return;
      }

      if (is_set && (is_proc_private(CS, like_target) || is_proc_suppress_result_set(CS, like_target))) {
        report_error(CS, ast, "CQL0091: object<T SET> has a T that is not a public procedure with a result set", kind);
        record_error(CS, ast);
	return;
      }
    }
...
static void sem_data_type_var(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_create_data_type(ast)) {
    ...
    sem_data_type_var(CS, data_type);
    ...
    if (core_type != SEM_TYPE_TEXT && core_type != SEM_TYPE_BLOB && core_type != SEM_TYPE_OBJECT) {
      report_error(CS, ast, "CQL0361: return data type in a create function declaration can only be text, blob or object", NULL);
      record_error(CS, ast);
      return;
    }

    // Create a node for me using my child's type but adding func create.
    ast->sem = new_sem(CS, SEM_TYPE_CREATE_FUNC | data_type->sem->sem_type);
    ...
    if (data_type->sem->sem_type & SEM_TYPE_NOTNULL) {
      report_error(CS, ast, "CQL0367: an attribute was specified twice", "not null");
      record_error(CS, ast);
      return;
    }

    // Create a node for me using my child's type but adding not null.
    ast->sem = new_sem(CS, SEM_TYPE_NOTNULL | data_type->sem->sem_type);
    // copy object type to the sem if applicable. It's used to rewrite
    // named type ast.
    ast->sem->kind = data_type->sem->kind;
  }
    ...
    if (data_type->sem->sem_type & SEM_TYPE_SENSITIVE) {
      report_error(CS, ast, "CQL0367: an attribute was specified twice", "@sensitive");
      record_error(CS, ast);
      return;
    }

    // Create a node for me using my child's type but adding not null.
    ast->sem = new_sem(CS, SEM_TYPE_SENSITIVE | data_type->sem->sem_type);
    // copy object type to the sem if applicable. It's used to rewrite
    // named type ast.
    ast->sem->kind = data_type->sem->kind;
  }
  else {
    sem_data_type_column(CS, ast);
  }
}
...
static bool_t sem_validate_name_list(CqlState* _Nonnull CS, ast_node *name_list, sem_join *jptr) {
  ...
  bool_t valid = sem_name_check(CS, &check);
  destroy_name_check(CS, &check);
  return valid;
}
...
static void sem_validate_previous_index(CqlState* _Nonnull CS, ast_node *prev_index) {
  ...
  ast_node *ast = find_index(CS, index_name);

  if (!ast) {
    if (CS->options.schema_exclusive) {
    ...
    report_error(CS, prev_index, "CQL0017: index was present but now it does not exist (use @delete instead)", index_name);
    record_error(CS, prev_index);
    return;
  }

  enqueue_pending_region_validation(CS, prev_index, ast, index_name);
}
...
static bool_t sem_validate_no_delete_migration(CqlState* _Nonnull CS, version_attrs_info *vers_info, ast_node *ast, CSTR obj_name) {
  ...
  if (vers_info->delete_proc) {
    report_error(CS, ast, "CQL0321: migration proc not allowed on object", obj_name);
    record_error(CS, ast);
    return false;
  }
...
static void sem_create_index_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.schema_upgrade_version > 0 && !CS->sem.current_proc) {
    record_ok(CS, ast);
    return;
  }
  ...
  if (CS->sem.validating_previous_schema) {
    record_ok(CS, ast);
    sem_validate_previous_index(CS, ast);
    return;
  }

  bool_t adding_current_entity = will_add_current_entity(CS);

  // if there is an existing index, save it here so we can check for duplicates later.
  ast_node *existing_defn = adding_current_entity ? find_index(CS, index_name) : NULL;

  version_attrs_info vers_info;
  init_version_attrs_info(&vers_info, index_name, ast, attrs);
  bool_t valid_version_info = sem_validate_version_attrs(CS, &vers_info);
  Invariant(valid_version_info);  // nothing can go wrong with index version info

  if (!sem_validate_vers_ok_in_context(CS, &vers_info)) {
    record_error(CS, ast);
    return;
  }

  if (!sem_validate_no_delete_migration(CS, &vers_info, ast, index_name)) {
    return;
  }
  ...
  if (deleting) {
    table_ast = find_usable_table_or_view_even_deleted(CS,
      table_name,
      table_name_ast,
      "CQL0019: create index table name not found");

    if (is_deleted(table_ast)) {
      report_error(CS, ast, "CQL0397: object is an orphan because its table is deleted. Remove rather than @delete", index_name);
      record_error(CS, ast);
      return;
    }
  }
  ...
  sem_non_backed_table(CS, ast, table_ast);
  if (is_error(ast)) {
    return;
  }

  sem_non_blob_storage_table(CS, ast, table_ast);
  if (is_error(ast)) {
    return;
  }

  if (is_virtual_ast(table_ast)) {
    report_error(CS, table_name_ast, "CQL0159: cannot add an index to a virtual table", table_name);
    record_error(CS, ast);
    return;
  }
  ...
  if (table_ast->sem->recreate && adding_current_entity) {
    ...
    if (table_region != CS->sem.current_region) {
      ...
      report_error(CS, ast, "CQL0066: if a table is marked @recreate, its indices must be in its schema region", index_name);
      record_error(CS, ast);
      return;
    }
  }
  ...
  if (!deleting && !sem_validate_name_list(CS, indexed_columns, table_ast->sem->jptr)) {
    record_error(CS, ast);
    return;
  }
  ...
  if (opt_where) {
    ...
    sem_validate_check_expr_for_table(CS, table_ast, expr, "WHERE");
  ...
  ast->sem = new_sem(CS, SEM_TYPE_OK);
  ast->sem->delete_version = vers_info.delete_version;
  ast->sem->region = CS->sem.current_region;

  if (existing_defn) {
    if (!sem_validate_identical_ddl(CS, existing_defn, ast)) {
      report_error(CS, index_name_ast, "CQL0018: duplicate index name", index_name);
      record_error(CS, index_name_ast);
      record_error(CS, ast);
    }
    else {
      // this will let us find the duplicate later, it marks this as not authoritative
  ...
  if (adding_current_entity) {
    ...
    add_item_to_list(CS, &CS->sem.all_indices_list, ast);

    // and consume the name
    add_index(CS, ast, index_name);

    // and record the annotation
    sem_record_annotation_from_vers_info(CS, &vers_info);

    // add the index to the table it is on
    add_item_to_list(CS, &table_ast->sem->table_info->index_list, ast);
  }
}
...
static void sem_unq_def(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *def) {
  ...
  if (def->left) {
    ...
    if (symtab_find(CS->sem.table_items, name)) {
      report_error(CS, def, "CQL0020: duplicate constraint name in table", name);
      record_error(CS, table_ast);
      return;
    }
    symtab_add(CS, CS->sem.table_items, name, def);
  }

  // check if def node is unique key compare to others in table_ast
  if (!is_unique_key_valid(CS, table_ast, def)) {
    report_error(CS, def, "CQL0269: at least part of this unique key is redundant with previous unique keys", NULL);
    record_error(CS, table_ast);
    return;
  }
  ...
  if (!sem_validate_name_list(CS, indexed_columns, table_ast->sem->jptr)) {
    record_error(CS, table_ast);
    return;
  }
}
...
static bool_t sem_validate_fk_flags(CqlState* _Nonnull CS, ast_node *def, int32_t flags) {
  ...
  if (CS->sem.enforcement.strict_fk_delete) {
    if (0 == (flags & FK_ON_DELETE)) {
      report_error(CS, def, "CQL0238: strict FK validation requires that some ON DELETE option be selected for every foreign key", NULL);
      record_error(CS, def);
      return false;
    }
  }
...
static ast_node *find_and_validate_referenced_table(CqlState* _Nonnull CS, CSTR table_name, ast_node *err_target, version_attrs_info *table_info) {
  ...
  Contract(!CS->sem.validating_previous_schema);
  ...
  if (!Strcasecmp(table_name, CS->sem.current_table_name)) {
    Invariant(CS->sem.current_table_ast);
    return CS->sem.current_table_ast;
  }

  ast_node *ref_table_ast;

  if (table_info->delete_version > 0 || CS->sem.current_proc) {
    ...
    ref_table_ast = find_table_or_view_even_deleted(CS, table_name);

    if (!ref_table_ast) {
      report_error(CS, err_target, "CQL0021: foreign key refers to non-existent table", table_name);
      record_error(CS, err_target);
    }
  ...
  ref_table_ast = find_usable_and_not_deleted_table_or_view(CS,
    table_name,
    err_target,
  ...
  if (!table_info->recreate) {
    ...
    if (ref_create_verison > 0 && ref_create_verison > cur_create_version) {
      report_error(CS, err_target, "CQL0324: referenced table was created in a later version so it cannot be used in a foreign key", table_name);
      record_error(CS, err_target);
      return NULL;
    }
  }
  ...
  if (ref_table_ast->sem->recreate) {
    if (table_info->recreate) {
      CSTR ref_recreate_gname = create_group_id(CS, ref_table_ast->sem->recreate_group_name, table_name);
      CSTR curr_recreate_gname = create_group_id(CS, table_info->recreate_group_name, CS->sem.current_table_name);
      bool_t success = add_group_dependency_to_recreate_group(CS, ref_recreate_gname, curr_recreate_gname);
      if (!success) {
        report_error(CS, err_target, "CQL0060: referenced table can be independently recreated so it cannot be used in a foreign key", table_name);
        record_error(CS, err_target);
        return NULL;
      }
    }
...
static bool_t find_referenceable_columns(CqlState* _Nonnull CS,
  ...
  for (; col_key_list; col_key_list = col_key_list->right) {
    ...
    if (is_ast_pk_def(col_def)) {
      ...
      if (callback(CS, indexed_columns2, context)) {
        return true;
      }
    }
  ...
  for (uint32_t i = 0; i < CS->sem.indices->capacity; i++) {
    symtab_entry entry = CS->sem.indices->payload[i];
    if (entry.sym) {
      ...
      if (callback(CS, indexed_columns, context)) {
        return true;
      }
    }
  }
...
static sem_t sem_validate_referenceable_fk_def(CqlState* _Nonnull CS, ast_node *ref_table_ast, ast_node *name_list) {
  ...
  bool_t valid = find_referenceable_columns(CS,
    ref_table_ast,
  ...
  if (!valid) {
    ...
    report_error(CS, name_list, "CQL0272: columns referenced in the foreign key statement should match exactly a unique key in the parent table", ref_table_name);
  }
  return valid;
}
...
static void record_table_dependencies(CqlState* _Nonnull CS, ast_node *src_ast, ast_node *target_ast) {
  ...
  symtab_append_bytes(CS, CS->sem.ref_sources_for_target_table, target_name, &src_ast, sizeof(src_ast));
  symtab_append_bytes(CS, CS->sem.ref_targets_for_source_table, src_name, &target_ast, sizeof(target_ast));
}
...
static void sem_fk_def(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *def, version_attrs_info *table_info) {
  ...
  if (def->left) {
    ...
    if (symtab_find(CS->sem.table_items, constraint_name)) {
      report_error(CS, def, "CQL0020: duplicate constraint name in table", constraint_name);
      record_error(CS, table_ast);
      return;
    }
    symtab_add(CS, CS->sem.table_items, constraint_name, def);
  }

  if (!sem_validate_name_list(CS, src_list, table_ast->sem->jptr)) {
    record_error(CS, table_ast);
    return;
  }
  ...
  ast_node *ref_table_ast = find_and_validate_referenced_table(CS,
    ref_table_name,
    def,
    table_info);
  if (!ref_table_ast) {
  ...
  if (!sem_validate_name_list(CS, ref_list, ref_table_ast->sem->jptr)) {
    record_error(CS, table_ast);
    return;
  }
  ...
  // in the target table.
  if (!sem_validate_referenceable_fk_def(CS, ref_table_ast, ref_list)) {
    record_error(CS, table_ast);
    record_error(CS, def);
    return;
  }
  ...
  if (src_list || ref_list) {
    report_error(CS, def, "CQL0023: number of columns on both sides of a foreign key must match", NULL);
    record_error(CS, table_ast);
    record_error(CS, def);
    return;
  }
  ...
  if (!sem_validate_fk_flags(CS, def, flags)) {
    record_error(CS, table_ast);
    return;
  }

  record_table_dependencies(CS, table_ast, ref_table_ast);
...
static void sem_validate_builtin_migration_proc(CqlState* _Nonnull CS, ast_node *ast, uint32_t code, CSTR name) {
  ...
  if (!is_from_recreate && !is_module_warn) {
    report_error(CS, ast, "CQL0379: unknown built-in migration procedure", name);
    record_error(CS, ast);
    return;
  }

  if (is_from_recreate && code != SCHEMA_ANNOTATION_CREATE_TABLE) {
    report_error(CS, ast, "CQL0378: built-in migration procedure not valid in this context", name);
    record_error(CS, ast);
    return;
  }
...
static bool_t sem_validate_version(CqlState* _Nonnull CS, uint32_t code, ast_node *ast, int32_t *version, CSTR *out_proc) {
  ...
  *out_proc = NULL;
  ...
  if (vers < 1) {
    report_error(CS, ast, "CQL0025: version number in annotation must be positive", NULL);
    record_error(CS, ast);
    return false;
  }

  if (*version > 0) {
    report_error(CS, ast, "CQL0026: duplicate version annotation", NULL);
    record_error(CS, ast);
    return false;
  }
  ...
  if (version_annotation->right) {
    ...
    if (is_ast_dot(version_annotation->right)) {
      ...
      proc_name = dup_printf(CS, "%s:%s", lhs, rhs);
      sem_validate_builtin_migration_proc(CS, dot, code, proc_name);
      if (is_error(dot)) {
        record_error(CS, ast);
        return false;
      }
      ...
      if (len >= 4) {
        ...
        if (!Strcasecmp(name + offset, "_crc")) {
          report_error(CS, ast, "CQL0338: the name of a migration procedure may not end in '_crc'", name);
          record_error(CS, ast);
          return false;
        }
      }

      if (!sem_create_migration_proc_prototype(CS, ast, proc_name)) {
        record_error(CS, ast);
        return false;
      }
    }
  ...
  if (CS->sem.validating_previous_schema) {
    ...
    if (vers > CS->sem.max_previous_schema_version) {
      bool_t excluded = CS->sem.current_region && CS->sem.excluded_regions && symtab_find(CS->sem.excluded_regions, CS->sem.current_region);

      if (!excluded) {
        CS->sem.max_previous_schema_version = vers;
      }
    }
  }
  else {
    ...
    if (version_annotation->right && !is_ast_dot(version_annotation->right)) {
      EXTRACT_STRING(name, version_annotation->right);
      if (!symtab_add(CS, CS->sem.upgrade_procs, name, ast)) {
        report_error(CS, version_annotation->right, "CQL0027: a procedure can appear in only one annotation", name);
        record_error(CS, ast);
        return false;
      }
    }
...
static void record_schema_annotation(CqlState* _Nonnull CS, int32_t vers, ast_node *target_ast, CSTR target_name, uint32_t type, ast_node *def, ast_node *ast, int32_t ordinal) {
  ...
  schema_annotation *note = bytebuf_alloc(CS->sem.schema_annotations, sizeof(*note));
...
static void sem_col_attrs_fk(CqlState* _Nonnull CS, ast_node *fk, ast_node *def, col_def_info *info) {
  ...
  enqueue_pending_table_validation(CS, &pending);
...
void sem_validate_fk_attr(CqlState* _Nonnull CS, pending_table_validation *pending) {
  ...
  if (ref_list->right || core_type_of(def->sem->sem_type) != core_type_of(ref->sem->sem_type)) {
    report_error(CS, def, "CQL0028: FK reference must be exactly one column with the correct type", def->sem->name);
    record_error(CS, fk);
    return;
  }

  // flags are only checked if we are in the appropriate strict mode
  if (!sem_validate_fk_flags(CS, fk, flags)) {
    return;
  }
  ...
  record_table_dependencies(CS, src_table_ast, ref_table_ast);
...
static sem_t sem_col_attrs(CqlState* _Nonnull CS, ast_node *def, ast_node *head, col_def_info *info) {
  ...
  for (ast_node *ast = head; ast; ast = ast->right) {
    ...
    if (is_ast_create_attr(ast)) {
      if (!sem_validate_version(CS, SCHEMA_ANNOTATION_CREATE_COLUMN, ast, &info->create_version, &info->create_proc)) {
        record_error(CS, head);
        return false;
      }
      if (adding_current_entity) {
        record_schema_annotation(CS, info->create_version, info->table_info->target_ast, info->table_info->name,
                                 SCHEMA_ANNOTATION_CREATE_COLUMN, def, ast->left, info->column_ordinal);
      }
    }
    else if (is_ast_delete_attr(ast)) {
      if (!sem_validate_version(CS, SCHEMA_ANNOTATION_DELETE_COLUMN, ast, &info->delete_version, &info->delete_proc)) {
        record_error(CS, head);
        return false;
      }
      if (adding_current_entity) {
        record_schema_annotation(CS, info->delete_version, info->table_info->target_ast, info->table_info->name,
      ...
      if (core_type != SEM_TYPE_TEXT) {
        report_error(CS, ast->left, "CQL0348: collate applied to a non-text column", info->col_name);
        record_error(CS, head);
        return false;
      }
      ...
      if (col_attrs_autoinc) {
        ...
        if (core_type != SEM_TYPE_INTEGER && core_type != SEM_TYPE_LONG_INTEGER) {
          report_error(CS, ast->left, "CQL0029: autoincrement column must be [LONG|INT] PRIMARY KEY", info->col_name);
          record_error(CS, head);
          return false;
        }
      }
      ...
      sem_col_attrs_fk(CS, ast, def, info);
      if (is_error(ast)) {
        record_error(CS, head);
        return false;
      }
      ...
      if (flags) {
        report_error(CS, ast, "CQL0362: HIDDEN column attribute must be the first attribute if present", NULL);
        record_error(CS, head);
        return false;
      }
    ...
    if (flags & new_flags) {
      report_error(CS, ast, "CQL0030: a column attribute was specified twice on the same column", info->col_name);
      record_error(CS, head);
      return false;
    }
  ...
  Invariant(CS->sem.schema_upgrade_version != 0);  // -1 or positive

  if (CS->sem.schema_upgrade_script) {
    // no deleted columns processing, keep it all..
  }
  else if (CS->sem.schema_upgrade_version < 0) {
    ...
    if (info->delete_version > 0 && CS->sem.schema_upgrade_version > info->delete_version) {
      flags |= SEM_TYPE_DELETED;
    }
    ...
    // cannot see it yet.
    if (info->create_version > 0 && CS->sem.schema_upgrade_version < info->create_version) {
      flags |= SEM_TYPE_DELETED;
    }
  }
...
static void sem_col_def(CqlState* _Nonnull CS, ast_node *def, col_def_info *info) {
  ...
  rewrite_right_col_def_type_attrs_if_needed(CS, col_def_type_attrs);
  ...
  // column name:  [name]
  sem_data_type_column(CS, data_type);

  def->sem = new_sem(CS, data_type->sem->sem_type);
  def->sem->name = name;
  ...
  if (is_qid(name_ast)) {
    sem_add_flags(CS, def, SEM_TYPE_QID);
  }

  if (attrs) {
    sem_add_flags(CS, def, sem_col_attrs(CS, def, attrs, info));
    if (is_error(attrs)) {
      record_error(CS, def);
      return;
    }
    ...
    if (expr) {
      sem_root_expr(CS, expr, SEM_EXPR_CONTEXT_NONE);
     ...
     if (!is_text(def->sem->sem_type)) {
       ...
       if (!sem_verify_assignment(CS, expr, def->sem->sem_type, expr->sem->sem_type, "default value")) {
         record_error(CS, def);
         return;
       }
     }
  ...
  if (is_primary_key(def->sem->sem_type) && is_unique_key(def->sem->sem_type)) {
    report_error(CS, def, "CQL0031: column can't be primary key and also unique key", name);
    record_error(CS, def);
    return;
  }
  ...
  if (info->create_version < info->previous_create_version) {
    report_error(CS, def, "CQL0032: created columns must be at the end and must be in version order", name);
    record_error(CS, def);
    return;
  }
  ...
  if (info->delete_version > 0 || info->create_version > 0) {
    if (info->table_info->recreate) {
      report_error(CS, def, "CQL0033: columns in a table marked @recreate cannot have @create or @delete", name);
      record_error(CS, def);
      return;
    }

    if (!is_nullable(def->sem->sem_type) && !has_default((def->sem->sem_type))) {
      report_error(CS, def, "CQL0034: create/delete version numbers can only be applied to "
  ...
  if (info->delete_version > 0 && info->delete_version <= info->create_version) {
    report_error(CS, def, "CQL0035: column delete version can't be <= column create version", name);
    record_error(CS, def);
    return;
  }
  ...
  if (info->delete_version > 0) {
    if (info->table_info->create_version > 0 && info->delete_version <= info->table_info->create_version) {
      report_error(CS, def, "CQL0036: column delete version can't be <= the table create version", name);
      record_error(CS, def);
      return;
    }

    if (info->table_info->delete_version > 0 && info->delete_version >= info->table_info->delete_version) {
      report_error(CS, def, "CQL0037: column delete version can't be >= the table delete version", name);
      record_error(CS, def);
    }
  }
  ...
  if (info->create_version > 0) {
    if (info->table_info->create_version > 0 && info->create_version <= info->table_info->create_version) {
      report_error(CS, def, "CQL0038: column create version can't be <= the table create version", name);
      record_error(CS, def);
      return;
    }

    if (info->table_info->delete_version > 0 && info->create_version >= info->table_info->delete_version) {
      report_error(CS, def, "CQL0039: column create version can't be >= the table delete version", name);
      record_error(CS, def);
      return;
    }
  }
  ...
  if (info->autoinc_columns > 1) {
    report_error(CS, name_ast, "CQL0040: table can only have one autoinc column", name);
    record_error(CS, def);
    return;
  }

  if (is_object(def->sem->sem_type)) {
    report_error(CS, name_ast, "CQL0041: tables cannot have object columns", name);
    record_error(CS, def);
    return;
  }
...
static void sem_constraints(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *col_key_list, col_def_info *info) {
  ...
  for (ast_node *item = col_key_list; item; item = item->right) {
    ...
    if (is_ast_col_def(def)) {
      ...
      sem_pk_def(CS, table_ast, def);
      info->primary_keys++;
    } else if (is_ast_fk_def(def)) {
      sem_fk_def(CS, table_ast, def, info->table_info);
    } else if (is_ast_check_def(def)) {
      sem_check_def(CS, table_ast, def);
    } else {
      Contract(is_ast_unq_def(def));
      sem_unq_def(CS, table_ast, def);
    }
  }

  symtab_delete(CS, CS->sem.table_items);
  CS->sem.table_items = NULL;
}
...
static bool_t sem_binary_prep_with_flow_analysis(CqlState* _Nonnull CS, ast_node *ast, sem_t *core_type_left, sem_t *core_type_right, sem_t *combined_flags, CSTR op) {
  ...
  sem_expr(CS, left);
  if (!strcmp(op, "AND")) {
    sem_set_improvements_for_true_condition(CS, left);
  } else if (!strcmp(op, "OR")) {
    sem_set_improvements_for_false_condition(CS, left);
  }
  sem_expr(CS, right);
...
static void sem_binary_like(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (!is_string_compat(core_type_left)) {
    report_error(CS, ast->left, "CQL0042: left operand must be a string in", op);
    record_error(CS, ast);
    return;
  }

  if (!is_string_compat(core_type_right)) {
    report_error(CS, ast->right, "CQL0043: right operand must be a string in", op);
    record_error(CS, ast);
    return;
  }
...
static void sem_collate(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (CURRENT_EXPR_CONTEXT_IS(SEM_EXPR_CONTEXT_NONE)) {
    report_error(CS, ast, "CQL0241: COLLATE may only appear in the context of a SQL statement", NULL);
    record_error(CS, ast);
    return;
  }
...
static void sem_concat(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (CURRENT_EXPR_CONTEXT_IS(SEM_EXPR_CONTEXT_NONE)) {
    report_error(CS, ast, "CQL0241: CONCAT may only appear in the context of a SQL statement", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  if (is_blob(sem_type_left) || is_blob(sem_type_right)) {
    ...
    report_error(CS, operand, "CQL0243: blob operand must be converted to string first in", op);
    record_error(CS, ast);
    return;
  }
  ...
  ast->sem = new_sem(CS, SEM_TYPE_TEXT | combined_flags);
}
...
static void sem_binary_math(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (error_any_object(CS, ast, core_type_left, core_type_right, op)) {
    return;
  }

  if (error_any_blob_types(CS, ast, core_type_left, core_type_right, op)) {
    return;
  }

  if (error_any_text_types(CS, ast, core_type_left, core_type_right, op)) {
    return;
  }
  ...
  CSTR kind = sem_combine_kinds(CS, ast->right, ast->left->sem->kind);
  if (is_error(ast->right)) {
    record_error(CS, ast);
    return;
  }

  ast->sem = new_sem(CS, core_type | combined_flags);
  ast->sem->kind = kind;
}
...
static void sem_binary_logical(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  bool_t has_error;
  if (CS->sem.enforcement.strict_and_or_not_null_check) {
    FLOW_PUSH_CONTEXT_BRANCH_GROUP();
    has_error = !sem_binary_prep_with_flow_analysis(CS, ast, &core_type_left, &core_type_right, &combined_flags, op);
    FLOW_POP_CONTEXT_BRANCH_GROUP();
  } else {
    has_error = !sem_binary_prep(CS, ast, &core_type_left, &core_type_right, &combined_flags);
  }
  if (has_error) {
    return;
  }
...
static void sem_binary_eq_or_ne(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (is_ast_null(ast->left) || is_ast_null(ast->right)) {
    report_error(CS, ast, "CQL0373: comparing against NULL always yields NULL; use IS and IS NOT instead", NULL);
    record_error(CS, ast);
    return;
  }

  if (!sem_verify_compat(CS, ast, core_type_left, core_type_right, op)) {
    return;
  }

  sem_combine_kinds(CS, ast->right, ast->left->sem->kind);
  if (is_error(ast->right)) {
    record_error(CS, ast);
    return;
  }
...
static void sem_expr_const(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  eval_node result = EVAL_NIL;
  eval(CS, ast->left, &result);

  if (result.sem_type == SEM_TYPE_ERROR) {
    report_error(CS, ast, "CQL0353: evaluation of constant failed", NULL);
    record_error(CS, ast);
    return;
  }

  ast_node *ast_new = eval_set(CS, ast, &result);
  sem_root_expr(CS, ast_new, SEM_EXPR_CONTEXT_NONE);
  ast->sem = ast_new->sem;
}
...
static bool_t sem_validate_numeric(CqlState* _Nonnull CS, ast_node *ast, sem_t core_type, CSTR op) {
  ...
  if (is_object(core_type)) {
    report_error(CS, ast->left, "CQL0046: object operand not allowed in", op);
    record_error(CS, ast);
    return false;
  }

  if (is_text(core_type)) {
    report_error(CS, ast->left, "CQL0047: string operand not allowed in", op);
    record_error(CS, ast);
    return false;
  }
...
static void sem_unary_math(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (!sem_validate_numeric(CS, ast, core_type, op)) {
    return;
  }
  ...
  ast->sem = new_sem(CS, sem_type_result);
...
static void sem_unary_is_true_or_false(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  ast->sem = new_sem(CS, SEM_TYPE_BOOL | SEM_TYPE_NOTNULL | combined_flags);
}
...
static void sem_binary_is_or_is_not(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (is_ast_null(ast->right) && is_not_nullable(ast->left->sem->sem_type)) {
    report_error(CS,
      ...
      expr_as_text(CS, ast->left));
    record_error(CS, ast);
    return;
  }

  // the result of is or is not is always a bool and never null
  ast->sem = new_sem(CS, SEM_TYPE_BOOL | SEM_TYPE_NOTNULL | sensitive_flag(combined_flags));
}
...
static void sem_arg_expr(CqlState* _Nonnull CS, ast_node *ast, bool_t is_count) {
  if (is_ast_star(ast)) {
    if (is_count) {
      ...
      report_error(CS, ast, "CQL0051: argument can only be used in count(*)", "*");
      record_error(CS, ast);
    }
  }
  else {
...
static void sem_select_star(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_struct *sptr = new_sem_struct(CS, "select", count);
  int32_t field = 0;
  ...
  ast->sem = new_sem(CS, SEM_TYPE_STRUCT);
  ast->sem->sptr = sptr;
...
static uint32_t sem_select_table_star_count(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!current_joinscope_lv || !current_joinscope_rv->jptr) {
    report_error(CS, ast, "CQL0053: select [table].* cannot be used with no FROM clause", NULL);
    record_error(CS, ast);
    return false;
  }
  ...
  for (uint32_t i = 0; i < jptr->count; i++) {
    if (!Strcasecmp(jptr->names[i], name)) {
      ...
      if (CS->sem.keep_table_name_in_aliases && !CS->sem.in_trigger && !CS->sem.in_trigger_when_expr) {
  ...
  report_error(CS, ast, "CQL0054: table not found", name);
  record_error(CS, ast);
  return 0;
}
...
static int32_t sem_select_table_star_add(CqlState* _Nonnull CS, ast_node *ast, sem_struct *sptr, int32_t index) {
  ...
  Invariant(current_joinscope_rv->jptr);
...
static void sem_verify_no_anon_columns(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (!strcmp(col, "_anon")) {
      report_error(CS, ast, "CQL0055: all columns in the select must have a name", NULL);
      record_error(CS, ast);
      return;
    }
  }
...
static void sem_verify_no_null_columns(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    if (is_null_type(sptr->semtypes[i])) {
      report_error(CS, ast, "CQL0056: NULL expression has no type to imply the type of the select result", sptr->names[i]);
      record_error(CS, ast);
    }
  }
}
...
static void sem_emit_column_diff_diagnostics(CqlState* _Nonnull CS, ast_node *left, ast_node *right) {
  ...
  for (i = 0; i < sptr_left->count; i++) {
    ...
    if (!symtab_add(CS, left_symbols, Strdup(CS, tmp.ptr), NULL)) {
  ...
  for (i = 0; i < sptr_right->count; i++) {
    ...
    if (!symtab_add(CS, right_symbols, Strdup(CS, tmp.ptr), NULL)) {
  ...
  symtab_delete(CS, left_symbols);
  symtab_delete(CS, right_symbols);
  report_error(CS, left, report.ptr, NULL);
  CHARBUF_CLOSE(tmp);
...
static sem_struct *sem_unify_compatible_columns(CqlState* _Nonnull CS, ast_node *left, ast_node *right) {
  ...
  if (sptr_left->count != sptr_right->count) {
    report_error(CS, left, "CQL0057: if multiple selects, all must have the same column count", NULL);
    sem_emit_column_diff_diagnostics(CS, left, right);
    record_error(CS, left);
    record_error(CS, right);
    return NULL;
  }
  ...
  for (uint32_t i = 0; i < sptr_left->count; i++) {
    ...
    if (strcmp(col1, col2)) {
      ...
      report_error(CS, left, err_msg, NULL);
      sem_emit_column_diff_diagnostics(CS, left, right);
      record_error(CS, left);
      record_error(CS, right);
      return NULL;
  ...
  sem_struct *sptr = new_sem_struct(CS, "union", sptr_left->count);
  ...
  for (uint32_t i = 0; i < sptr_left->count; i++) {
    ...
    if (!sem_verify_compat(CS, left, sem_type_1, sem_type_2, col)) {
      record_error(CS, left);
      record_error(CS, right);
      return NULL;
    }
    ...
    CSTR kind = sem_combine_kinds_general(CS, left, kind_1, kind_2);
    if (is_error(left)) {
      record_error(CS, left);
      record_error(CS, right);
      return NULL;
...
cql_noexport void sem_verify_identical_columns(CqlState* _Nonnull CS, ast_node *expected, ast_node *actual, CSTR target) {
  ...
  if (sptr_expected->count != sptr_actual->count) {
    CSTR errmsg = dup_printf(CS, "CQL0057: %s, all must have the same column count", target);
    report_error(CS, actual, errmsg, NULL);
    sem_emit_column_diff_diagnostics(CS, expected, actual);
    record_error(CS, actual);
    return;
  }
  ...
  for (uint32_t i = 0; i < sptr_expected->count; i++) {
    ...
    if (core_type_of(sem_type_1) != core_type_of(sem_type_2)) {
      CSTR error_message = dup_printf(CS, "CQL0061: %s, all columns must be an exact type match", target);
      report_sem_type_mismatch(CS, sem_type_1, sem_type_2, actual, error_message, col2);
      record_error(CS, actual);
      return;
    }
    ...
    if (is_nullable(sem_type_1) != is_nullable(sem_type_2)) {
      CSTR error_message =
        dup_printf(CS, "CQL0062: %s, all columns must be "
...
static void sem_update_proc_type_for_select(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // at the root node
  if (CS->sem.current_explain_stmt && !is_ast_explain_stmt(ast)) {
  ...
  if (!CS->sem.current_proc || is_error(CS->sem.current_proc) || is_error(ast) || CS->sem.in_trigger) {
    return;
  }
  ...
  Invariant(is_proc(CS->sem.current_proc));
  ...
  if (core_type_of(CS->sem.current_proc->sem->sem_type) == SEM_TYPE_OK) {
    ...
    sem_replace_flags(CS, CS->sem.current_proc, sem_type);
  ...
  Invariant(is_struct(CS->sem.current_proc->sem->sem_type));
  ...
  bool_t did_out = has_out_stmt_result(CS->sem.current_proc);
  bool_t did_call_out_union = has_out_union_call(CS->sem.current_proc);
  bool_t did_out_union = has_out_union_stmt_result(CS->sem.current_proc) && !did_call_out_union;
  bool_t did_select = has_result_set(CS->sem.current_proc) && !did_call_out_union;
  ...
  if (is_out != did_out || is_out_union != did_out_union || is_select != did_select || is_calling_out_union != did_call_out_union) {
    report_error(CS, ast, "CQL0063: can't mix and match out, out union, or select/call for return values", name);
    record_error(CS, ast);
    return;
  }

  sem_verify_identical_columns(CS, CS->sem.current_proc, ast, "in multiple select/out statements");
}
...
static sem_resolve sem_try_resolve_locals_bundle(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  if (!entry) {
    CHARBUF_OPEN(tmp);
      bprintf(&tmp, "%s_", name);
      entry = symtab_find(CS->sem.locals, tmp.ptr);
    CHARBUF_CLOSE(tmp);

    if (!entry) {
      report_resolve_error(CS, ast, "CQL0201: expanding FROM LOCALS, there is no local matching", name);
  ...
  if (ast) {
    ast->sem = new_sem(CS, var->sem->sem_type);
...
static sem_resolve sem_try_resolve_arguments_bundle(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  ast_node *params = get_proc_params(CS, CS->sem.current_proc);
  Invariant(params);
  ...
  if (param) {
    if (ast) {
      ast->sem = new_sem(CS, param->sem->sem_type);
  ...
  report_resolve_error(CS, ast, "CQL0201: expanding FROM ARGUMENTS, there is no argument matching", name);
  record_resolve_error(CS, ast);
  return SEM_RESOLVE_STOP;
}
...
static bool_t try_find_possible_dot_overload(CqlState* _Nonnull CS, charbuf *sym, ast_node *expr, CSTR name, CSTR choice1, CSTR choice2) {
  ...
  if (find_any_callable(CS, sym->ptr)) {
    return true;
  }
...
static sem_resolve sem_try_resolve_dot_rewrite(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  ast_node *var = find_local_or_global_variable(CS, scope);
  ...
  bool_t found = try_find_possible_dot_overload(CS, &sym, var, name, "get", "get_from");
  if (found) {
    *type_ptr = &var->sem->sem_type;
    if (ast) {
      ast->sem = var->sem;
      sem_add_flags(CS, ast, 0);    // no flags added this is a clone
...
static void add_variable(CqlState* _Nonnull CS, CSTR name, ast_node *variable) {
  ...
  if (CS->sem.current_variables == CS->sem.locals && CS->sem.unitary_locals) {
    if (is_unitary(variable->sem->sem_type)) {
      bytebuf_append(CS->sem.unitary_locals, &variable, sizeof(variable));
    }
  }

  symtab_add(CS, CS->sem.current_variables, name, variable);
}
...
cql_noexport ast_node *find_local_or_global_variable(CqlState* _Nonnull CS, CSTR name) {
  ...
  if (!entry) {
    entry = symtab_find(CS->sem.globals, name);
  }
...
static sem_resolve sem_try_resolve_variable(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  ast_node *variable = find_local_or_global_variable(CS, name);
  if (!variable) {
    return SEM_RESOLVE_CONTINUE;
  }
  ...
  if (is_object(sem_type) &&
    ...
    report_resolve_error(CS, ast, "CQL0064: object variables may not appear in the context"
...
static bool_t sem_resolve_column_does_not_conflict_with_variable(CqlState* _Nonnull CS, ast_node *ast, CSTR column_name, CSTR table_name) {
  ...
  if (!table_name && find_local_or_global_variable(CS, column_name)) {
    report_resolve_error(CS, ast, "CQL0059: a variable name might be ambiguous "
...
static sem_resolve sem_try_resolve_column(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
        ...
        for (uint32_t j = 0; j < table->count; j++) {
          if (!Strcasecmp(name, table->names[j])) {
            if (found_in_this_joinscope) {
              ...
              // join-scopes anyway.
              report_resolve_error(CS, ast, "CQL0065: identifier is ambiguous", name);
              record_resolve_error(CS, ast);
              return SEM_RESOLVE_STOP;
            }
            ...
            if (CS->sem.keep_table_name_in_aliases && !CS->sem.in_trigger && !CS->sem.in_trigger_when_expr && ast && scope) {
              ...
              insert_table_alias_string_overide(CS, ast->left, table->struct_name);
            }
          }
        }
      }
  ...
  if (!sem_resolve_column_does_not_conflict_with_variable(CS, ast, name, scope)) {
    return SEM_RESOLVE_STOP;
  }
  ...
  if (ast) {
    ...
    if (found_jptr && found_jptr == CS->sem.monitor_jptr) {
      symtab_add(CS, CS->sem.monitor_symtab, col, NULL);
    }
  }
...
static sem_resolve sem_try_resolve_rowid(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  if (scope == NULL && jptr->count == 1) {
    ...
    report_resolve_error(CS, ast, "CQL0066: identifier is ambiguous", name);
    record_resolve_error(CS, ast);
    return SEM_RESOLVE_STOP;
  }
...
static bool_t sem_find_column_for_name(CqlState* _Nonnull CS, ast_node *ast, CSTR name) {
  ...
  bool_t found = sem_try_resolve_column(CS, ast, name, NULL, &type) == SEM_RESOLVE_STOP;
  if (!found) {
    found = sem_try_resolve_rowid(CS, ast, name, NULL, &type) == SEM_RESOLVE_STOP;
  }
...
static void sem_resolve_cursor_field(CqlState* _Nonnull CS, ast_node *ast, ast_node *cursor, CSTR field, sem_t **type_ptr) {
  ...
  if (!is_auto_cursor(sem_type)) {
    report_resolve_error(CS, ast, "CQL0067: cursor was not used with 'fetch [cursor]'", scope);
    record_resolve_error(CS, ast);
    return;
  }
  ...
  for (uint32_t i = 0; i < sptr->count; i++) {
    if (!Strcasecmp(sptr->names[i], field)) {
      if (ast) {
        ast->sem = new_sem(CS, sptr->semtypes[i] | SEM_TYPE_VARIABLE);
        ast->sem->name = dup_printf(CS, "%s.%s", scope, sptr->names[i]);
  ...
  report_resolve_error(CS, ast, "CQL0068: field not found in cursor", field);
  record_resolve_error(CS, ast);
}
...
static sem_resolve sem_try_resolve_cursor_field(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  ast_node *variable = find_local_or_global_variable(CS, scope);
  if (!variable || !is_cursor(variable->sem->sem_type)) {
    return SEM_RESOLVE_CONTINUE;
  }

  sem_resolve_cursor_field(CS, ast, variable, name, type_ptr);
  return SEM_RESOLVE_STOP;
}
...
static sem_resolve sem_try_resolve_enum(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  ast_node *enum_stmt = find_enum(CS, scope);
  if (!enum_stmt) {
    return SEM_RESOLVE_CONTINUE;
  }
  ...
  while (enum_values) {
    ...
    if (!Strcasecmp(enum_member, name)) {
      if (ast) {
        ast_node *ast_new = eval_set(CS, ast, enum_value->left->sem->value);
        sem_root_expr(CS, ast_new, SEM_EXPR_CONTEXT_NONE);
        ast->sem = ast_new->sem;
  ...
  report_resolve_error(CS, ast, "CQL0357: enum does not contain", name);
  record_resolve_error(CS, ast);
...
static sem_resolve sem_try_resolve_global_constant(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  ast_node *const_value = find_constant(CS, name);
  if (!const_value) {
    return SEM_RESOLVE_CONTINUE;
  }
  ...
  if (ast) {
    if (is_numeric(const_value->sem->sem_type)) {
      ast_node *ast_new = eval_set(CS, ast, const_value->left->sem->value);
      sem_root_expr(CS, ast_new, SEM_EXPR_CONTEXT_NONE);
      ast->sem = ast_new->sem;
    }
    else {
...
static sem_resolve sem_try_resolve_arg_bundle(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  Contract(type_ptr);
   ...
   ast_node *shape = find_arg_bundle(CS, scope);
   if (!shape) {
     return SEM_RESOLVE_CONTINUE;
   }
  ...
  for (uint32_t i = 0; i < sptr->count; i++) {
    if (!Strcasecmp(sptr->names[i], name)) {
      ...
      sem_resolve result = sem_try_resolve_variable(CS, ast, underscore_name.ptr, NULL, type_ptr);
  ...
  report_resolve_error(CS, ast, "CQL0068: field not found in shape", name);
  record_resolve_error(CS, ast);
...
static void sem_resolve_id_with_type(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope, sem_t **type_ptr) {
  ...
  *type_ptr = NULL;
  ...
  for (uint32_t i = 0; i < sizeof(resolver) / sizeof(void *); i++) {
    if (resolver[i](CS, ast, name, scope, type_ptr) == SEM_RESOLVE_STOP) {
      return;
    }
  }

  if (scope) {
    name = dup_printf(CS, "%s.%s", scope, name);
  }

  report_resolve_error(CS, ast, "CQL0069: name not found", name);
  record_resolve_error(CS, ast);
}
...
cql_noexport void sem_resolve_id(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope) {
  ...
  sem_t *type = NULL;
  sem_resolve_id_with_type(CS, ast, name, scope, &type);
}
...
static void sem_validate_variable_referenced_is_initialized_if_required(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (sem_type & SEM_TYPE_INIT_REQUIRED && !(sem_type & SEM_TYPE_INIT_COMPLETE)) {
    report_error(CS, ast, "CQL0438: variable possibly used before initialization", ast->sem->name);
    record_error(CS, ast);
    return;
  }
...
static void sem_validate_has_row_check_requirements_if_applicable(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.enforcement.strict_cursor_has_row) {
    if (is_ast_dot(ast)) {
      ...
      sem_t *type = find_mutable_type(CS, scope, NULL);
      if (type && is_cursor(*type)) {
        ...
        sem_validate_auto_cursor_field_accessed_has_row_check_if_required(CS, ast, *type);
      }
    }
  }
}
...
static void sem_validate_initialization_requirements_if_applicable(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_variable(sem_type)) {
    ...
    sem_validate_variable_referenced_is_initialized_if_required(CS, ast);
  }
}
...
static void sem_cursor_as_expression(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_auto_cursor(ast->sem->sem_type)) {
    vname = dup_printf(CS, "%s._has_row_", ast->sem->name);
  } else {
    if (!(ast->sem->sem_type & SEM_TYPE_FETCH_INTO)) {
      report_error(CS, ast, "CQL0067: cursor was not used with 'fetch [cursor]'", ast->sem->name);
      record_error(CS, ast);
      return;
    }
    vname = dup_printf(CS, "_%s_has_row_", ast->sem->name);
  }
  ast->sem = new_sem(CS, SEM_TYPE_BOOL | SEM_TYPE_VARIABLE | SEM_TYPE_NOTNULL);
  ast->sem->name = vname;
}
...
static void sem_notnull_improved_expr(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.is_analyzing_notnull_rewrite) {
  ...
  if (CS->sem.current_loop_analysis_state == LOOP_ANALYSIS_STATE_ANALYZE) {
  ...
  rewrite_nullable_to_notnull(CS, ast);
}
...
static void sem_resolve_id_expr(CqlState* _Nonnull CS, ast_node *ast, CSTR name, CSTR scope) {
  ...
  sem_resolve_id(CS, ast, name, scope);
  if (is_error(ast)) {
    return;
  }

  sem_validate_has_row_check_requirements_if_applicable(CS, ast);
  if (is_error(ast)) {
    return;
  }

  sem_validate_initialization_requirements_if_applicable(CS, ast);
  if (is_error(ast)) {
    return;
  }
  ...
  if (is_cursor(sem_type)) {
    ...
    sem_cursor_as_expression(CS, ast);
    return;
  }
  ...
  if (sem_type & SEM_TYPE_INFERRED_NOTNULL) {
    ...
    sem_notnull_improved_expr(CS, ast);
  }
}
...
static sem_t *find_mutable_type(CqlState* _Nonnull CS, CSTR name, CSTR scope) {
  ...
  sem_t *type = NULL;
  sem_resolve_id_with_type(CS, NULL, name, scope, &type);
...
static sem_t *find_mutable_type_and_global_status(CqlState* _Nonnull CS, CSTR name, CSTR scope, bool_t *is_global) {
  ...
  *is_global = false;
  ...
  if (scope) {
    ...
    // auto cursor field.
    symtab_entry *entry = symtab_find(CS->sem.globals, scope);
    if (!entry) {
    ...
    sem_resolve_cursor_field(CS, NULL, ast, name, &global_type);

    *is_global = type == global_type;
  }
  else {
    // We only have a name: The name might refer to a global variable.
    symtab_entry *entry = symtab_find(CS->sem.globals, name);
    if (!entry) {
  ...
  if (kind_right) {
    if (kind_left) {
      if (Strcasecmp(kind_left, kind_right)) {
        CSTR errmsg = dup_printf(CS, "CQL0070: expressions of different kinds can't be mixed: '%s' vs. '%s'", kind_right, kind_left);
        report_error(CS, ast, errmsg, NULL);
        record_error(CS, ast);
      }
    }
    return kind_right;
  ...
  bool_t is_iif)
{
  ...
  for (ast_node *ast = head; ast; ast = ast->right) {
    ...
    if (is_null_type(when_expr->sem->sem_type)) {
      report_error(CS, ast, "CQL0501: WHEN expression must not be a constant NULL but can be of a nullable type", NULL);
      record_error(CS, ast);
      return;
    }

    if (!sem_verify_compat(CS, when_expr, sem_type_required_for_when, when_expr->sem->sem_type, is_iif ? "iif" : "when")) {
      record_error(CS, ast);
      record_error(CS, head);
      return;
    }

    sem_combine_kinds(CS, when_expr, kind_required_for_when);
    if (is_error(when_expr)) {
      record_error(CS, ast);
      record_error(CS, head);
      return;
    ...
    if (!has_expression_to_match) {
      sem_set_improvements_for_true_condition(CS, when_expr);
    }
    sem_expr(CS, then_expr);
    FLOW_POP_CONTEXT_BRANCH();
    ...
    sem_set_improvements_for_false_condition(CS, when_expr);
    ...
    if (sem_type_result == SEM_TYPE_PENDING) {
      ...
      if (!sem_verify_compat(CS, then_expr, sem_type_result, sem_type_current, "then")) {
        record_error(CS, ast);
        record_error(CS, head);
        return;
      }

      then_kind = sem_combine_kinds(CS, then_expr, then_kind);
      if (is_error(then_expr)) {
        record_error(CS, ast);
        record_error(CS, head);
        return;
  ...
  head->sem = new_sem(CS, sem_type_result | sem_sensitive);
...
cql_noexport void sem_case(CqlState* _Nonnull CS, ast_node *ast, bool_t is_iif) {
  ...
  sem_case_list(CS, case_list, !!expr, sem_type_required_for_when, kind_required_for_when, is_iif);
  ...
  if (else_expr) {
    flow_set_context_branch_group_covers_all_cases(CS, true);
    FLOW_PUSH_CONTEXT_BRANCH();
    sem_expr(CS, else_expr);
    FLOW_POP_CONTEXT_BRANCH();
    ...
    if (!sem_verify_compat(CS, else_expr, sem_type_result, sem_type_else, is_iif ? "iif" : "else")) {
      goto error;
    }

    CSTR else_kind = sem_combine_kinds(CS, else_expr, ast->sem->kind);
    if (is_error(else_expr)) {
      goto error;
    }

    sem_type_result = sem_combine_types(sem_type_result, sem_type_else);
    ast->sem = new_sem(CS, sem_type_result | sem_sensitive);
    ...
    sem_replace_flags(CS, ast, new_flags);
  }
...
static void sem_expr_between_or_not_between(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  sem_expr(CS, ast->left);
  sem_expr(CS, range->left);
  sem_expr(CS, range->right);
  ...
  if (is_blob(sem_type_item)) {
    report_error(CS, ast->left, "CQL0071: first operand cannot be a blob in", operation);
    record_error(CS, ast);
    return;
  }

  if (is_object(sem_type_item)) {
    report_error(CS, ast->left, "CQL0072: first operand cannot be an object in", operation);
    record_error(CS, ast);
    return;
  }

  // the min can't be compared with the item
  if (!sem_verify_compat(CS, ast, sem_type_item, sem_type_min, operation)) {
    return;
  }

  // the max can't be compared with the item
  if (!sem_verify_compat(CS, ast, sem_type_item, sem_type_max, operation)) {
    return;
  }
  ...
  if (!sem_verify_compat(CS, ast, sem_type_min, sem_type_max, operation_or_and)) {
    return;
  }

  // check for compatible kinds between item and the min
  sem_combine_kinds(CS, range->left, ast->left->sem->kind);
  ...
  sem_combine_kinds(CS, range->right, ast->left->sem->kind);
  ...
  sem_combine_kinds(CS, range->right, range->left->sem->kind);
  if (is_error(range->right)) {
    record_error(CS, ast);
    return;
  }
  ...
  if (CS->sem.current_expr_context == SEM_EXPR_CONTEXT_NONE) {
    ...
    Invariant(scope);
    CSTR name = dup_printf(CS, "_between_%d_", CS->sem.between_count++);

    // Implicitly declare the local variable we need
    ast_node *asts = new_ast_str(CS, name);
    asts->sem = new_sem(CS, expr->sem->sem_type | SEM_TYPE_VARIABLE);
    asts->sem->name = name;
    symtab_add(CS, scope, name, asts);

    ast_node *left_item = new_ast_str(CS, name);
    ast_node *right_item = new_ast_str(CS, name);
    ast_node *left_cmp;
    ast_node *right_cmp;
    ast_node *combine;

    if (between) {
      left_cmp = new_ast_ge(CS, left_item, range->left);
      right_cmp = new_ast_le(CS, right_item, range->right);
      combine = new_ast_and(CS, left_cmp, right_cmp);
    }
    else {
      left_cmp = new_ast_lt(CS, left_item, range->left);
      right_cmp = new_ast_gt(CS, right_item, range->right);
      combine = new_ast_or(CS, left_cmp, right_cmp);
    }

    ast_set_right(range, combine);
    ast_set_left(range, new_ast_str(CS, name));
...
static void sem_expr_cast(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (!is_numeric(data_type->sem->sem_type) || !is_numeric(expr->sem->sem_type)) {
    if (CURRENT_EXPR_CONTEXT_IS(SEM_EXPR_CONTEXT_NONE)) {
      report_error(CS, ast, "CQL0073: CAST may only appear in the context of SQL statement", NULL);
      record_error(CS, ast);
      return;
    }
  }

  if (CS->sem.enforcement.strict_cast) {
    if (core_type_of(data_type->sem->sem_type) == core_type_of(expr->sem->sem_type)) {
      ...
      if (same) {
        CSTR err_msg = dup_expr_text(CS, ast);
        report_error(CS, expr, "CQL0170: cast is redundant, remove to reduce code size", err_msg);
        record_error(CS, ast);
        return;
      }
    }
  ...
  ast->sem = new_sem(CS, data_type->sem->sem_type | combined_flags);
  ast->sem->kind = data_type->sem->kind;
}
...
static void sem_expr_type_check(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (core_type_of(sem_type_1) != core_type_of(sem_type_2)
    ...
    CSTR err_expr = dup_expr_text(CS, expr);
    CSTR error_message = "CQL0009: incompatible types in expression";
    report_sem_type_mismatch(CS, sem_type_2, sem_type_1, expr, error_message, err_expr);
    record_error(CS, expr);
    record_error(CS, ast);
    return;
  }
  ...
  if (!kind_left && !kind_right) {
    ...
    CSTR errmsg = dup_printf(CS, "CQL0070: expressions of different kinds can't be mixed: '%s' vs. '%s'", kind_left, kind_right);
    report_error(CS, ast, errmsg, NULL);
    record_error(CS, ast);
    return;
  }
...
static void sem_coalesce(CqlState* _Nonnull CS, ast_node *call_ast, bool_t is_ifnull) {
  ...
  if (arg_count < 2) {
    report_error(CS, name_ast, "CQL0074: too few arguments provided", name);
    record_error(CS, call_ast);
    return;
  }

  if (is_ifnull && arg_count != 2) {
    report_error(CS, name_ast, "CQL0075: incorrect number of arguments", name);
    record_error(CS, call_ast);
    return;
  }
  ...
  for (ast_node *ast = arg_list; ast; ast = ast->right) {
    ...
    if (is_ast_null(expr)) {
      report_error(CS, expr, "CQL0076: NULL literal is useless in function", name);
      record_error(CS, expr);
      record_error(CS, call_ast);
      return;
    }
    ...
    if (sem_type_result == SEM_TYPE_PENDING) {
      ...
      if (!sem_verify_compat(CS, expr, sem_type_result, sem_type_current, name)) {
        record_error(CS, expr);
        record_error(CS, call_ast);
        return;
      }
    ...
    if (is_not_nullable(sem_type_result) && ast->right) {
      CSTR err_msg = dup_expr_text(CS, expr);
      report_error(CS, expr, "CQL0077: encountered arg known to be not null"
                          " before the end of the list, rendering the rest useless.", err_msg);
      record_error(CS, call_ast);
      return;
    }

    kind_result = sem_combine_kinds(CS, expr, kind_result);
    if (is_error(expr)) {
      record_error(CS, call_ast);
      return;
    }
  ...
  call_ast->sem = new_sem(CS, sem_type_result | sem_sensitive);
...
static void sem_expr_in_pred_or_not_in(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (ast->right == NULL) {
    ...
    for (ast_node *item = expr_list; item; item = item->right) {
      ...
      sem_t sem_type_current = expr->sem->sem_type;
      if (!sem_verify_compat(CS, ast, sem_type_needed, sem_type_current, is_ast_in_pred(ast) ? "IN" : "NOT IN")) {
        return;
      }

      sem_combine_kinds(CS, expr, kind_needed);
      if (is_error(expr)) {
        record_error(CS, ast);
        return;
      }
    ...
    if (CURRENT_EXPR_CONTEXT_IS_NOT(valid)) {
      report_error(CS, ast, "CQL0078: [not] in (select ...) is only allowed inside "
    ...
    sem_expr_select(CS, (ast_node *)select_stmt, "SELECT");
    ...
    if (!sem_verify_compat(CS, ast, sem_type_needed, select_stmt->sem->sem_type, is_ast_in_pred(ast) ? "IN" : "NOT IN")) {
      return;
    }

    sem_combine_kinds(CS, select_stmt, kind_needed);
    if (is_error(select_stmt)) {
      record_error(CS, ast);
      return;
    }
...
static bool_t sem_validate_arg_count(CqlState* _Nonnull CS, ast_node *ast, uint32_t count, uint32_t expected) {
  ...
  if (count != expected) {
    report_error(CS, name_ast, "CQL0079: function got incorrect number of arguments", name);
    record_error(CS, name_ast);
    record_error(CS, ast);
    return false;
  }
...
static bool_t sem_validate_context(CqlState* _Nonnull CS, ast_node *ast, CSTR name, uint32_t valid_contexts) {
  ...
  report_error(CS, ast, "CQL0080: function may not appear in this context", name);
  record_error(CS, ast);
  return false;
}
...
static bool_t sem_validate_function_context(CqlState* _Nonnull CS, ast_node *ast, uint32_t valid_contexts) {
  ...
  return sem_validate_context(CS, ast, name, valid_contexts);
}
...
static void sem_expr_exists(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (!sem_validate_exists_context(CS, ast)) {
    return;
  }

  sem_select(CS, select_stmt);
  if (is_error(select_stmt)) {
    record_error(CS, ast);
    return;
  }
...
static bool_t sem_validate_window_context(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!sem_validate_function_context(CS, ast, SEM_EXPR_CONTEXT_WINDOW)) {
    return false;
  }
...
static bool_t sem_validate_aggregate_context(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // check the context first, better error message
  if (!sem_validate_function_context(CS, ast,
            SEM_EXPR_CONTEXT_SELECT_LIST |
            SEM_EXPR_CONTEXT_HAVING |
  ...
  if (!current_joinscope_lv || !current_joinscope_rv->jptr) {
    report_error(CS, ast, "CQL0081: aggregates only make sense if there is a FROM clause", name);
    record_error(CS, ast);
    return false;
  }
...
static void sem_special_func_cql_blob_get_type(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = false;
  ...
  if (!sem_validate_appear_inside_sql_stmt(CS, ast)) {
    return;
  }

  if (!sem_validate_arg_count(CS, ast, arg_count, 1)) {
    return;
  }

  ast_node *blob_expr = first_arg(arg_list);

  sem_expr(CS, blob_expr);
  if (is_error(blob_expr)) {
    record_error(CS, ast);
    return;
  }

  if (!sem_verify_compat(CS, blob_expr, blob_expr->sem->sem_type, SEM_TYPE_BLOB, "cql_blob_get_type")) {
    record_error(CS, ast);
    return;
  }
  ...
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_LONG_INTEGER | combined_flags);
}
...
static void sem_special_func_cql_blob_create(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = false;
  ...
  if (arg_count % 2 != 1) {
    ...
    sem_validate_arg_count(CS, ast, arg_count, arg_count + 1);
    return;
  }
  ...
  if (!is_ast_str(backed_name_ast)) {
    report_error(CS, backed_name_ast, "CQL0491: argument 1 must be a table name that is a backed table", "cql_blob_create");
    record_error(CS, ast);
    return;
  }
  ...
  ast_node *backed_table_ast = find_usable_and_not_deleted_table_or_view(CS,
      backed_table_name,
      backed_name_ast,
  ...
  if (!is_backed(backed_table_ast->sem->sem_type)) {
    report_error(CS, backed_table_ast, "CQL0488: the indicated table is not declared for backed storage", backed_table_name);
    record_error(CS, ast);
    return;
  }
  ...
  for (ast_node *next_arg = arg_list->right; next_arg; next_arg = next_arg->right->right) {
    ...
    if (!is_ast_dot(table_expr) || !is_ast_str(table_expr->left) || !is_ast_str(table_expr->right)) {
      report_error(CS, table_expr, "CQL0490: argument must be table.column where table is a backed table", "cql_blob_create");
      record_error(CS, ast);
      return;
    }
    ...
    // give a better error if the table is not found
    ast_node *table_ast = find_usable_and_not_deleted_table_or_view(CS,
        t_name,
        table_expr->left,
        "CQL0095: table/view not defined");
    if (!table_ast) {
    ...
    if (backed_table_ast != table_ast) {
      report_error(CS, table_expr, "CQL0488: the indicated table is not consistently used through all of cql_blob_create", t_name);
      record_error(CS, ast);
      return;
    }

    sem_t sem_type = find_column_type(CS, t_name, c_name);
    if (!sem_type) {
      CSTR err_data = dup_printf(CS, "%s.%s", t_name, c_name);
      report_error(CS, table_expr, "CQL0489: the indicated column is not present in the named backed storage", err_data);
      record_error(CS, ast);
      return;
    }

    if (!sem_verify_compat(CS, val_expr, val_expr->sem->sem_type, sem_type, "cql_blob_create")) {
      record_error(CS, ast);
      return;
    }

    table_expr->sem = new_sem(CS, sem_type);
  }

  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_BLOB | SEM_TYPE_NOTNULL);
}
...
static void sem_special_func_cql_blob_update(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = false;
  ...
  if (!sem_verify_compat(CS, blob_expr, blob_expr->sem->sem_type, SEM_TYPE_BLOB, "cql_blob_update")) {
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_ast_dot(dot) || !is_ast_str(dot->left) || !is_ast_str(dot->right)) {
    report_error(CS, dot, "CQL0490: argument must be table.column where table is a backed table", "cql_blob_update");
    record_error(CS, ast);
    return;
  }
  ...
  for (ast_node *next_arg = arg_list->right; next_arg; next_arg = next_arg->right->right) {
    ...
    if (!is_ast_dot(table_expr) || !is_ast_str(table_expr->left) || !is_ast_str(table_expr->right)) {
      report_error(CS, table_expr, "CQL0257: argument must be table.column where table is a backed table", "cql_blob_update");
      record_error(CS, ast);
      return;
    }
    ...
    if (backed_table_ast != table_ast) {
      report_error(CS, table_expr, "CQL0488: the indicated table is not consistently used through all of cql_blob_update", t_name);
      record_error(CS, ast);
      return;
    }
    ...
    if (!sem_verify_compat(CS, val_expr, val_expr->sem->sem_type, sem_type, "cql_blob_update")) {
      record_error(CS, ast);
      return;
    }
...
static void sem_special_func_cql_blob_get(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = false;
  ...
  if (!sem_validate_arg_count(CS, ast, arg_count, 2)) {
    return;
  }
  ...
  if (!sem_verify_compat(CS, blob_expr, blob_expr->sem->sem_type, SEM_TYPE_BLOB, "cql_blob_get")) {
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_ast_dot(table_expr) || !is_ast_str(table_expr->left) || !is_ast_str(table_expr->right)) {
    report_error(CS, table_expr, "CQL0490: argument must be table.column where table is a backed table", "cql_blob_get");
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_backed(table_ast->sem->sem_type)) {
    report_error(CS, table_expr, "CQL0488: the indicated table is not declared for backed storage", t_name);
    record_error(CS, ast);
    return;
  }
  ...
  table_expr->sem = name_ast->sem = ast->sem = new_sem(CS, sem_type | combined_flags);

  table_expr->sem->kind = find_column_kind(CS, t_name, c_name);
}
...
static void sem_special_func_count(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = true;
  ...
  sem_arg_list(CS, arg_list, IS_COUNT);
  if (arg_list && is_error(arg_list)) {
    record_error(CS, ast);
    return;
  }

  if (!sem_validate_aggregate_context(CS, ast)) {
    return;
  }
  ...
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_INTEGER | SEM_TYPE_NOTNULL | sensitive_flag(sem->sem_type));
...
static void sem_aggr_func_min_or_max(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (arg_count == 0) {
    sem_validate_arg_count(CS, ast, arg_count, 1);
    return;
  }
  ...
  if (!is_numeric(core_type) && !is_text(core_type)) {
    report_error(CS, ast, "CQL0257: argument must be a string or numeric in", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (arg_count == 1) {
    ...
    name_ast->sem = ast->sem = new_sem(CS, core_type | sensitive_flag(sem->sem_type));
  ...
  for (; ast_args; ast_args = ast_args->right) {
    ...
    if (!sem_verify_compat(CS, ast, sem_type_needed, sem_type_current, name)) {
      return;
    }
  ...
  name_ast->sem = ast->sem = new_sem(CS, sem_type_needed | combined_flags);
...
static void sem_func_round(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  // if too many args
  if (arg_count > 2) {
    sem_validate_arg_count(CS, ast, arg_count, 2);
    return;
  }
  ...
  if (core_type != SEM_TYPE_REAL) {
    report_error(CS, ast, "CQL0087: first argument must be of type real", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (arg_count == 2) {
    ...
    if (!is_numeric_expr(arg2)) {
      report_error(CS, name_ast, "CQL0082: second argument must be numeric", name);
      record_error(CS, ast);
      return;
    }
    sem_reject_real(CS, arg2, "ROUND argument 2");
    if (is_error(arg2)) {
      record_error(CS, ast);
      return;
    }
    combined_flags = combine_flags(sem_type, arg2->sem->sem_type);
    name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_REAL | combined_flags);
  }
  else {
    name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_LONG_INTEGER | combined_flags);
  }
...
static void sem_aggr_func_avg(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!is_numeric_expr(arg) || is_ast_null(arg)) {
    report_error(CS, name_ast, "CQL0082: argument must be numeric", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_func_cql_compressed(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!sem_validate_context(CS, ast, name, SEM_EXPR_CONTEXT_NONE)) {
    return;
  }
  ...
  if (!is_strlit(arg)) {
    report_error(CS, ast, "CQL0421: first argument must be a string literal", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_func_cql_get_blob_size(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!is_blob(arg->sem->sem_type)) {
    report_error(CS, ast, "CQL0345: argument must be of type blob", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_func_length(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!is_text(arg->sem->sem_type) && !is_blob(arg->sem->sem_type)) {
    report_error(CS, ast, "CQL0085: all arguments must be strings", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_func_trim(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  // one or two args
  if (arg_count != 1 && !sem_validate_arg_count(CS, ast, arg_count, 2)) {
    return;
  }
...
static void sem_func_nullif(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!sem_verify_compat(CS, arg1, arg1->sem->sem_type, arg2->sem->sem_type, "NULLIF")) {
    return;
  }

  sem_combine_kinds(CS, arg2, arg1->sem->kind);
  if (is_error(arg2)) {
    record_error(CS, ast);
    return;
  }
  ...
  name_ast->sem = ast->sem = new_sem(CS, arg1->sem->sem_type & sem_not(SEM_TYPE_NOTNULL));
  ast->sem->kind = arg1->sem->kind;
...
static void sem_func_instr(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_INTEGER | combine);
...
static void sem_func_sign(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (CS->sem.enforcement.strict_sign_function) {
...
static void sem_func_char(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  do {
    ...
    if (!is_any_int(sem_type)) {
      report_error(CS, ast, "CQL0317: char function arguments must be integer", name);
      record_error(CS, ast);
      return;
    }
  ...
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_TEXT | sensitive);
...
static bool_t sem_validate_cursor_from_variable(CqlState* _Nonnull CS, ast_node *ast, CSTR target) {
  ...
  report_error(CS, ast, "CQL0341: argument must be a variable in function", target);
  record_error(CS, ast);
  return false;
}
...
static void sem_func_attest_notnull(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, uint32_t valid_contexts) {
  ...
  if (!sem_validate_context(CS, ast, name, valid_contexts)) {
    return;
  }
  ...
  if (is_null_type(sem_type) || is_not_nullable(sem_type)) {
    report_error(CS, arg1, "CQL0344: argument must be a nullable type (but not constant NULL) in", name);
    record_error(CS, ast);
    return;
  }

  ast->sem = arg1->sem;
  sem_add_flags(CS, ast, SEM_TYPE_NOTNULL); // note this makes a copy
...
static void sem_special_func_cql_inferred_notnull(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  CS->sem.is_analyzing_notnull_rewrite = true;
  sem_arg_list(CS, arg_list, IS_NOT_COUNT);
  CS->sem.is_analyzing_notnull_rewrite = false;
  ...
  sem_func_attest_notnull(CS, ast, arg_count, SEM_EXPR_CONTEXT_FLAGS);
...
static bool_t validate_cql_cursor_diff(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!sem_validate_cursor_from_variable(CS, arg1, name) ||
      !sem_validate_cursor_from_variable(CS, arg2, name)) {
    record_error(CS, ast);
    return false;
  }
  ...
  if (sptr1->count != sptr2->count) {
    report_error(CS, ast, "CQL0342: cursor arguments must have identical column count", name);
    record_error(CS, ast);
    return false;
  }
  ...
  if (!is_auto_cursor(arg1->sem->sem_type) || !is_auto_cursor(arg2->sem->sem_type)) {
    ...
    report_error(CS, arg1, "CQL0067: cursor was not used with 'fetch [cursor]'", cursor_name);
    record_error(CS, arg1);
    record_error(CS, ast);
    return false;
  }
  ...
  CSTR target = dup_printf(CS, "in %s", name);
  sem_verify_identical_columns(CS, arg1, arg2, target);
  if (is_error(arg2)) {
    record_error(CS, ast);
    return false;
  }
  ...
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_TEXT);
...
static void sem_func_cql_cursor_diff_val(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  // a case_expr node.
  rewrite_cql_cursor_diff(CS, ast, false);
}
...
static void sem_func_upper(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!is_text(arg->sem->sem_type)) {
    report_error(CS, name_ast, "CQL0086: first argument must be a string in function", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_validate_sum_or_total(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!is_numeric_expr(arg) || is_ast_null(arg)) {
    report_error(CS, name_ast, "CQL0083: argument must be numeric", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_aggr_func_sum(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  sem_validate_sum_or_total(CS, ast, arg_count);
  if (is_error(ast)) {
    return;
  }
  ...
  name_ast->sem = ast->sem = new_sem(CS, result);
...
static void sem_aggr_func_total(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  // set the result type accordingly
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_REAL | SEM_TYPE_NOTNULL | sensitive_flag(arg->sem->sem_type));
...
static void sem_func_substr(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (!is_text(arg1->sem->sem_type)) {
    report_error(CS, ast, "CQL0086: first argument must be a string in function", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (result.sem_type != SEM_TYPE_ERROR && result.sem_type != SEM_TYPE_NULL) {
    ...
    if (result.int64_value == 0) {
      report_error(CS, arg2, "CQL0406: substr uses 1 based indices, the 2nd argument of substr may not be zero", NULL);
      record_error(CS, arg2);
      record_error(CS, ast);
      return;
    }
  }
  ...
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_TEXT | flags);
...
static void sem_validate_window_func(CqlState* _Nonnull CS,
  ...
  if (!sem_validate_window_context(CS, ast)) {
    return;
  }
  ...
  if (arg_count_actual < arg_count_needed_min || arg_count_actual > arg_count_needed_max) {
    ...
    sem_validate_arg_count(CS, ast, arg_count_needed_max, arg_count_actual);
    return;
  }

  name_ast->sem = ast->sem = new_sem(CS, sem_func_return);
}
...
static void sem_func_ntile(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  sem_validate_window_func(CS, ast, arg_count, 1, 1, SEM_TYPE_INTEGER | SEM_TYPE_NOTNULL);
  if (is_error(ast)) {
    return;
  }

  ast_node *arg = first_arg(arg_list);
  if (!is_num_int_in_range(CS, arg, 1, INT_MAX)) {
    report_error(CS, ast, "CQL0300: argument must be an integer (between 1 and max integer) in function", name);
    record_error(CS, ast);
...
static void sem_func_lag(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  sem_validate_window_func(CS, ast, arg_count, 1, 3, SEM_TYPE_OK);
  if (is_error(ast)) {
    return;
  }
  ...
  if (arg_count > 1) {
    ast_node *arg2 = second_arg(arg_list);
    if (!is_num_int_in_range(CS, arg2, 0, INT_MAX)) {
      report_error(CS, ast, "CQL0301: second argument must be an integer (between 0 and max integer) in function", name);
      record_error(CS, ast);
      record_error(CS, arg_list);
      return;
    }
  ...
  if (arg_count > 2) {
    ...
    bool_t ok = sem_verify_assignment(CS, arg3, arg1_effective, arg3_effective, "arg3 used as default value for arg1");

    // now check the type<kind>

    if (ok) {
      sem_combine_kinds(CS, arg3, arg1->sem->kind);
      ok = !is_error(arg3);
    }

    if (!ok) {
      report_error(CS, ast, "CQL0302: first and third arguments must be compatible in function", name);
      record_error(CS, ast);
      record_error(CS, arg_list);
      return;
    }
  ...
  ast->sem = arg1->sem;
  sem_replace_flags(CS, ast, type | combined_flags);
  name_ast->sem = ast->sem;
}
...
static void sem_func_first_value(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  sem_validate_window_func(CS, ast, arg_count, 1, 1, SEM_TYPE_OK);
  if (is_error(ast)) {
    return;
  }
...
static void sem_func_nth_value(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  sem_validate_window_func(CS, ast, arg_count, 2, 2, SEM_TYPE_OK);
  if (is_error(ast)) {
    return;
  }
  ...
  ast_node *arg1 = first_arg(arg_list);
  ast_node *arg2 = second_arg(arg_list);
  if (!is_num_int_in_range(CS, arg2, 1, INT_MAX)) {
    report_error(CS, ast, "CQL0303: second argument must be an integer between 1 and max integer in function", name);
    record_error(CS, ast);
    record_error(CS, arg_list);
    return;
  }
...
static void sem_aggr_func_group_concat(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (arg_count == 2) {
    ...
    if (!is_text(sem_type_2)) {
      report_error(CS, ast, "CQL0084: second argument must be a string in function", name);
      record_error(CS, ast);
      return;
    }
  }

  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_TEXT | combined_flags);
...
static void sem_strftime(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t has_format, sem_t sem_type) {
  ...
  if (arg_count < min_arg_count) {
    sem_validate_arg_count(CS, ast, arg_count, min_arg_count);
    return;
  }
  ...
  if (arg_count == 2 && has_format) {
    ...
    if (is_ast_str(first) && is_ast_str(second)) {
       ...
       if (!strcmp(arg1, "'%s'") && !strcmp(arg2, "'now'")) {
         ...
         if (!sem_validate_sql_not_constraint(CS, ast)) {
           return;
         }
...
static void sem_special_func_ptr(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_LONG_INTEGER | SEM_TYPE_NOTNULL);
}
...
static void sem_func_sensitive(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  // add sensitive
  ast->sem = arg->sem;
  sem_add_flags(CS, ast, SEM_TYPE_SENSITIVE); // note this makes a copy
  name_ast->sem = ast->sem;
}
...
static void sem_func_nullable(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  ast->sem = arg->sem;
  sem_remove_flags(CS, ast, SEM_TYPE_NOTNULL); // note this makes a copy
  name_ast->sem = ast->sem;
}
...
static void sem_func_changes(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  if (sem_validate_db_func_with_no_args(CS, ast, arg_count)) {
    return;
  }

  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_INTEGER | SEM_TYPE_NOTNULL);
}
  ...
  CSTR proc_name)
{
  ...
  for (ast_node *arg_item = arg_list; arg_item; arg_item = arg_item->right) {
    ...
    if (sem_type == SEM_TYPE_OK) {
      report_error(CS, arg_list, "CQL0422: more arguments provided than expected by format string", proc_name);
      return false;
    }
    ast_node *arg = arg_item->left;
    if (!sem_verify_assignment(CS, arg, sem_type, core_type_of(arg->sem->sem_type), proc_name)) {
      return false;
    }
  }
  ...
  if (sem_type != SEM_TYPE_OK) {
    report_error(CS, arg_list, "CQL0423: fewer arguments provided than expected by format string", proc_name);
    return false;
  }
...
static CSTR format_string_from_format_strlit(CqlState* _Nonnull CS, ast_node *format_strlit) {
  ...
  result = Strdup(CS, format_string_buf.ptr);
...
static void sem_func_printf(CqlState* _Nonnull CS, ast_node *ast, uint32_t arg_count) {
  ...
  CSTR format_string = format_string_from_format_strlit(CS, format_strlit);

  // Verify that the arguments are appropriate for the format string provided.
  ast_node *args_for_format = arg_list->right;
  if (!sem_validate_args_for_format(CS, args_for_format, format_strlit, format_string, name)) {
    record_error(CS, ast);
    return;
  }

  name_ast->sem = ast->sem = new_sem(CS, SEM_TYPE_TEXT | SEM_TYPE_NOTNULL);
  ...
  if (CS->sem.current_expr_context == SEM_EXPR_CONTEXT_NONE) {
    ...
    if (!is_rewriting) {
      is_rewriting = true;
      rewrite_printf_inserting_casts_as_needed(CS, ast, format_string);
      is_rewriting = false;
    }
  }
}
...
static void sem_user_func(CqlState* _Nonnull CS, ast_node *ast, ast_node *user_func) {
  ...
  if (!select_func) {
    if (CURRENT_EXPR_CONTEXT_IS_NOT(SEM_EXPR_CONTEXT_NONE)) {
      report_error(CS, ast, "CQL0088: user function may not appear in the context of a SQL statement", name);
      record_error(CS, ast);
      return;
    }
    ...
    if (CURRENT_EXPR_CONTEXT_IS(SEM_EXPR_CONTEXT_NONE)) {
      report_error(CS, ast, "CQL0089: user function may only appear in the context of a SQL statement", name);
      record_error(CS, ast);
      return;
    }
    ...
    if (CURRENT_EXPR_CONTEXT_IS(SEM_EXPR_CONTEXT_CONSTRAINT) && !deterministic) {
      report_error(CS, ast, "CQL0393: not deterministic user function cannot appear in a constraint expression ", name);
      record_error(CS, ast);
      return;
    }
  ...
  if (is_struct(user_func->sem->sem_type)) {
    report_error(CS, ast, "CQL0395: table valued functions may not be used in an expression context", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (no_check) {
    ...
    if (sql_udf_context) {
      PUSH_EXPR_CONTEXT(SEM_EXPR_CONTEXT_UDF);
        sem_validate_args_vs_formals(CS, ast, name, arg_list, params, NORMAL_CALL);
      POP_EXPR_CONTEXT();
    }
    else {
      sem_validate_args_vs_formals(CS, ast, name, arg_list, params, NORMAL_CALL);
...
static void sem_validate_expression_fragment(CqlState* _Nonnull CS, ast_node *ast, ast_node *proc) {
  ...
  EXTRACT_STRING(proc_name, get_proc_name(CS, proc));
  EXTRACT_NOTNULL(call_arg_list, ast->right);
  EXTRACT(arg_list, call_arg_list->right);
  ...
  if (!is_proc_shared_fragment(CS, proc)) {
    report_error(CS, ast,
  ...
  if (sptr->count > 1) {
      report_error(CS, ast, "CQL0232: nested select expression must return exactly one column", proc_name);
      record_error(CS, ast);
      return;
  }
  ...
  if (!is_no_clause_simple_select(select_stmt)) {
    report_error(CS, ast, "CQL0450: a shared fragment used like a function must be a simple SELECT with no FROM clause", proc_name);
    record_error(CS, ast);
    return;
  }

  if (has_nested_shared_cte_with_args(CS, select_stmt)) {
    report_error(CS, ast, "CQL0467: a shared fragment used like a function cannot nest fragments that use arguments", proc_name);
    record_error(CS, ast);
    return;
  }

  sem_validate_args_vs_formals(CS, ast, proc_name, arg_list, params, NORMAL_CALL);
  if (is_error(ast)) {
    return;
  }
...
static void sem_proc_as_func(CqlState* _Nonnull CS, ast_node *ast, ast_node *proc) {
  ...
  if (is_error(proc)) {
    report_error(CS, ast, "CQL0213: procedure had errors, can't call", proc_name);
    record_error(CS, ast);
    return;
  }
  ...
  if (distinct || opt_filter_clause) {
    report_error(CS, ast, "CQL0451: procedure as function call is not compatible with DISTINCT or filter clauses", proc_name);
    record_error(CS, ast);
    return;
  }
  ...
  if (CURRENT_EXPR_CONTEXT_IS_NOT(SEM_EXPR_CONTEXT_NONE)) {
    ...
    sem_validate_expression_fragment(CS, ast, proc);
    return;
  }
  ...
  sem_validate_args_vs_formals(CS, ast, proc_name, arg_list, params, validation_type);
  ...
  if (result_set_return && !is_error(ast)) {
    ...
    ast->sem = new_sem(CS, SEM_TYPE_OBJECT | SEM_TYPE_NOTNULL);
    ast->sem->kind = dup_printf(CS, "%s SET", proc_name);
  }

  // The call may have mutated any or all of the currently improved globals, so
  // we simply invalidate all of them.
  sem_unset_global_notnull_improvements(CS);

  CS->sem.has_dml |= is_dml_proc(proc->sem->sem_type);
}
...
static void sem_expr_raise(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (!CS->sem.in_trigger) {
    report_error(CS, ast, "CQL0092: RAISE may only be used in a trigger statement", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  if (expr) {
    ...
    if (!is_text(expr->sem->sem_type)) {
      report_error(CS, expr, "CQL0093: RAISE 2nd argument must be a string", NULL);
      record_error(CS, ast);
      return;
    }
  }
...
static void sem_validate_array_transform(CqlState* _Nonnull CS, ast_node *ast,  CSTR op) {
  ...
  if (!array->sem->kind) {
    report_error(CS, array, "CQL0470: operation is only available for types with a declared type kind like object<something>", "[]");
    record_error(CS, array);
    record_error(CS, ast);
    return;
  }

  rewrite_array_as_call(CS, ast, op);
  record_ok(CS, ast);
}
...
static void sem_expr_call(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (opt_filter_clause) {
    sem_opt_filter_clause(CS, opt_filter_clause);
  ...
  if (!rewrite_shape_forms_in_list_if_needed(CS, arg_list)) {
    record_error(CS, ast);
    return;
  }
  ...
  if (distinct && (arg_count != 1 || is_ast_star(first_arg(arg_list)))) {
    report_error(CS, ast, "CQL0304: DISTINCT may only be used with one explicit argument in an aggregate function", name);
    record_error(CS, ast);
    return;
  }

  // check for functions
  symtab_entry *entry = symtab_find(CS->sem.builtin_funcs, name);
  if (!entry) {
    // check for aggregate functions
    entry = symtab_find(CS->sem.builtin_aggregated_funcs, name);
    if (entry) {
  ...
  if (entry) {
    ...
    ((sem_func *)entry->val)(CS, ast, arg_count);
    goto additional_checks;
  }

  // check for special functions which do their own analysis of their arguments
  entry = symtab_find(CS->sem.builtin_special_funcs, name);
  if (entry) {
    ((sem_special_func *)entry->val)(CS, ast, arg_count, &call_aggr_or_user_def_func);
    goto additional_checks;
  }

  // check for user defined functions
  ast_node *user_func = find_func(CS, name);
  if (user_func) {
    sem_user_func(CS, ast, user_func);
    call_aggr_or_user_def_func = 1;
    goto additional_checks;
  }

  ast_node *unchecked_user_func = find_unchecked_func(CS, name);
  if (unchecked_user_func) {
    ...
    sem_user_func(CS, ast, unchecked_user_func);
    call_aggr_or_user_def_func = 1;
    goto additional_checks;
  }
  ...
  ast_node *unchecked_proc = find_unchecked_proc(CS, name);
  if (unchecked_proc) {
    report_error(CS, name_ast, "CQL0405: procedure of an unknown type used in an expression", name);
    record_error(CS, ast);
    return;
  }

  report_error(CS, name_ast, "CQL0094: function not yet implemented", name);
  record_error(CS, ast);
  return;
...
additional_checks:
  if (!call_aggr_or_user_def_func) {
    if (distinct) {
      ...
      report_error(CS, ast, "CQL0305: DISTINCT may only be used in function that are aggregated or user defined", name);
      record_error(CS, ast);
      return;
    }
    ...
    if (opt_filter_clause) {
      ...
      report_error(CS, ast, "CQL0306: FILTER clause may only be used in function that are aggregated or user defined", name);
      record_error(CS, ast);
      return;
    }
  }
...
static bool_t sem_reverse_apply_if_needed(CqlState* _Nonnull CS, ast_node *ast, bool_t analyze) {
  ...
  if (op[0] == ':' && op[1] != '=') {
    ...
    if (!sem_try_as_cursor(CS, ast->left, &hard_fail) && !hard_fail) {
      sem_expr(CS, ast->left);
      hard_fail = is_error(ast->left);
    }
    if (!hard_fail) {
      rewrite_reverse_apply(CS, ast, op);
      if (analyze) {
        sem_expr_call(CS, ast, op);
...
static void sem_opt_partition_by(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_validate_args(CS, ast, expr_list);
...
static void sem_opt_frame_spec(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (frame_boundary_flags) {
    ...
    if (left_expr) {
      sem_root_expr(CS, left_expr, SEM_EXPR_CONTEXT_WHERE);
    ...
    if (right_expr) {
      sem_root_expr(CS, right_expr, SEM_EXPR_CONTEXT_WHERE);
...
static void sem_window_defn(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (opt_partition_by) {
    sem_opt_partition_by(CS, opt_partition_by);
    error = is_error(opt_partition_by);
  }
  if (opt_orderby) {
    sem_opt_orderby(CS, opt_orderby);
    error |= is_error(opt_orderby);
  }
  if (opt_frame_spec) {
    sem_opt_frame_spec(CS, opt_frame_spec);
...
static void sem_window_reference(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  report_error(CS, ast->left, "CQL0296: window name definition is not used", window_name);
  record_error(CS, ast->left);
  record_error(CS, ast);
}
...
static void sem_window_name_defn(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // name AS [window_defn]
  sem_window_reference(CS, ast);
  if (is_error(ast)) {
    return;
  }

  sem_window_defn(CS, window_defn);
...
static bool_t sem_window_name_defn_list(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (window_name_defn_list) {
    sem_window_name_defn_list(CS, window_name_defn_list);
    error = is_error(window_name_defn_list->left);
  }

  sem_window_name_defn(CS, window_name_defn);
...
static void sem_window_clause(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  error = sem_window_name_defn_list(CS, window_name_defn_list);
...
static void sem_window_name(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!valid) {
    report_error(CS, ast, "CQL0295: window name is not defined", window_name);
    record_error(CS, ast);
    return;
  }
  record_ok(CS, ast);
...
static void sem_window_name_or_defn(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_str(ast)) {
    sem_window_name(CS, ast);
    error = is_error(ast);
  }
  else {
    Contract(is_ast_window_defn(ast));
    sem_window_defn(CS, ast);
    error = is_error(ast);
  }
...
static void sem_expr_window_func_inv(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (CURRENT_EXPR_CONTEXT_IS_NOT(SEM_EXPR_CONTEXT_SELECT_LIST)) {
    report_error(CS, ast, "CQL0294: window function invocations can only appear in the select list of a select statement", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  sem_expr_call(CS, call, cstr);
  error = is_error(call);
  POP_EXPR_CONTEXT();

  sem_window_name_or_defn(CS, window_name_or_defn);
...
static void sem_as_alias(CqlState* _Nonnull CS, ast_node *ast, CSTR *alias_target, ast_node *ast_target) {
  ...
  if (is_qid(ast->left) && ast_target) {
    sem_add_flags(CS, ast_target, SEM_TYPE_QID);
  }
}
...
static void sem_select_expr(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // expr [AS alias]
  sem_root_expr(CS, expr, SEM_EXPR_CONTEXT_SELECT_LIST);
  ...
  ast->sem = new_sem(CS, expr->sem->sem_type);
  ast->sem->name = expr->sem->name;
  ast->sem->kind = expr->sem->kind;

  if (opt_as_alias) {
    sem_as_alias(CS, opt_as_alias, &ast->sem->name, ast);
  }
}
...
static void sem_select_expr_list(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_star(ast->left)) {
    // select * from [etc]
    if (ast->right) {
       report_error(CS, ast, "CQL0474: when '*' appears in an expression list there can be nothing else in the list", NULL);
       record_error(CS, ast);
       return;
    }
    sem_select_star(CS, ast->left);
  ...
  for (; node; node = node->right) {
    if (is_ast_star(node->left)) {
      ...
      sem_expr_invalid_op(CS, node->left, "*");
      return;
    }
    if (is_ast_table_star(node->left)) {
      EXTRACT_NOTNULL(table_star, node->left);
      count += sem_select_table_star_count(CS, table_star);
      ...
      sem_select_expr(CS, select_expr);
  ...
  for (ast_node *snode = ast; snode; snode = snode->right) {
    if (is_ast_table_star(snode->left)) {
      ...
      i = sem_select_table_star_add(CS, table_star, sptr, i);
    }
    else {
...
static ast_node *sem_find_table(CqlState* _Nonnull CS, CSTR name, ast_node *ast_error) {
  ...
  if (!table_ast) {
    table_ast = find_usable_and_not_deleted_table_or_view(CS,
      ...
      "CQL0095: table/view not defined");
    if (!table_ast) {
      record_error(CS, ast_error);
    }
    else {
      sem_non_blob_storage_table(CS, ast_error, table_ast);
...
static void sem_table_or_subquery(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_str(factor)) {
    ...
    ast_node *table_ast = sem_find_table(CS, name, ast);
    if (!table_ast) {
      return;
    }
    ...
    if (is_backed(table_ast->sem->sem_type)) {
      add_item_to_list(CS, &CS->sem.backed_tables_list, ast);
    }

    sem_node *sem = new_sem(CS, SEM_TYPE_JOIN);
    sem->jptr = sem_join_from_sem_struct(CS, table_ast->sem->sptr);
    ast->sem = factor->sem = sem;
    alias_target = &ast->sem->jptr->names[0];
  }
  else if (is_select_stmt(factor)) {
    // [SELECT ...]
    sem_select(CS, factor);
    ...
    ast->sem = new_sem(CS, SEM_TYPE_JOIN);
    ast->sem->jptr = sem_join_from_sem_struct(CS, factor->sem->sptr);
    alias_target = &ast->sem->jptr->names[0];
  }
  else if (is_ast_shared_cte(factor)) {
    // [CALL shared_fragment ...]
    sem_shared_cte(CS, factor);
    ...
    // this is all that's left...
    sem_query_parts(CS, factor);
    ast->sem = factor->sem;
  }

  EXTRACT(opt_as_alias, ast->right);
  if (opt_as_alias) {
    sem_as_alias(CS, opt_as_alias, alias_target, NULL);
    ...
    if (is_ast_str(factor) && CS->sem.keep_table_name_in_aliases && !CS->sem.in_trigger && !CS->sem.in_trigger_when_expr) {
      EXTRACT_STRING(table_name, factor);
      EXTRACT_STRING(original_alias_name, opt_as_alias->left)
      insert_table_alias_string_overide(CS, opt_as_alias->left, table_name);
    }
  }
}
...
static sem_t sem_join_using_columns(CqlState* _Nonnull CS, ast_node *join, ast_node *join_cond, sem_join *left, sem_join *right) {
  ...
  if (!sem_verify_no_duplicate_names(CS, name_list)) {
    record_error(CS, join_cond);
    record_error(CS, join);
    return false;
  }
  ...
  for (ast_node *ast = name_list; ast; ast = ast->right) {
    ...
    // check left (and only the left!)
    {
      ...
      if (!sem_find_column_for_name(CS, ast, name)) {
        report_error(CS, ast, "CQL0096: join using column not found on the left side of the join", name);
        record_error(CS, join_cond);
        record_error(CS, join);
      }
      POP_JOIN();
    ...
    // and then right (and only the right!)
    {
      ...
      if (!sem_find_column_for_name(CS, ast, name)) {
        report_error(CS, ast, "CQL0097: join using column not found on the right side of the join", name);
        record_error(CS, join_cond);
        record_error(CS, join);
      }
      POP_JOIN();
    ...
    if (core_type_of(sem_type_left) != core_type_of(sem_type_right)) {
      report_error(CS, ast, "CQL0098: left/right column types in join USING(...) do not match exactly", name);
      record_error(CS, join);
      return false;
    }
  }
...
static sem_t sem_join_cond_on(CqlState* _Nonnull CS, ast_node *join, ast_node *join_cond) {
  ...
  sem_numeric_expr(CS, expr, cond_type, "ON", SEM_EXPR_CONTEXT_ON);
  POP_JOIN();
...
static void sem_join_target(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_table_or_subquery(CS, table_or_subquery);
  ...
  join_tables(CS, table_ref, table_or_subquery, ast, join_type);
  ...
  if (join_cond) {
    ...
    if (is_ast_on(cond_type)) {
      sem_type = sem_join_cond_on(CS, ast, join_cond);
    }
    else {
      Contract(is_ast_using(cond_type)); // only other type
      sem_type = sem_join_using_columns(CS, ast, join_cond, table_ref->sem->jptr, table_or_subquery->sem->jptr);
    }
...
static void sem_table_or_subquery_list(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!table_or_subquery_list) {
    ...
    if (CS->sem.enforcement.strict_join) {
      report_error(CS, ast, "CQL0263: non-ANSI joins are forbidden if strict join mode is enabled", NULL);
      record_error(CS, ast);
      return;
    }

    sem_table_or_subquery_list(CS, table_or_subquery_list);

    // not really an inner join but that gives the correct nullability
    join_tables(CS, table_or_subquery, table_or_subquery_list, ast, JOIN_INNER);
  }
}
...
static void sem_join_target_list(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  do {
    ...
    sem_join_target(CS, join_target);
...
static void sem_join_clause(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_table_or_subquery(CS, table_or_subquery);
  sem_join_target_list(CS, join_target_list);
...
static void sem_table_function(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!user_func) {
    report_error(CS, name_ast, "CQL0250: table-valued function not declared", name);
    record_error(CS, ast);
    return;
  }

  if (!is_struct(user_func->sem->sem_type)) {
    report_error(CS, name_ast, "CQL0249: function is not a table-valued-function", name);
    record_error(CS, ast);
    return;
  }
  ...
  sem_node *sem = new_sem(CS, SEM_TYPE_JOIN|SEM_TYPE_TVF);
  sem->jptr = sem_join_from_sem_struct(CS, user_func->sem->sptr);
...
static void sem_groupby_list(CqlState* _Nonnull CS, ast_node *head) {
  ...
  for (ast_node *ast = head; ast; ast = ast->right) {
    ...
    sem_root_expr(CS, expr, SEM_EXPR_CONTEXT_GROUP_BY);
    if (is_error(expr)) {
      record_error(CS, head);
      return;
    }
...
static void sem_orderby_list(CqlState* _Nonnull CS, ast_node *head) {
  ...
  for (ast_node *ast = head; ast; ast = ast->right) {
    ...
    sem_root_expr(CS, expr, SEM_EXPR_CONTEXT_ORDER_BY);
    if (is_error(expr)) {
      record_error(CS, head);
      return;
    }
...
static void sem_opt_where(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // WHERE [ast->left]
  sem_numeric_expr(CS, ast->left, ast, "WHERE", SEM_EXPR_CONTEXT_WHERE);
}
...
static void sem_opt_having(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // HAVING [ast->left]
  sem_numeric_expr(CS, ast->left, ast, "HAVING", SEM_EXPR_CONTEXT_HAVING);
}
...
static void sem_opt_select_window(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // WINDOW [ast->left]
  sem_window_clause(CS, window_clause);
...
static void sem_opt_groupby(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_groupby_list(CS, groupby_list);
...
static void sem_opt_orderby(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_orderby_list(CS, orderby_list);
...
static void sem_opt_limit(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // LIMIT [ast->left]
  sem_numeric_expr(CS, ast->left, ast, "LIMIT", SEM_EXPR_CONTEXT_LIMIT);
...
static void sem_opt_offset(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // LIMIT [ast->left]
  sem_numeric_expr(CS, ast->left, ast, "OFFSET", SEM_EXPR_CONTEXT_OFFSET);
...
static void sem_select_from(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // FROM [query_parts]
 if (query_parts) {
    sem_query_parts(CS, query_parts);
...
static bool_t sem_select_orderby(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (opt_offset) {
    if (!opt_limit) {
      report_error(CS, opt_offset, "CQL0271: OFFSET clause may only be used if LIMIT is also present", NULL);
      record_error(CS, opt_offset);
      error = true;
    }
    else {
      sem_opt_offset(CS, opt_offset);
...
static sem_t sem_select_where_etc(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (opt_groupby) {
    sem_opt_groupby(CS, opt_groupby);
    error |= is_error(opt_groupby);
  }

  if (opt_having && !opt_groupby) {
    error = true;
    report_error(CS, opt_having, "CQL0099: HAVING clause requires GROUP BY clause", NULL);
    record_error(CS, opt_having);
  }

  if (opt_having) {
    // HAVING
    sem_opt_having(CS, opt_having);
    error |= is_error(opt_having);
  }

  if (opt_select_window) {
    // WINDOW
    sem_opt_select_window(CS, opt_select_window);
...
static sem_struct *select_expr_list_alias_sptr(CqlState* _Nonnull CS, ast_node *select_expr_list) {
  ...
  for (ast_node *item = select_expr_list; item; item = item->right) {
    ...
    if (opt_as_alias) {
      add_item_to_list(CS, &aliases, opt_as_alias);
      alias_count++;
    }
  }

  sem_struct *sptr = new_sem_struct(CS, "aliases", alias_count);
...
static void sem_select_expr_list_con(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_select_from(CS, select_from_etc);
  ...
  if (!error) {
    ...
    rewrite_select_expr_list(CS, ast, from_jptr);
    error = is_error(ast);
  }
  ...
  if (!error) {
    ...
    PUSH_JOIN(alias_scope, sem_join_from_sem_struct(CS, select_expr_list_alias_sptr(CS, select_expr_list)));
    {
      if (query_parts) {
        PUSH_JOIN(from_scope, from_jptr);
        sem_sensitive = sem_select_where_etc(CS, select_from_etc);
        error = is_error(select_from_etc);
        if (!error && opt_where) {
          EXTRACT_ANY_NOTNULL(where_expr, opt_where->left);
          sem_set_improvements_for_true_condition(CS, where_expr);
        }
        POP_JOIN();
      } else {
        sem_sensitive = sem_select_where_etc(CS, select_from_etc);
  ...
  if (!error) {
    ...
    if (query_parts) {
      PUSH_JOIN(from_scope, from_jptr);
      sem_select_expr_list(CS, select_expr_list);
      POP_JOIN();
    } else {
      sem_select_expr_list(CS, select_expr_list);
    }
  ...
  if (error) {
    record_error(CS, ast);
    record_error(CS, select_expr_list);
  }
}
...
static void sem_values(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!rewrite_shape_forms_in_list_if_needed(CS, insert_list)) {
    record_error(CS, ast);
    return;
  }
  ...
  sem_struct *sptr = new_sem_struct(CS, "values", total_count);
  ...
  for (ast_node *values = ast; values; values = values->right) {
    ...
    if (values_insert_list == NULL) {
      report_error(CS, ast, "CQL0336: select statement with VALUES clause requires a non empty list of values", NULL);
      record_error(CS, ast);
      return;
    }

    // if there are any FROM C(like shape) thing in the values list, expand them
    if (!rewrite_shape_forms_in_list_if_needed(CS, values_insert_list)) {
      record_error(CS, ast);
      return;
    }
    ...
    for (ast_node *node = values_insert_list; node; node = node->right) {
      ...
      if (values_count >= total_count) {
        report_error(CS, expr, "CQL0337: the number of columns values for each row should be identical in VALUES clause", NULL);
        record_error(CS, expr);
        record_error(CS, ast);
        return;
      }
      ...
      sem_name = dup_printf(CS, "column%d", values_count + 1);

      if (sem_type == SEM_TYPE_ERROR) {
        sem_type = expr->sem->sem_type;
      }
      else {
        if (!sem_verify_compat(CS, expr, sem_type, expr->sem->sem_type, "VALUES clause")) {
          record_error(CS, ast);
          return;
        }
    ...
    if (total_count != values_count) {
      report_error(CS, last_expr, "CQL0337: the number of columns values for each row should be identical in VALUES clause", NULL);
      record_error(CS, last_expr);
      record_error(CS, ast);
      return;
    }
  }
...
static void sem_select_core(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_select_values(any_select_opts)) {
    ...
    sem_values(CS, select_core_right);
  } else {
    ...
    sem_select_expr_list_con(CS, select_core_right);
  }
    ...
    *used_symbols = add_symbols;
    ...
    for (uint32_t i = 0; i < add_symbols->capacity; i++) {
      if (add_symbols->payload[i].sym) {
        symtab_add(CS, *used_symbols, add_symbols->payload[i].sym, NULL);
      }
    }
  }
}
...
static bool_t sem_select_orderby_with_simple_ordering_only(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  for (ast_node *list = orderby_list; list; list = list->right) {
    ...
    report_error(CS, expr, "CQL0398: compound select cannot be ordered by the result of an expression", NULL);
    record_error(CS, ast);
    return 1;
  }
...
static void sem_select_no_with(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_select_core_list(CS, select_core_list);
  ...
  PUSH_JOIN(list_scope, sem_join_from_sem_struct(CS, select_core_right->sem->sptr));
  {
    PUSH_MONITOR_SYMTAB();
    if (select_core_compound || is_ast_values(select_core->right)) {
      ...
      // [select_insert_list].
      error = sem_select_orderby_with_simple_ordering_only(CS, select_orderby);
    }
    else {
      // [SELECT ...]
      ...
      error = sem_select_orderby(CS, select_orderby);
      POP_JOIN();
    }
    POP_MONITOR_SYMTAB();
  }
  ...
  sem_add_used_symbols(CS, &select_core_list->sem->used_symbols, used_symbols);
...
static void sem_select_core_list(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_select_core(CS, select_core);
  ...
  sem_struct *sptr = sem_unify_compatible_columns(CS, select_core, select_core_list);
  if (!sptr) {
    record_error(CS, ast);
    return;
  }

  sem_add_used_symbols(CS, &select_core->sem->used_symbols, select_core_list->sem->used_symbols);
...
static void sem_select_rewrite_backing(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (BACKING_REWRITE_NEEDED(ast, target_table)) {
    rewrite_select_for_backed_tables(CS, ast, CS->sem.backed_tables_list);
  }
...
static void sem_select_nothing_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!is_proc_shared_fragment(CS, CS->sem.current_proc)) goto error;
  ...
  ast->sem = new_sem(CS, CS->sem.current_proc->sem->sem_type);
  ast->sem->sptr = CS->sem.current_proc->sem->sptr;
  return;

error:
  report_error(CS, ast, "CQL0496: SELECT NOTHING may only appear in the ELSE clause of a shared fragment", NULL);
  record_error(CS, ast);
}
...
static void sem_explain(CqlState* _Nonnull CS, ast_node *stmt) {
  ...
  CS->sem.current_explain_stmt = stmt;

  // EXPLAIN [explain_op] [explain_target] is only available in dev mode
  if (!CS->options.dev) {
    report_error(CS, stmt, "CQL0292: explain statement is only available in dev mode because its result set may vary between SQLite versions", NULL);
    record_error(CS, stmt);
    goto cleanup;
  }

  if (query_plan != EXPLAIN_QUERY_PLAN) {
    report_error(CS, stmt, "CQL0293: only [EXPLAIN QUERY PLAN ...] statement is supported", NULL);
    record_error(CS, stmt);
    goto cleanup;
  }

  symtab_entry *entry = symtab_find(CS->sem.sql_stmts, sql_stmt->type);
  Contract(entry);
  ((void (*)(CqlState*, ast_node*))entry->val)(CS, sql_stmt);
  ...
  sem_struct *sptr = new_sem_struct(CS, "explain_query", 4);
  ...
  stmt->sem = new_sem(CS, SEM_TYPE_STRUCT);
  stmt->sem->sptr = sptr;

cleanup:
  Contract(CS->sem.current_explain_stmt != NULL);
  CS->sem.current_explain_stmt = NULL;
}
...
static void sem_cte_decl(CqlState* _Nonnull CS, ast_node *ast, ast_node *select_core)  {
  ...
  if (find_cte(CS, name)) {
    report_error(CS, ast, "CQL0100: duplicate common table name", name);
    record_error(CS, ast);
    return;
  }

  if (name_hides_root_table(CS, name)) {
    report_error(CS, ast, "CQL0437: common table name shadows previously declared table or view", name);
    record_error(CS, ast);
    return;
  }

  if (is_ast_star(name_list)) {
    rewrite_cte_name_list_from_columns(CS, ast, select_core);
    if (is_error(ast)) {
      return;
    }
    name_list = ast->right;
  ...
  sem_struct *sptr = new_sem_struct_strip_table_flags(CS, select_core->sem->sptr);

  ast_node *item = name_list;

  for (uint32_t i = 0; i < sptr->count; i++) {
    if (!item) {
      report_error(CS, ast, "CQL0101: too few column names specified in common table expression", name);
      record_error(CS, ast);
      return;
    }
  ...
  if (item) {
    report_error(CS, ast, "CQL0102: too many column names specified in common table expression", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_accumulate_cte_info(CqlState* _Nonnull CS, ast_node *stmt, shared_cte_info *info)
{
  ...
  for (ast_node *ast = cte_tables; ast; ast = ast->right) {
    ...
    if (is_ast_like(cte_body)) {
      ...
      if (info->callback) {
        info->callback(CS, info->context, cte_name, cte_decl);
      }
    }
  }
}
...
static void sem_accumulate_stmt_list(CqlState* _Nonnull CS, ast_node *ast, shared_cte_info *info) {
  ...
  if (is_ast_with_select_stmt(stmt)) {
    sem_accumulate_cte_info(CS, stmt, info);
  }
...
static void sem_accumulate_cond_action(CqlState* _Nonnull CS, ast_node *ast, shared_cte_info *info) {
  ...
  sem_accumulate_stmt_list(CS, stmt_list, info);
}
...
static void sem_accumulate_elseif_list(CqlState* _Nonnull CS, ast_node *ast, shared_cte_info *info) {
  ...
  while (ast) {
    ...
    sem_accumulate_cond_action(CS, cond_action, info);
    ast = ast->right;
  }
}
...
static void sem_accumulate_if_stmt(CqlState* _Nonnull CS, ast_node *ast, shared_cte_info *info) {
  ...
  if (elseif) {
    sem_accumulate_elseif_list(CS, elseif, info);
  }
 ...
 */
  ...
  ast_node *ast_name = new_ast_str(CS, name);
  ast_node *proc_name_flags = new_ast_proc_name_type(CS, ast_name, new_ast_option(CS, PROC_FLAG_USES_DML));
  ast_node *declare_proc_stmt = new_ast_declare_proc_stmt(CS, proc_name_flags, new_ast_proc_params_stmts(CS, NULL, NULL));

  AST_REWRITE_INFO_RESET();

  sem_declare_proc_stmt(CS, declare_proc_stmt);
...
static void sem_accumulate_proc_cte_info(CqlState* _Nonnull CS, ast_node *create_proc_stmt, shared_cte_info *info) {
  ...
  if (is_ast_with_select_stmt(stmt)) {
    ...
    sem_accumulate_if_stmt(CS, stmt, info);
  }
  ...
  ast_node *create_proc_stmt)
{
  ...
  sem_accumulate_proc_cte_info(CS, create_proc_stmt, &info);

  if (cte_name) {
    report_error(CS, call_stmt, "CQL0430: no actual table was provided for the table parameter", cte_name);
    record_error(CS, call_stmt);
  }
}
...
static void make_distinct_table_params_list_callback(CqlState* _Nonnull CS, void *context, CSTR cte_name, ast_node *cte_decl) {
  ...
  // duplicate not found
  if (!item) {
     add_item_to_list(CS, head, cte_decl);
  }
}
...
cql_noexport void sem_check_bound_cte_name_conflict(CqlState* _Nonnull CS, ast_node *node, binding_info *info) {
  ...
  if (is_ast_cte_binding(node)) {
    ...
    if (!Strcasecmp(info->formal, actual)) {
      ...
      ast_node *proc_ast = find_proc(CS, new.proc);
      Invariant(proc_ast);

      sem_check_bound_cte_name_conflict(CS, proc_ast, &new);
  ...
  if (info->err->used == 1 && ast_has_left(node)) {
    sem_check_bound_cte_name_conflict(CS, node->left, info);
  }

  if (info->err->used == 1 && ast_has_right(node)) {
    sem_check_bound_cte_name_conflict(CS, node->right, info);
  }
}
  ...
  ast_node *cte_binding_list)
{
  ...
  for (item = cte_binding_list ; item; item = item->right) {
    ...
    bool_t added = symtab_add(CS, bindings, formal, cte_binding);
    if (!added) {
      report_error(CS, cte_binding->right, "CQL0428: duplicate binding of table in CALL/USING clause", formal);
      record_error(CS, call_stmt);
      goto cleanup;
    }
  }
  ...
  if (!parms_head) {
    report_error(CS, cte_binding_list, "CQL0429: called procedure has no table arguments but a USING clause is present", proc_name);
    record_error(CS, call_stmt);
    goto cleanup;
  }
  ...
  for (list_item *it = parms_head; it; it = it->next) {
    ...
    if (!entry) {
      report_error(CS, cte_binding_list, "CQL0430: no actual table was provided for the table parameter", cte_name);
      record_error(CS, call_stmt);
      goto cleanup;
    }
    ...
    bool_t added = symtab_add(CS, formals, cte_name, cte_decl);
    ...
    ast_node *table = sem_find_table(CS, actual, ast_actual);
    if (!table) {
    ...
    sem_add_flags(CS, ast_formal, 0); // forces the sem info to be copied but change nothing
    sem_add_flags(CS, ast_actual, 0); // forces the sem info to be copied but change nothing
    ...
    if (sptr_formals->count != sptr_actuals->count) {
      report_error(CS, ast_actual, "CQL0432: table provided must have the same number of columns as the table parameter", actual);
      record_error(CS, call_stmt);
      goto cleanup;
    }
    ...
    for (uint32_t i = 0; i < sptr_actuals->count; i++) {
      ...
      symtab_add(CS, cols, sptr_actuals->names[i], &sptr_actuals->names[i]);
    }
    ...
    for (uint32_t i = 0; i < sptr_formals->count; i++) {
      ...
      if (!sem_verify_assignment(CS, ast_actual, sem_type_formal, sem_type_actual, name)) {
        record_error(CS, call_stmt);
        goto cleanup;
      }

      sem_combine_kinds_general(CS, ast_actual, kformal, kactual);
  ...
  for (item = cte_binding_list ; item; item = item->right) {
    ...
    if (!symtab_find(formals, formal)) {
      report_error(CS, ast_formal, "CQL0431: an actual table was provided for a table parameter that does not exist", formal);
      record_error(CS, call_stmt);
      goto cleanup;
    }
  }
  ...
  for (item = cte_binding_list ; item; item = item->right) {
    ...
    ast_node *cte_decl = find_cte(CS, actual);
    ...
    sem_check_bound_cte_name_conflict(CS, create_proc_stmt, &bind_info);

    if (tmp.used > 1) {
      CSTR err_msg =
        dup_printf(CS, "CQL0444: this use of the named shared fragment is not legal because of a name conflict '%s'\n%s",
          proc_name,
          tmp.ptr);
      report_error(CS, cte_binding, err_msg, NULL);
      record_error(CS, call_stmt);
      goto cleanup;
    }
  }
...
cleanup:
  ...
  symtab_delete(CS, bindings);
  symtab_delete(CS, formals);
}
...
static void sem_shared_cte(CqlState* _Nonnull CS, ast_node *cte_body) {
  ...
  bool_t in_shared_fragment_call_saved = CS->sem.in_shared_fragment_call;
  CS->sem.in_shared_fragment_call = true;
  ...
  sem_call_stmt_opt_cursor(CS, call_stmt, NULL);
  if (is_error(call_stmt)) {
    record_error(CS, cte_body);
    goto cleanup;
  }
  ...
  ast_node *proc_stmt = find_proc(CS, proc_name);
  if (!is_proc_shared_fragment(CS, proc_stmt)) {
    report_error(CS, proc_name_ast, "CQL0224: a CALL statement inside SQL may call only a shared fragment i.e. @attribute(cql:shared_fragment)", proc_name);
    record_error(CS, cte_body);
    goto cleanup;
  }

  if (!is_ast_create_proc_stmt(proc_stmt)) {
    report_error(CS, proc_name_ast, "CQL0468: @attribute(cql:shared_fragment) may only be placed on a CREATE PROC statement", proc_name);
    record_error(CS, cte_body);
    goto cleanup;
  }
  ...
  if (cte_binding_list) {
    ...
    sem_shared_fragment_table_binding(CS, call_stmt, proc_stmt, cte_binding_list);
    if (is_error(call_stmt)) {
      record_error(CS, cte_body);
      goto cleanup;
    }
    ...
    sem_shared_fragment_ensure_no_table_binding(CS, call_stmt, proc_stmt);
    if (is_error(call_stmt)) {
      record_error(CS, cte_body);
      goto cleanup;
    }
...
cleanup:
  CS->sem.in_shared_fragment_call = in_shared_fragment_call_saved;
}
...
static void sem_cte_table(CqlState* _Nonnull CS, ast_node *ast)  {
  ...
  if (is_ast_like(cte_body)) {
    ...
    if (!CS->sem.in_shared_fragment || sem_with_depth(CS) > 1) {
      CSTR name = NULL;
      if (CS->sem.current_proc) {
        EXTRACT_STRING(proc_name, CS->sem.current_proc->left);
        name = proc_name;
      }

      report_error(CS, cte_body,
    ...
    if (is_id(like_ast->left)) {
      ...
      ast_node *found_shape = sem_find_shape_def_base(CS, like_ast, LIKEABLE_FOR_VALUES);
      if (!found_shape) {
        record_error(CS, ast);
        return;
      }
      ...
      sem_cte_decl(CS, cte_decl, found_shape);
  ...
  if (is_ast_select_stmt(cte_body)) {
    ...
    sem_cte_decl(CS, cte_decl, select_core);
    if (is_error(cte_decl)) {
      record_error(CS, ast);
      return;
    }
    ...
    sem_select_no_with(CS, select_stmt);
    if (is_error(select_stmt)) {
      record_error(CS, ast);
      return;
    }
    ...
    sem_cte_decl(CS, cte_decl, cte_body);
    if (is_error(cte_decl)) {
      record_error(CS, ast);
      return;
    }
    ...
    // now process the declaration using the types from the select
    sem_cte_decl(CS, cte_decl, select_stmt);
    if (is_error(cte_decl)) {
      record_error(CS, ast);
      return;
    }
...
static void sem_pop_cte_state(CqlState* _Nonnull CS) {
  ...
  symtab_delete(CS, CS->sem.cte_cur->ctes);
  CS->sem.cte_cur = CS->sem.cte_cur->prev;
...
static void sem_with_select(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_push_cte_state(CS);

  sem_cte_tables(CS, cte_tables);
...
static void sem_validate_previous_view(CqlState* _Nonnull CS, ast_node *prev_view) {
  ...
  ast_node *ast = find_table_or_view_even_deleted(CS, name);

  if (!ast) {
    if (is_temp || CS->options.schema_exclusive) {
  ...
  if (!ast) {
    report_error(CS, prev_view, "CQL0104: view was present but now it does not exist (use @delete instead)", name);
    record_error(CS, prev_view);
    return;
  }

  if (is_ast_create_table_stmt(ast)) {
    report_error(CS, ast, "CQL0105: object was a view but is now a table", name);
    record_error(CS, prev_view);
    record_error(CS, ast);
    return;
  }

  enqueue_pending_region_validation(CS, prev_view, ast, name);
}
...
static void sem_validate_previous_trigger(CqlState* _Nonnull CS, ast_node *prev_trigger) {
  ...
  ast_node *ast = find_trigger(CS, name);
  if (!ast) {
    if (is_temp || CS->options.schema_exclusive) {
    ...
    ast_node *ast_table = find_table_or_view_even_deleted(CS, prev_table_name);
    ...
    report_error(CS, prev_trigger, "CQL0106: trigger was present but now it does not exist (use @delete instead)", name);
    record_error(CS, prev_trigger);
    return;
  }

  enqueue_pending_region_validation(CS, prev_trigger, ast, name);
}
...
static void sem_found_dep_in_view(CqlState* _Nonnull CS, CSTR _Nonnull name, ast_node *_Nonnull target_ast, void *_Nullable context) {
  ...
  record_table_dependencies(CS, create_view_stmt, target_ast);
}
...
static void sem_create_view_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.validating_previous_schema) {
    ...
    record_ok(CS, ast);
    sem_validate_previous_view(CS, ast);
    return;
  }
  ...
  ast_node *existing_defn = adding_current_entity ? find_table_or_view_even_deleted(CS, name) : NULL;
  ...
  if (CS->sem.schema_upgrade_version > 0 && !CS->sem.current_proc) {
    ...
    symtab_add(CS, CS->sem.tables, name, ast);
  ...
  if (name_list && !sem_verify_no_duplicate_names(CS, name_list)) {
    record_error(CS, ast);
    return;
  }

  if (name_list) {
    sem_verify_no_null_columns(CS, select_stmt);
    if (is_error(select_stmt)) {
      record_error(CS, ast);
      return;
    }
    ...
    sem_verify_no_anon_no_null_columns(CS, select_stmt);
    if (is_error(select_stmt)) {
      record_error(CS, ast);
      return;
    }
  ...
  select_stmt->sem->jptr = sem_join_from_sem_struct(CS, select_stmt->sem->sptr);

  ast->sem = new_sem(CS, select_stmt->sem->sem_type);
  ...
  if (name_list) {
    ...
    for (uint32_t i = 0; i < sptr->count; i++) {
      if (!item) {
        report_error(CS, ast, "CQL0101: too few column names specified in view", name);
        record_error(CS, ast);
        return;
      }
    ...
    if (item) {
      report_error(CS, ast, "CQL0102: too many column names specified in view", name);
      record_error(CS, ast);
      return;
    }
  }
  ...
  if (existing_defn) {
    if (!sem_validate_identical_ddl(CS, existing_defn, ast)) {
      report_error(CS, name_ast, "CQL0103: duplicate table/view name", name);
      record_error(CS, name_ast);
      record_error(CS, ast);
    }
    else {
  ...
  if (adding_current_entity) {
    // deleted or no it goes in the main list
    add_item_to_list(CS, &CS->sem.all_views_list, ast);
    ...
    add_table_or_view(CS, ast);

    // and record the annotation
    sem_record_annotation_from_vers_info(CS, &vers_info);

    // record the tables used by this view (and cross link)
    sem_record_view_dependencies(CS, ast);
  }
}
...
static bool_t sem_validate_version_attrs(CqlState* _Nonnull CS, version_attrs_info *vers_info) {
  ...
  for (ast_node *ast = vers_info->attrs_ast; ast; ast = ast->right) {
    if (is_ast_recreate_attr(ast)) {
      ...
      if (ast->right) {
        AST_REWRITE_INFO_SET(ast->lineno, ast->filename);
        ast_node *version_annotation = new_ast_version_annotation(CS, new_ast_option(CS, 1), NULL);
        AST_REWRITE_INFO_RESET();
    ...
    if (is_ast_create_attr(ast)) {
      if (!sem_validate_version(CS, vers_info->create_code, ast, &vers_info->create_version, &vers_info->create_proc)) {
        record_error(CS, vers_info->target_ast);
        return false;
      }
      EXTRACT(version_annotation, ast->left);
      ...
      if (!sem_validate_version(CS, vers_info->delete_code, ast, &vers_info->delete_version, &vers_info->delete_proc)) {
        record_error(CS, vers_info->target_ast);
        return false;
      }
      EXTRACT(version_annotation, ast->left);
  ...
  if (vers_info->delete_version > 0 && vers_info->delete_version <= vers_info->create_version) {
    report_error(CS, vers_info->target_ast, "CQL0107: delete version can't be <= create version", vers_info->name);
    return false;
  }

  if (CS->sem.schema_upgrade_version == -1) {
    ...
    // if were on that version (in a migration context) then you're allowed
    // to look at that column so that you can zero it or some such.
    if (vers_info->delete_version != -1 && CS->sem.schema_upgrade_version > vers_info->delete_version) {
      vers_info->flags |= SEM_TYPE_DELETED;
    }
    ...
    // If we are migrating to a schema before the column was created then we
    // cannot see it yet.
    if (vers_info->create_version != -1 && CS->sem.schema_upgrade_version < vers_info->create_version) {
      vers_info->flags |= SEM_TYPE_DELETED;
    }
  }
...
static void sem_non_backed_table(CqlState* _Nonnull CS, ast_node *ast_error, ast_node *ast_table) {
  ...
  if (is_ast_create_table_stmt(ast_table) && is_table_backed(CS, ast_table)) {
    EXTRACT_NOTNULL(create_table_name_flags, ast_table->left);
    EXTRACT_NAME_AST(name_ast, create_table_name_flags->right);
    EXTRACT_STRING(name, name_ast);

    report_error(CS, ast_error, "CQL0493: backed storage tables may not be used in indexes/triggers/drop", name);
    record_error(CS, ast_error);
    return;
  }

  record_ok(CS, ast_error);
}
...
static void sem_non_blob_storage_table(CqlState* _Nonnull CS, ast_node *ast_error, ast_node *ast_table) {
  ...
  if (is_ast_create_table_stmt(ast_table) && is_table_blob_storage(CS, ast_table)) {
    EXTRACT_NOTNULL(create_table_name_flags, ast_table->left);
    EXTRACT_NAME_AST(name_ast, create_table_name_flags->right);
    EXTRACT_STRING(name, name_ast);

    report_error(CS, ast_error, "CQL0458: the indicated table may only be used for blob storage", name);
    record_error(CS, ast_error);
    return;
  }
...
static void sem_drop_table_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast_node *table_ast = find_usable_table_or_view_even_deleted(CS, name, name_ast, "CQL0108: table in drop statement does not exist");
  if (!table_ast) {
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_ast_create_table_stmt(table_ast)) {
    report_error(CS, name_ast, "CQL0109: cannot drop a view with drop table", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_drop_view_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast_node *view_ast = find_usable_table_or_view_even_deleted(CS, name, name_ast, "CQL0110: view in drop statement does not exist");
  if (!view_ast) {
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_ast_create_view_stmt(view_ast)) {
    report_error(CS, name_ast, "CQL0111: cannot drop a table with drop view", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_drop_index_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast_node *index_ast = find_usable_index(CS, name, name_ast, "CQL0112: index in drop statement was not declared");
  if (!index_ast) {
    record_error(CS, ast);
    return;
  }
...
static void sem_drop_trigger_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast_node *trigger_ast = find_usable_trigger(CS, name,
                                              name_ast,
...
static bool_t sem_validate_attrs_prev_cur(CqlState* _Nonnull CS, version_attrs_info *prev, version_attrs_info *cur, ast_node *name_ast) {
  ...
  bool_t valid = sem_validate_version_attrs(CS, cur);
  Invariant(valid);  // already verified

  valid = sem_validate_version_attrs(CS, prev);
  Invariant(valid);  // already verified
  ...
  if (prev->create_version > 0 || prev->delete_version > 0) {
    if (cur->recreate && !prev->recreate) {
      report_error(CS, name_ast, "CQL0114: current schema can't go back to @recreate semantics for", name);
      return false;
    }
  }
  ...
  if (prev->recreate) {
    ...
    if (!cur->recreate && cur->delete_version < 0 && cur->create_code == SCHEMA_ANNOTATION_CREATE_TABLE) {
       if (!cur->create_proc || Strcasecmp(CQL_FROM_RECREATE, cur->create_proc)) {
         report_error(CS, name_ast, "CQL0377: table transitioning from @recreate to @create must use @create(nn,cql:from_recreate)", name);
         return false;
       }
    }
  }
    ...
    if (!sem_validate_create_prev_cur(prev->create_version, cur->create_version)) {
      report_error(CS, name_ast, "CQL0115: current create version not equal to previous create version for", name);
      return false;
    }
    ...
    if (!sem_match_optional_string(prev->create_proc, cur->create_proc)) {
      report_error(CS, name_ast, "CQL0118: @create procedure changed in object", name);
      return false;
    }
  }

  if (!sem_validate_delete_prev_cur(prev->delete_version, cur->delete_version)) {
    report_error(CS, name_ast, "CQL0116: current delete version not equal to previous delete version for", name);
    return false;
  }
  ...
  if (prev->delete_version > 0) {
    if (!sem_match_optional_string(prev->delete_proc, cur->delete_proc)) {
      report_error(CS, name_ast, "CQL0117: @delete procedure changed in object", name);
      return false;
    }
  }
...
static void sem_validate_col_def_prev_cur(CqlState* _Nonnull CS, ast_node *def, ast_node *prev_def, version_attrs_info *cur_info, version_attrs_info *prev_info) {
  ...
  if (strcmp(name, prev_name)) {
    report_error(CS, name_ast, "CQL0119: column name is different between previous and current schema", name);
    record_error(CS, prev_def);
    return;
  }
  ...
  if (cur_type != prev_type) {
    report_error(CS, name_ast, "CQL0120: column type is different between previous and current schema", name);
    record_error(CS, prev_def);
    return;
  }
  ...
  sem_col_def(CS, def, &cur_cd_info);

  col_def_info prev_cd_info;
  init_col_def_info(&prev_cd_info, prev_info);
  sem_col_def(CS, prev_def, &prev_cd_info);

  if (!sem_validate_create_prev_cur(prev_cd_info.create_version, cur_cd_info.create_version)) {
    report_error(CS, name_ast, "CQL0121: column current create version not equal to previous create version", name);
    record_error(CS, prev_def);
    return;
  }

  if (!sem_validate_delete_prev_cur(prev_cd_info.delete_version, cur_cd_info.delete_version)) {
    report_error(CS, name_ast, "CQL0122: column current delete version not equal to previous delete version", name);
    record_error(CS, prev_def);
    return;
  }
  ...
  if (prev_cd_info.delete_version != -1) {
    if (!sem_match_optional_string(prev_cd_info.delete_proc, cur_cd_info.delete_proc)) {
      report_error(CS, name_ast, "CQL0123: column @delete procedure changed", name);
      record_error(CS, prev_def);
      return;
    }
  }
  ...
  if (!sem_match_optional_string(prev_cd_info.create_proc, cur_cd_info.create_proc)) {
    report_error(CS, name_ast, "CQL0124: column @create procedure changed", name);
    record_error(CS, prev_def);
    return;
  }
  ...
  if (has_default(def->sem->sem_type)) {
    ...
    bool_t identical = sem_validate_identical_text(CS, prev_default, cur_default, gen_root_expr, NULL);
    if (!identical) {
      report_error(CS, name_ast, "CQL0125: column current default value not equal to previous default value", name);
      record_error(CS, prev_def);
      return;
    }
  }
...
static bool_t sem_validate_identical_ddl(CqlState* _Nonnull CS, ast_node *cur, ast_node *prev) {
  ...
  return sem_validate_identical_text(CS, cur, prev, gen_one_stmt, &callbacks);
}
...
static bool_t sem_named_type_gen_sql_callback(CqlState* _Nonnull CS, ast_node *ast, void *context, charbuf *buf) {
  ...
  if (named_type) {
    AST_REWRITE_INFO_SET(ast->lineno, ast->filename);
    gen_data_type(CS, rewrite_gen_data_type(CS, named_type->sem->sem_type, named_type->sem->kind));
    AST_REWRITE_INFO_RESET();
    return true;
  }
...
static bool_t sem_validate_identical_text(CqlState* _Nonnull CS, ast_node *prev, ast_node *cur, gen_func fn, gen_sql_callbacks *callbacks) {
  ...
  gen_set_output_buffer(CS, &prev_sql);
  gen_with_callbacks(CS, prev, fn, &callbacks_with_named_type_callback);

  gen_set_output_buffer(CS, &cur_sql);
  gen_with_callbacks(CS, cur, fn, &callbacks_with_named_type_callback);

  bool_t identical = !strcmp(prev_sql.ptr, cur_sql.ptr);

  if (!identical) {
    cql_error(CS, "Incompatible declarations found\n");
    report_error(CS, prev, prev_sql.ptr, NULL);
    report_error(CS, cur, cur_sql.ptr, NULL);
    cql_error(CS, "The above must be identical.\n");
  }
...
static bool_t sem_validate_identical_funcs(CqlState* _Nonnull CS, ast_node *prev_func, ast_node *cur_func) {
  ...
  gen_set_output_buffer(CS, &prev_sql);
  gen_statement_with_callbacks(CS, prev_func, NULL);

  gen_set_output_buffer(CS, &cur_sql);
  gen_statement_with_callbacks(CS, cur_func, NULL);
...
static void sem_validate_previous_table(CqlState* _Nonnull CS, ast_node *prev_table) {
  ...
  if (!ast) {
    report_error(CS, prev_table, "CQL0126: table was present but now it does not exist (use @delete instead)", name);
    record_error(CS, prev_table);
    return;
  }

  if (!is_ast_create_table_stmt(ast)) {
    report_error(CS, ast, "CQL0127: object was a table but is now a view", name);
    record_error(CS, prev_table);
    record_error(CS, ast);
    return;
  }
  ...
  sem_add_flags(CS, ast, SEM_TYPE_VALIDATED);
  ...
  if (!sem_validate_attrs_prev_cur(CS, &prev_info, &cur_info, name_ast)) {
    record_error(CS, prev_table);
    record_error(CS, ast);
    return;
  }
  ...
  if (prev_info.recreate && !cur_info.recreate) {
    ...
    if (cur_info.delete_version > 0 && !cur_info.is_virtual_table) {
      report_error(CS, ast, "CQL0448: table was marked @delete but it needs to be marked @recreate @delete", name);
      record_error(CS, prev_table);
      record_error(CS, ast);
      return;
    }
    ...
    if (ast->sem->create_version > 0) {
      add_item_to_list(CS, &CS->sem.all_prev_recreate_tables, ast);
    }
  }
  ...
  for (;;) {
    ...
    sem_validate_col_def_prev_cur(CS, def, prev_def, &cur_info, &prev_info);
    ...
    if (!sem_validate_identical_coldef(CS, def, prev_def)) {
      report_error(CS, def, "CQL0128: table has a column that is different in the previous and current schema", def->sem->name);
      record_error(CS, prev_table);
      record_error(CS, ast);
      return;
    }
  ...
  if (prev_item) {
    report_error(CS, prev_item, "CQL0129: a column was removed from the table rather than marked with @delete", prev_item->left->sem->name);
    record_error(CS, prev_table);
    record_error(CS, ast);
    return;
  }
  ...
  for ( ;item; item = item->right) {
    ...
    if (cdef->sem->create_version < 0) {
      report_error(CS, cdef, "CQL0130: table has columns added without marking them @create", name);
      record_error(CS, prev_table);
      record_error(CS, ast);
      return;
    }
    ...
    if (cdef->sem->delete_version > 0) {
      report_error(CS, cdef, "CQL0131: table has newly added columns that are marked both @create and @delete", name);
      record_error(CS, prev_table);
      record_error(CS, ast);
      return;
    }
    ...
    add_item_to_list(CS, &CS->sem.created_columns, cdef);
  }
  ...
  for (;;) {
    ...
    // any other differences are found by comparing the canonical def text
    if (!sem_validate_identical_coldef(CS, def, prev_def)) {
      report_error(CS, def, "CQL0132: table has a facet that is different in the previous and current schema", name);
      record_error(CS, prev_table);
      record_error(CS, ast);
      return;
    }
  ...
  if (prev_item) {
    report_error(CS, prev_item, "CQL0133: non-column facets have been removed from the table", name);
    record_error(CS, prev_table);
    return;
  }
  ...
  if (item) {
    report_error(CS, item, "CQL0134: table has a new non-column facet in the current schema", name);
    record_error(CS, prev_table);
    record_error(CS, ast);
    return;
  }
  ...
  if (flags != prev_flags) {
    report_error(CS, ast, "CQL0135: table create statement attributes different than previous version", name);
    record_error(CS, prev_table);
    record_error(CS, ast);
    return;
  }

  enqueue_pending_region_validation(CS, prev_table, ast, name);
}
...
static void sem_record_annotation_from_vers_info(CqlState* _Nonnull CS, version_attrs_info *vers_info) {
  ...
  if (vers_info->recreate) {
    ...
    record_recreate_annotation(CS, target_ast, vers_info->name, group_name, recreate_ast);
  ...
  if (vers_info->create_version > 0) {
    ...
    record_schema_annotation(CS, vers_info->create_version, target_ast, vers_info->name, code, NULL, version_annotation, 0);
  }

  if (vers_info->delete_version > 0) {
    EXTRACT(version_annotation, vers_info->delete_version_ast);
    uint32_t code = vers_info->delete_code;
    record_schema_annotation(CS, vers_info->delete_version, target_ast, vers_info->name, code, NULL, version_annotation, 0);
  }
...
static void sem_found_dep_in_trigger(CqlState* _Nonnull CS, CSTR _Nonnull target_name, ast_node *_Nonnull target_ast, void *_Nullable context) {
  ...
  if (Strcasecmp(info->trigger_on_table_name, target_name)) {
    ...
    record_table_dependencies(CS, info->trigger_ast, target_ast);
  }
}
...
static void sem_create_trigger_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.validating_previous_schema) {
    record_ok(CS, ast);
    sem_validate_previous_trigger(CS, ast);
    return;
  }
  ...
  ast_node *existing_defn = adding_current_entity ? find_trigger(CS, trigger_name) : NULL;
  ...
  if (!sem_validate_no_delete_migration(CS, &vers_info, ast, trigger_name)) {
    return;
  }
  ...
  if (deleting) {
    target = find_usable_table_or_view_even_deleted(CS,
      table_name,
      table_name_ast,
      "CQL0137: table/view not found");

    if (is_deleted(target)) {
      report_error(CS, ast, "CQL0397: object is an orphan because its table is deleted. Remove rather than @delete", trigger_name);
      record_error(CS, ast);
      return;
    }
  }
  else {
    target = find_usable_and_not_deleted_table_or_view(CS,
      table_name,
      table_name_ast,
      "CQL0137: table/view not found");
  }
  ...
  sem_non_backed_table(CS, ast, target);
  if (is_error(ast)) {
    return;
  }

  sem_non_blob_storage_table(CS, ast, target);
  if (is_error(ast)) {
    return;
  }

  if (is_virtual_ast(target)) {
    report_error(CS, table_name_ast, "CQL0162: cannot add a trigger to a virtual table", table_name);
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_ast_create_table_stmt(target) && !(flags & TRIGGER_INSTEAD_OF)) {
    report_error(CS, table_name_ast, "CQL0138: a trigger on a view must be the INSTEAD OF form", table_name);
    record_error(CS, ast);
    return;
  }
  ...
  if (flags & TRIGGER_INSERT) {
    ...
    if (name_list && !sem_validate_name_list(CS, name_list, jptr)) {
      record_error(CS, ast);
      return;
    }
  ...
  if (when_expr) {
    Invariant(!CS->sem.in_trigger_when_expr);
    CS->sem.in_trigger_when_expr = 1;

    PUSH_JOIN(when_scope, jptr);
    sem_numeric_expr(CS, when_expr, NULL, "WHEN", SEM_EXPR_CONTEXT_WHERE);
    POP_JOIN();

    Invariant(CS->sem.in_trigger_when_expr);
    CS->sem.in_trigger_when_expr = 0;
  ...
  Invariant(!CS->sem.in_trigger);
  CS->sem.in_trigger = 1;

  sem_stmt_list(CS, stmt_list);
  ...
  if (existing_defn) {
    if (!sem_validate_identical_ddl(CS, existing_defn, ast)) {
      report_error(CS, trigger_name_ast, "CQL0136: trigger already exists", trigger_name);
      record_error(CS, trigger_name_ast);
      record_error(CS, ast);
    }
    else {
      // this will let us find the duplicate later, it marks this as not authoritative
  ...
  if (adding_current_entity) {
    add_trigger(CS, ast, trigger_name);
    add_item_to_list(CS, &CS->sem.all_triggers_list, ast);

    // and record the annotation
    sem_record_annotation_from_vers_info(CS, &vers_info);

    // record the tables used by this trigger (and cross link)
    sem_record_trigger_dependencies(CS, ast);
  }
}
...
static bool_t sem_validate_virtual_table_vers(CqlState* _Nonnull CS, version_attrs_info *table_vers_info) {
  ...
  if (table_vers_info->is_virtual_table && table_vers_info->delete_version_ast) {
     if (!table_vers_info->delete_proc || Strcasecmp(CQL_MODULE_WARN, table_vers_info->delete_proc )) {
        report_error(CS, table_vers_info->delete_version_ast, "CQL0392: when deleting a virtual table you must specify @delete(nn, "
            CQL_MODULE_WARN ") as a reminder not to delete the module for this virtual table", table_vers_info->name);
        record_error(CS, create_table_stmt);
        return false;
     }
  }
  return true;
...
static bool_t sem_validate_vers_ok_in_context(CqlState* _Nonnull CS, version_attrs_info *vers) {
  ...
  if (CS->sem.current_proc && is_versioned) {
     report_error(CS, vers->target_ast, "CQL0396: versioning attributes may not be used on DDL inside a procedure", vers->name);
     return false;
  }

  if (vers->is_temp && is_versioned) {
    report_error(CS, vers->target_ast, "CQL0139: temp objects may not have versioning annotations", vers->name);
    return false;
  }
...
static void report_invalid_blob_storage_column(CqlState* _Nonnull CS, ast_node *ast, CSTR reason, CSTR column, CSTR table) {
  ...
  CSTR err_msg = dup_printf(CS, "CQL0459: table is not suitable for use as blob storage: column '%s' %s in", column, reason);
  report_error(CS, ast, err_msg, table);
  record_error(CS, ast);
}
...
static void sem_blob_storage_col_def(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *def, CSTR table_name) {
  ...
  for (ast_node *ast = attrs; ast; ast = ast->right) {
    if (is_ast_create_attr(ast) || is_ast_col_attrs_not_null(ast) || is_ast_sensitive_attr(ast)) {
      ...
      report_invalid_blob_storage_column(CS, table_ast, "has been deleted", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_default(ast)) {
      // In principle we could support this, but we don't for now.
      report_invalid_blob_storage_column(CS, table_ast, "has a default value", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_check(ast)) {
      report_invalid_blob_storage_column(CS, table_ast, "has a check expression", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_collate(ast)) {
      report_invalid_blob_storage_column(CS, table_ast, "specifies collation order", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_pk(ast)) {
      // note that autoinc must be on a pk and is in its node so this also detects autoinc
      report_invalid_blob_storage_column(CS, table_ast, "has a primary key", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_fk(ast)) {
      report_invalid_blob_storage_column(CS, table_ast, "has a foreign key", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_hidden(ast)) {
      report_invalid_blob_storage_column(CS, table_ast, "is a hidden column", col_name, table_name);
      return;
    }
    else {
      // this is all that's left
      Contract(is_ast_col_attrs_unique(ast));
      report_invalid_blob_storage_column(CS, table_ast, "has a unique key", col_name, table_name);
      return;
    }
  }
}
...
static void report_invalid_blob_storage(CqlState* _Nonnull CS, ast_node *ast, CSTR reason, CSTR table) {
  ...
  CSTR err_msg = dup_printf(CS, "CQL0459: table is not suitable for use as blob storage: %s", reason);
  report_error(CS, ast, err_msg, table);
  record_error(CS, ast);
}
...
static void sem_validate_table_for_blob_storage(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (temp) {
    report_invalid_blob_storage(CS, ast, "it is redundantly marked TEMP", name);
    return;
  }

  if (no_rowid) {
    report_invalid_blob_storage(CS, ast, "it is redundantly marked WITHOUT ROWID", name);
    return;
  }
  ...
  for (ast_node *item = col_key_list; item; item = item->right) {
    ...
    if (!is_ast_col_def(def)) {
      report_invalid_blob_storage(CS, ast, "it has at least one constraint", name);
      return;
    }

    sem_blob_storage_col_def(CS, ast, def, name);
    if (is_error(ast)) {
      return;
    }
  }
  ...
  while (table_attrs) {
    if (is_ast_recreate_attr(table_attrs)) {
      report_invalid_blob_storage(CS, ast, "it is declared using @recreate", name);
      return;
    }
...
static void report_invalid_backing_column(CqlState* _Nonnull CS, ast_node *ast, CSTR reason, CSTR column, CSTR table) {
  ...
  CSTR err_msg = dup_printf(CS, "CQL0483: table is not suitable for use as backing storage: column '%s' %s in", column, reason);
  report_error(CS, ast, err_msg, table);
  record_error(CS, ast);
}
...
static void sem_backing_col_def(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *def, CSTR table_name) {
  ...
  for (ast_node *ast = attrs; ast; ast = ast->right) {
    if (is_ast_col_attrs_not_null(ast) || is_ast_sensitive_attr(ast)) {
      ...
      if (col_attrs_autoinc) {
        report_invalid_backing_column(CS, table_ast, "specifies auto increment", col_name, table_name);
        return;
      }
    }
    else if (is_ast_create_attr(ast)) {
      // backing tables do not support schema changes at this time
      report_invalid_backing_column(CS, table_ast, "has create attribute", col_name, table_name);
      return;
    }
    else if (is_ast_delete_attr(ast)) {
      // backing tables do not support schema changes at this time
      report_invalid_backing_column(CS, table_ast, "has delete attribute", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_default(ast)) {
      // In principle we could support this, but we don't for now.
      report_invalid_backing_column(CS, table_ast, "has a default value", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_check(ast)) {
      // In principle we could support this, but we don't for now.
      report_invalid_backing_column(CS, table_ast, "has a check expression", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_collate(ast)) {
      // In principle we could support this, but we don't for now.  Assuming it makes sense for blobs?
      report_invalid_backing_column(CS, table_ast, "specifies collation order", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_fk(ast)) {
      // In principle we could support this, but we don't for now.  Planned for "Stage 3"
      report_invalid_backing_column(CS, table_ast, "has a foreign key", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_hidden(ast)) {
      // Doesn't make a lot of sense for a backing store, invalid.
      report_invalid_backing_column(CS, table_ast, "is a hidden column", col_name, table_name);
      return;
    }
    else {
      // In principle we could support this, but we don't for now.  Planned for "Stage 3"
      ...
      report_invalid_backing_column(CS, table_ast, "has a unique key", col_name, table_name);
      return;
    }
  }
  ...
  if (!is_ast_type_blob(col_type)) {
   report_invalid_backing_column(CS, table_ast, "has a column that is not a blob", col_name, table_name);
   return;
  }
}
...
static void report_invalid_backing(CqlState* _Nonnull CS, ast_node *ast, CSTR reason, CSTR table) {
  ...
  CSTR err_msg = dup_printf(CS, "CQL0483: table is not suitable for use as backing storage: %s", reason);
  report_error(CS, ast, err_msg, table);
  record_error(CS, ast);
}
...
static void sem_validate_table_for_backing(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (temp) {
    report_invalid_backing(CS, ast, "it is redundantly marked TEMP", name);
    return;
  }

  if (no_rowid) {
    report_invalid_backing(CS, ast, "it is redundantly marked WITHOUT ROWID", name);
    return;
  }
  ...
  for (ast_node *item = col_key_list; item; item = item->right) {
    ...
    if (is_ast_pk_def(def)) {
      ...
      for (ast_node *pk_item = indexed_columns; pk_item; pk_item = pk_item->right) {
        if (!is_ast_str(pk_item->left->left)) {
          report_invalid_backing(CS, ast, "it has an expression in its primary key", string_from_name_list_item(CS, pk_item));
          return;
        }
      }
    ...
    if (!is_ast_col_def(def)) {
      report_invalid_backing(CS, ast, "it has at least one invalid constraint", name);
      return;
    }
    ...
    sem_backing_col_def(CS, ast, def, name);
    if (is_error(ast)) {
      return;
    }
  }
  ...
  if (col_count != 2) {
    report_invalid_backing(CS, ast, "it does not have exactly two blob columns", name);
    return;
  }

  if (!has_key) {
    report_invalid_backing(CS, ast, "it does not have a primary key", name);
    return;
  }

  if (!has_value) {
    report_invalid_backing(CS, ast, "it has only primary key columns", name);
    return;
  }
}
...
static void report_invalid_backed_column(CqlState* _Nonnull CS, ast_node *ast, CSTR reason, CSTR column, CSTR table) {
  ...
  CSTR err_msg = dup_printf(CS, "CQL0487: table is not suitable for use as backed storage: column '%s' %s in", column, reason);
  report_error(CS, ast, err_msg, table);
  record_error(CS, ast);
}
...
static void sem_backed_col_def(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *def, CSTR table_name) {
  ...
  for (ast_node *ast = attrs; ast; ast = ast->right) {
    if (is_ast_col_attrs_not_null(ast) || is_ast_sensitive_attr(ast)) {
      ...
      if (col_attrs_autoinc) {
        report_invalid_backed_column(CS, table_ast, "specifies auto increment", col_name, table_name);
        return;
      }
    }
    else if (is_ast_create_attr(ast)) {
      // backed tables do not use schema annotations, they can change at whim
      report_invalid_backed_column(CS, table_ast, "has create attribute", col_name, table_name);
      return;
    }
    else if (is_ast_delete_attr(ast)) {
      // backed tables do not use schema annotations, they can change at whim
      report_invalid_backed_column(CS, table_ast, "has delete attribute", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_default(ast)) {
      ...
      report_invalid_backed_column(CS, table_ast, "has a check expression", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_collate(ast)) {
      report_invalid_backed_column(CS, table_ast, "specifies collation order", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_fk(ast)) {
      // these could be supported when the backing table is allowed to have an explicit FK column or columns to match
      report_invalid_backed_column(CS, table_ast, "has a foreign key", col_name, table_name);
      return;
    }
    else if (is_ast_col_attrs_hidden(ast)) {
      report_invalid_backed_column(CS, table_ast, "is a hidden column", col_name, table_name);
      return;
    }
    else {
      // this is all that's left
      Contract(is_ast_col_attrs_unique(ast));
      report_invalid_backed_column(CS, table_ast, "has a unique key", col_name, table_name);
      return;
    }
  }
}
...
static void report_invalid_backed(CqlState* _Nonnull CS, ast_node *ast, CSTR reason, CSTR table) {
  ...
  CSTR err_msg = dup_printf(CS, "CQL0487: table is not suitable for use as backed storage: %s", reason);
  report_error(CS, ast, err_msg, table);
  record_error(CS, ast);
}
...
static void sem_validate_table_for_backed(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!backing_table) {
    report_invalid_backed(CS, ast, "backing table does not exist", backing_table_name);
    return;
  }

  if (!is_backing(backing_table->sem->sem_type)) {
    report_invalid_backed(CS, ast, "table exists but is not a valid backing table", backing_table_name);
    return;
  }
  ...
  if (temp) {
    report_invalid_backed(CS, ast, "it is redundantly marked TEMP", name);
    return;
  }

  if (no_rowid) {
    report_invalid_backed(CS, ast, "it is redundantly marked WITHOUT ROWID", name);
    return;
  }
  ...
  for (ast_node *item = col_key_list; item; item = item->right, icol++) {
    ...
    if (is_ast_pk_def(def)) {
      ...
      for (ast_node *pk_item = indexed_columns; pk_item; pk_item = pk_item->right) {
        if (!is_ast_str(pk_item->left->left)) {
          report_invalid_backed(CS, ast, "it has an expression in its primary key", string_from_name_list_item(CS, pk_item));
          return;
        }
      }
    ...
    if (!is_ast_col_def(def)) {
      report_invalid_backed(CS, ast, "it has at least one invalid constraint", name);
      return;
    }
    ...
    sem_backed_col_def(CS, ast, def, name);
    if (is_error(ast)) {
      return;
    }
  }
  ...
  if (!has_key) {
    report_invalid_backed(CS, ast, "it does not have a primary key", name);
    return;
  }

  if (!has_value) {
    report_invalid_backed(CS, ast, "it has only primary key columns", name);
    return;
  }
  ...
  if (backed_sem->create_version > 0 || backed_sem->delete_version > 0) {
    report_invalid_backed(CS, ast, "it is declared using schema directives (@create or @delete", name);
    return;
  }
  ...
  if (backing_sem->recreate != backed_sem->recreate) {
    ...
    report_invalid_backed(CS, ast, "@recreate attribute doesn't match the backing table", name);
    return;
  }
  ...
  if (Strcasecmp(backing_group, backed_group)) {
    ...
    report_invalid_backed(CS, ast, "@recreate group doesn't match the backing table", name);
    return;
  }
  ...
  if (icol_pk >= 0) {
    ...
    key_count = 0;
    for (ast_node *item = indexed_columns; item; item = item->right) {
      CSTR c_name = string_from_name_list_item(CS, item);
...
static void sem_create_table_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // These never nest
  Invariant(!CS->sem.current_table_name);
  Invariant(!CS->sem.current_table_ast);
  ...
  CS->sem.current_table_name = name;
  CS->sem.current_table_ast = ast;
  ...
  if (!sem_validate_version_attrs(CS, &table_vers_info)) {
    record_error(CS, ast);
    goto cleanup;
  }

  if (!sem_validate_virtual_table_vers(CS, &table_vers_info)) {
    record_error(CS, ast);
    goto cleanup;
  }

  if (!sem_validate_vers_ok_in_context(CS, &table_vers_info)) {
    record_error(CS, ast);
    goto cleanup;
  }
  ...
  bool_t rewrite_col = rewrite_col_key_list(CS, col_key_list);
  ...
  for (ast_node *item = col_key_list; item; item = item->right) {
    ...
    if (is_ast_col_def(def)) {
      if (found_constraint) {
        report_error(CS, def, "CQL0349: column definitions may not come after constraints", col_info.col_name);
        record_error(CS, ast);
        goto cleanup;;
      }

      sem_col_def(CS, def, &col_info);
      if (is_error(def)) {
        record_error(CS, ast);
        goto cleanup;
      }
      ...
      if (col_info.default_value) {
        symtab_add(CS, def_values, col_info.col_name, col_info.default_value);
      }

      if (temp && (col_info.create_version > 0 || col_info.delete_version > 0)) {
        report_error(CS, def, "CQL0140: columns in a temp table may not have versioning attributes", col_info.col_name);
        record_error(CS, ast);
        goto cleanup;;
      }
  ...
  if (col_info.autoinc_columns && no_rowid) {
    report_error(CS, ast, "CQL0141: table has an AUTOINCREMENT column; it cannot also be WITHOUT ROWID", name);
    record_error(CS, ast);
    goto cleanup;
  }

  if (CS->sem.enforcement.strict_without_rowid && no_rowid) {
    report_error(CS, ast, "CQL0339: WITHOUT ROWID tables are forbidden if strict without rowid mode is enabled", name);
    record_error(CS, ast);
    goto cleanup;
  }
  ...
  sem_struct *sptr = new_sem_struct(CS, name, cols);
  ...
  for (ast_node *item = col_key_list; item; item = item->right) {
    ...
    if (is_ast_col_def(def)) {
      ...
      if (!symtab_add(CS, columns, def->sem->name, NULL)) {
        EXTRACT_NOTNULL(col_def_type_attrs, def->left);
        EXTRACT_NOTNULL(col_def_name_type, col_def_type_attrs->left);
        EXTRACT_NAME_AST(col_name_ast, col_def_name_type->left);

        report_error(CS, col_name_ast, "CQL0142: duplicate column name", def->sem->name);
        record_error(CS, ast);
        symtab_delete(CS, columns);
        goto cleanup;;
      }
  ...
  ast->sem = new_sem(CS, SEM_TYPE_STRUCT);
  ast->sem->sptr = sptr;
  ast->sem->jptr = sem_join_from_sem_struct(CS, sptr);
  ast->sem->region = CS->sem.current_region;
  ast->sem->table_info = _ast_pool_new(table_node);
  memset(ast->sem->table_info, 0, sizeof(table_node));

  sem_constraints(CS, ast, col_key_list, &col_info);
  ...
  if (col_info.primary_keys > 1) {
    report_error(CS, name_ast, "CQL0143: more than one primary key in table", name);
    record_error(CS, ast);
    goto cleanup;;
  }
  ...
  for (ast_node *item = col_key_list; item; item = item->right) {
    ...
    if (is_ast_col_def(def)) {
      ...
      if (not_nullable_flag(sptr->semtypes[col]) && !not_nullable_flag(def->sem->sem_type)) {
        sem_add_flags(CS, def, SEM_TYPE_NOTNULL);
      }

      if (is_partial_pk(sptr->semtypes[col]) && !is_partial_pk(def->sem->sem_type)) {
        sem_add_flags(CS, def, SEM_TYPE_PARTIAL_PK);
      }
      col++;
    }
  }
  ...
  run_pending_table_validations(CS);
  ...
  if (!is_error(ast)) {
    if (existing_defn) {
      ...
      if (!sem_validate_identical_ddl(CS, existing_cmp, current_cmp)) {
        report_error(CS, name_ast, "CQL0103: duplicate table/view name", name);
        record_error(CS, name_ast);
        record_error(CS, ast);
      }
    ...
    if (is_table_blob_storage(CS, ast)) {
      sem_validate_table_for_blob_storage(CS, ast);
      if (is_error(ast)) {
        goto cleanup;
      }
    }

    if (is_table_backing(CS, ast)) {
      sem_validate_table_for_backing(CS, ast);
      if (is_error(ast)) {
        goto cleanup;
      }
    ...
    if (is_table_backed(CS, ast)) {
      sem_validate_table_for_backed(CS, ast);
      if (is_error(ast)) {
        goto cleanup;
      }
    ...
    if (CS->sem.validating_previous_schema) {
      sem_validate_previous_table(CS, ast);
    }
    else if (adding_current_entity) {
      // deleted or no it goes in the main list
      add_item_to_list(CS, &CS->sem.all_tables_list, ast);

      // The name is consumed, some clients will use find_usable_and_not_deleted_table_or_view
      // to not see deleted views (e.g. select) others don't (e.g. drop)
      add_table_or_view(CS, ast);

      add_default_values(CS, def_values, name);
      def_values = NULL;

      sem_record_annotation_from_vers_info(CS, &table_vers_info);

      if (is_backed(ast->sem->sem_type)) {
        rewrite_shared_fragment_from_backed_table(CS, ast);
      }
    }
  }

cleanup:
  CS->sem.current_table_name = NULL;
  CS->sem.current_table_ast = NULL;
  if (def_values) {
    // if it's been stored in tables_default_values the local will be null
    symtab_delete(CS, def_values);
  }
}
...
void sem_create_virtual_table_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_table_blob_storage(CS, ast)) {
    report_invalid_blob_storage(CS, ast, "it is a virtual table", name);
    return;
  }

  if (is_table_backing(CS, ast)) {
    report_invalid_backing(CS, ast, "it is a virtual table", name);
    return;
  }

  if (is_table_backed(CS, ast)) {
    report_invalid_backed(CS, ast, "it is a virtual table", name);
    return;
  }
  ...
  if (is_eponymous && Strcasecmp(name, module_name)) {
    CSTR err_msg = dup_printf(CS,
         ...
         name, module_name);
    report_error(CS, ast, err_msg, NULL);
    record_error(CS, ast);
    return;
  }

  sem_create_table_stmt(CS, create_table_stmt);
...
static void sem_alter_table_add_column_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast_node *table_ast = find_usable_table_or_view_even_deleted(CS, name, name_ast, "CQL0144: table in alter statement does not exist");
  if (!table_ast) {
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_ast_create_table_stmt(table_ast)) {
    report_error(CS, name_ast, "CQL0144: cannot alter a view", name);
    record_error(CS, ast);
    return;
  }

  if (is_virtual_ast(table_ast)) {
    report_error(CS, name_ast, "CQL0164: cannot use ALTER TABLE on a virtual table", name);
    record_error(CS, ast);
    return;
  }
  ...
  sem_col_def(CS, col_def, &col_info);
  if (is_error(col_def)) {
    record_error(CS, ast);
    return;
  }
  ...
  if (col_info.create_version > 0 || col_info.delete_version > 0) {
    report_error(CS, col_def, "CQL0145: version annotations not valid in alter statement", col_def->sem->name);
    record_error(CS, ast);
    return;
  }

  if (col_info.autoinc_columns) {
    report_error(CS, col_def, "CQL0146: adding an auto increment column is not allowed", col_def->sem->name);
    record_error(CS, ast);
    return;
  }

  if (is_not_nullable(col_def->sem->sem_type) && !has_default(col_def->sem->sem_type)) {
    report_error(CS, col_def, "CQL0147: adding a not nullable column with no default value is not allowed", col_def->sem->name);
    record_error(CS, ast);
    return;
  }
  ...
  if (!def_found) {
    report_error(CS, col_def, "CQL0148: added column must already be reflected in declared schema, with @create, exact name match required", col_def->sem->name);
    record_error(CS, ast);
    return;
  }
  ...
  if (sem_type_added != sem_type_required) {
    report_error(CS, col_def, "CQL0149: added column must be an exact match for the column type declared in the table", col_def->sem->name);
    record_error(CS, ast);
    return;
  }
...
static void sem_set_notnull_improved(CqlState* _Nonnull CS, CSTR name, CSTR scope) {
  ...
  sem_t *type = find_mutable_type_and_global_status(CS, name, scope, &is_global);
  Contract(type);
  ...
  if (is_global) {
    ...
    global_item->next = global_notnull_improvements_rv;
    global_notnull_improvements_lv = global_item;
  }

  flow_set_flag_for_type(CS, SEM_TYPE_INFERRED_NOTNULL, type);
}
...
static void sem_unset_notnull_improved(CqlState* _Nonnull CS, CSTR name, CSTR scope) {
  ...
  if (*type & SEM_TYPE_INFERRED_NOTNULL) {
    flow_unset_flag_for_type(CS, SEM_TYPE_INFERRED_NOTNULL, type);
  }
}
...
static void sem_set_improvements_for_true_condition(CqlState* _Nonnull CS, ast_node* ast)
{
  ...
  if (is_ast_and(ast)) {
    ...
    Invariant(ast->left);
    Invariant(ast->right);
    sem_set_improvements_for_true_condition(CS, ast->left);
    sem_set_improvements_for_true_condition(CS, ast->right);
    return;
  }
  ...
  if (is_ast_is_not(ast) && is_ast_null(ast->right) && is_id_or_dot(ast->left)) {
    ...
    sem_set_notnull_improved(CS, name, scope);
    return;
  }
}
...
static void sem_set_improvements_for_false_condition(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_or(ast)) {
    ...
    sem_set_improvements_for_false_condition(CS, ast->left);
    sem_set_improvements_for_false_condition(CS, ast->right);
    return;
  }
...
static void sem_cond_action(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!is_numeric_expr(expr)) {
    report_error(CS, expr, "CQL0150: expected numeric expression in IF predicate", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  if (stmt_list) {
    ...
    sem_set_improvements_for_true_condition(CS, expr);
    sem_stmt_list_in_current_flow_context(CS, stmt_list);
    FLOW_POP_CONTEXT_BRANCH();
    if (is_error(stmt_list)) {
      record_error(CS, ast);
      return;
    ...
    flow_context_branch_group_add_empty_branch(CS);
  }
  ...
  // `expr`.
  sem_set_improvements_for_false_condition(CS, expr);
...
static void sem_set_initialization_improved(CqlState* _Nonnull CS, CSTR name, CSTR scope) {
  ...
  flow_set_flag_for_type(CS, SEM_TYPE_INIT_COMPLETE, type);
}
...
static void sem_set_has_row_improved(CqlState* _Nonnull CS, CSTR cursor_name) {
  ...
  flow_set_flag_for_type(CS, SEM_TYPE_HAS_ROW, type);
}
...
static void sem_unset_has_row_improved(CqlState* _Nonnull CS, CSTR cursor_name) {
  ...
  flow_unset_flag_for_type(CS, SEM_TYPE_HAS_ROW, type);
}
...
static void sem_elseif_list(CqlState* _Nonnull CS, ast_node *head) {
  ...
  for (ast_node *ast = head; ast; ast = ast->right) {
    ...
    // ELSE IF [cond_action]
    sem_cond_action(CS, cond_action);
...
static void sem_expr_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_eq(expr)) {
    report_error(CS, expr, "CQL0471: a top level equality is almost certainly an error. ':=' is assignment, not '='\n", NULL);
    record_error(CS, expr);
    record_error(CS, ast);
    return;
  }
  ...
  symtab_entry *entry = symtab_find(CS->sem.exprs, expr->type);
  Invariant(entry);
  sem_expr_dispatch *disp = (sem_expr_dispatch*)entry->val;

  // rewrite top level +=, -=, /=, *=, and %= before we go any further
  // if it's not one of these (e.g. ==) then this is a no-op
  rewrite_op_equals_assignment_if_needed(CS, expr, disp->str);
  ...
  if (is_ast_expr_assign(expr)) {
     ...
     if (is_ast_array(left)) {
       sem_validate_array_transform(CS, left, "set_in");
       rewritten_assignment = true;
     }
     else if (is_ast_dot(left)) {
       sem_validate_dot_transform(CS, left, "set", "set_in");
       rewritten_assignment = true;
     }
     ...
     if (rewritten_assignment) {
       ...
       rewrite_append_arg(CS, left, value);
  ...
  bool_t hard_error = sem_reverse_apply_if_needed(CS, expr, SEM_REVERSE_APPLY_REWRITE_ONLY);
  if (hard_error) {
    record_error(CS, ast);
    return;
  }
  ...
  if (is_ast_call(expr)) {
    ...
    if (!filters) {
      ...
      if (proc_stmt) {
        ...
        if (!rewrite_ast_star_if_needed(CS, arg_list, name_ast)) {
          record_error(CS, ast);
          return;
        }
      ...
      else if (find_unchecked_proc(CS, name)) {
      ...
      if  (perform_proc_rewrite) {
        ...
        rewrite_func_call_as_proc_call(CS, ast);
    ...
    if (!is_id(expr->left)) {
      report_error(CS, expr, "CQL0465: left operand of assignment operator must be a name", ":=");
      record_error(CS, expr);
      record_error(CS, ast);
      return;
    }
...
static void sem_if_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (elseif) {
    sem_elseif_list(CS, elseif);
    if (is_error(elseif)) {
      record_error(CS, if_alt);
      goto error;
    }
  }
...
cleanup:
  ...
  if (!elseif && !elsenode) {
    ...
    if (stmt_list && stmt_list_contains_control_stmt(stmt_list)) {
      ...
      sem_set_improvements_for_false_condition(CS, cond_expr);
    }
  }
...
static void sem_guard_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  rewrite_guard_stmt_to_if_stmt(CS, ast);
}
...
static void sem_delete_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!is_ast_create_table_stmt(table_ast)) {
    report_error(CS, name_ast, "CQL0152: cannot delete from a view", name);
    record_error(CS, ast);
    goto cleanup;
  }
  ...
  if (BACKING_REWRITE_NEEDED(ast, table_ast)) {
    rewrite_delete_statement_for_backed_table(CS, sem_recover_with_stmt(ast), CS->sem.backed_tables_list);
  }
...
static void sem_with_delete_stmt(CqlState* _Nonnull CS, ast_node *stmt) {
  ...
  sem_delete_stmt(CS, delete_stmt);
...
static void sem_update_entry(CqlState* _Nonnull CS, ast_node *ast, symtab *update_columns, sem_join *from_jptr) {
  ...
  if (!symtab_add(CS, update_columns, left->sem->name, NULL)) {
    report_error(CS, left, "CQL0153: duplicate target column name in update statement", left->sem->name);
    record_error(CS, ast);
    record_error(CS, left);
    return;
  }
  ...
  if (from_jptr) {
    PUSH_JOIN(from_scope, from_jptr);
    sem_root_expr(CS, right, SEM_EXPR_CONTEXT_SELECT_LIST);
    POP_JOIN();
  }
  else {
    sem_root_expr(CS, right, SEM_EXPR_CONTEXT_SELECT_LIST);
  ...
  if (!sem_verify_assignment(CS, right, sem_type_left, sem_type_right, left->sem->name)) {
    record_error(CS, ast);
    return;
  }

  sem_combine_kinds(CS, right, left->sem->kind);
  if (is_error(right)) {
    record_error(CS, ast);
    return;
  }
...
static void sem_update_list(CqlState* _Nonnull CS, ast_node *head, sem_join *from_jptr) {
  ...
  for (ast_node *ast = head; ast; ast = ast->right) {
    ...
    sem_update_entry(CS, update_entry, update_columns, from_jptr);
    if (is_error(update_entry)) {
      record_error(CS, head);
      symtab_delete(CS, update_columns);
      return;
    }
  }
...
static void sem_update_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.enforcement.strict_update_from && query_parts) {
    report_error(CS, ast,
  ...
  if (name_ast) {
    ...
    if (!is_ast_create_table_stmt(table_ast)) {
      report_error(CS, name_ast, "CQL0155: cannot update a view", name);
      goto cleanup;
    }
    ...
    if (CS->sem.in_upsert) {
      report_error(CS, name_ast, "CQL0281: upsert statement does not include table name in the update statement", name);
      goto cleanup;
    }
  } else {
    ...
    if (!CS->sem.in_upsert) {
      report_error(CS, ast, "CQL0282: update statement require table name", NULL);
      goto cleanup;
    }
    table_ast = CS->sem.current_upsert_table_ast;
    Invariant(table_ast);
  }
  ...
  if (is_ast_columns_values(update_list)) {
    ...
    rewrite_column_values_for_update_stmts(CS, ast, columns_values, table_ast->sem->sptr);

    if (is_error(ast)) {
      goto cleanup;
    }

    update_list = rewrite_column_values_as_update_list(CS, columns_values);
  ...
  sem_update_list(CS, update_list, from_jptr);
  ...
  if (query_parts && table_ast && is_backed(table_ast->sem->sem_type)) {
    report_error(CS, ast, "CQL0497: FROM clause not supported when updating backed table", table_name);
    goto cleanup;
  }
...
cleanup:
  ...
  if (BACKING_REWRITE_NEEDED(ast, table_ast)) {
    rewrite_update_statement_for_backed_table(CS, sem_recover_with_stmt(ast), CS->sem.backed_tables_list);
  }
...
static void sem_update_cursor_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_expr_names(columns_values)) {
    rewrite_expr_names_to_columns_values(CS, columns_values);
    Contract(is_ast_columns_values(columns_values));
  }

  rewrite_empty_column_list(CS, columns_values, cursor->sem->sptr);

  rewrite_like_column_spec_if_needed(CS, columns_values);
  if (is_error(columns_values)) {
    record_error(CS, ast);
    return;
  }

  rewrite_from_shape_if_needed(CS, ast, columns_values);
  if (is_error(ast)) {
    return;
  }
  ...
  if (!is_auto_cursor(sem_type)) {
    report_error(CS, cursor, "CQL0067: cursor was not used with 'fetch [cursor]'", name);
    record_error(CS, cursor);
    record_error(CS, ast);
    return;
  }
  ...
  sem_join *jptr = sem_join_from_sem_struct(CS, cursor->sem->sptr);
  ...
  if (valid && check.count != cols) {
    report_error(CS, ast, "CQL0157: count of columns differs from count of values", NULL);
    valid = 0;
  }

  if (valid) {
    valid = sem_validate_compatible_cols_vals(CS, name_list, insert_list);
  }
...
static void sem_with_update_stmt(CqlState* _Nonnull CS, ast_node *stmt) {
  ...
  sem_update_stmt(CS, update_stmt);
...
static int32_t sem_insert_dummy_spec(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_root_expr(CS, seed_expr, SEM_EXPR_CONTEXT_NONE);
  ...
  if (is_nullable(sem_type) || core_type_of(sem_type) != SEM_TYPE_INTEGER) {
    report_error(CS, seed_expr, "CQL0156: seed expression must be a non-nullable integer", NULL);
    record_error(CS, ast);
    return false;
  }
...
error:
  report_error(CS, stmt, "CQL0334: @dummy_seed @dummy_nullables @dummy_defaults many only be used with a single VALUES row", NULL);
  record_error(CS, stmt);
  return false;
}
...
static void sem_column_spec_and_values(CqlState* _Nonnull CS, ast_node *ast, ast_node *table_ast) {
  ...
  if (is_ast_insert_list(value_source)) {
    ...
    if (!sem_validate_one_values_row(CS, value_source, &insert_list)) {
  ...
  if (insert_dummy_spec) {
    dummy_flags = sem_insert_dummy_spec(CS, insert_dummy_spec);
  ...
  if (select_stmt) {
    ...
    if (CS->sem.enforcement.strict_insert_select && sem_select_stmt_is_mixed_results(CS, select_stmt)) {
      report_error(CS, select_stmt,
  ...
  if (valid) {
    ...
    for (uint32_t icol = 0; icol < sptr->count; icol++) {
      ...
      if (!insert_dummy_spec) {
        report_error(CS, ast, "CQL0158: required column missing in INSERT statement", sptr->names[icol]);
        valid = 0;
        break;
      }
      ...
      sem_synthesize_dummy_value(CS, &info);
  ...
  if (insert_dummy_spec) {
    ...
    ast_set_left(insert_type, new_ast_seed_stub(CS, insert_dummy_spec->left, insert_dummy_spec->right));
    AST_REWRITE_INFO_RESET();
  }

  if (valid) {
    if (select_stmt) {
      valid = sem_validate_compatible_table_cols_select(CS, table_ast, name_list, select_stmt);
    }
    else {
      valid = sem_validate_compatible_table_cols_vals(CS, table_ast, name_list, insert_list);
    }
  }
...
static void sem_insert_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!is_ast_create_table_stmt(table_ast)) {
    report_error(CS, name_ast, "CQL0161: cannot insert into a view", name);
    record_error(CS, ast);
    goto cleanup;
  }
  ...
  if (is_select_stmt(columns_values)) {
    sem_select_stmt(CS, columns_values);
    if (is_error(columns_values)) {
      record_error(CS, ast);
      goto cleanup;
    }

    sem_verify_no_anon_columns(CS, columns_values);
    if (is_error(columns_values)) {
      record_error(CS, ast);
      goto cleanup;
    }

    rewrite_select_stmt_to_columns_values(CS, columns_values);
    Contract(is_ast_columns_values(columns_values));
  }
  ...
  if (CS->sem.in_upsert) {
    if (!is_ast_insert_normal(insert_type)) {
      report_error(CS, insert_type, "CQL0283: upsert syntax only supports INSERT INTO", name);
      record_error(CS, ast);
      goto cleanup;
    }
      ...
      report_error(CS, insert_type, "CQL0316: upsert-clause is not compatible with DEFAULT VALUES", name);
      record_error(CS, ast);
      goto cleanup;
    }
  }
  ...
  if (is_ast_columns_values(columns_values)) {
    rewrite_empty_column_list(CS, columns_values, table_ast->sem->sptr);
    ...
    sem_column_spec_and_values(CS, ast, table_ast);
  }
  else {
    // INSERT [conflict resolution] INTO name DEFAULT VALUES
  ...
  if (BACKING_REWRITE_NEEDED(ast, table_ast)) {
    rewrite_insert_statement_for_backed_table(CS, sem_recover_with_stmt(ast), CS->sem.backed_tables_list);
  }
...
static void sem_with_upsert_stmt(CqlState* _Nonnull CS, ast_node *stmt) {
  ...
  sem_upsert_stmt(CS, upsert_stmt);
...
static void sem_upsert_stmt(CqlState* _Nonnull CS, ast_node *stmt) {
  ...
  CS->sem.current_upsert_table_ast = table_ast;

  sem_insert_stmt(CS, insert_stmt);
  ...
  sem_struct *sptr = new_sem_struct(CS, "excluded", names_count);
  ast = name_list;
  ...
  conflict_target->sem = new_sem(CS, SEM_TYPE_STRUCT);
  conflict_target->sem->sptr = sptr;

  // and here is our join target
  sem_join *jptr_excluded = sem_join_from_sem_struct(CS, sptr);

  int32_t select_count = 1;
  bool_t found_where_stmt = is_root_select_stmt_has_opt_where_node(insert_stmt, &select_count);
  if (select_count == 0 && !found_where_stmt && !CS->sem.in_upsert_rewrite) {
    report_error(CS, insert_stmt, "CQL0280: upsert statement requires a where clause if the insert clause uses select", NULL);
    record_error(CS, insert_stmt);
    goto error;
  }

  if (indexed_columns) {
    if (!sem_validate_name_list(CS, indexed_columns, CS->sem.current_upsert_table_ast->sem->jptr)) {
      record_error(CS, upsert_update);
      record_error(CS, conflict_target);
      goto error;
    }
    ...
    if (!indexed_columns->right) {
      ...
      is_single_unique_key = is_column_unique_key(CS->sem.current_upsert_table_ast, column_name);
    }
    ...
    if (!is_single_unique_key) {
      bool_t valid = find_referenceable_columns(CS, CS->sem.current_upsert_table_ast,
                                                validate_referenceable_fk_def_callback,
                                                indexed_columns);
      if (!valid) {
        report_error(CS, indexed_columns, "CQL0279: columns referenced in an UPSERT conflict target must exactly match a unique key the target table", NULL);
        record_error(CS, upsert_update);
        record_error(CS, conflict_target);
        goto error;
      }
  ...
  if (opt_where) {
    ...
    PUSH_JOIN_BLOCK()
    PUSH_JOIN(upsert_scope, CS->sem.current_upsert_table_ast->sem->jptr)
    sem_opt_where(CS, opt_where);
    POP_JOIN()
    POP_JOIN()
    if (is_error(opt_where)) {
  ...
  record_ok(CS, upsert_update);

  if (BACKING_REWRITE_NEEDED(stmt, CS->sem.current_upsert_table_ast)) {
    rewrite_upsert_statement_for_backed_table(CS, sem_recover_with_stmt(stmt), CS->sem.backed_tables_list);
  }
  ...
  ast_node *src)
{
  ...
  if (!is_blob(blob->sem->sem_type)) {
    report_error(CS, blob, "CQL0461: fetch from blob operand is not a blob", NULL);
    record_error(CS, ast_error);
    return;
  }
  ...
  if (!is_auto_cursor(cursor->sem->sem_type)) {
    report_error(CS, cursor, "CQL0454: cursor was not declared for storage", cursor->sem->name);
    record_error(CS, ast_error);
    return;
  }
  ...
  if (!kind) {
    report_error(CS, blob, "CQL0455: blob variable must have a type-kind for type safety", blob_name);
    record_error(CS, ast_error);
    return;
  }
  ...
  if (!is_ast_create_table_stmt(table_ast)) {
    report_error(CS, blob, "CQL0456: blob type is a view, not a table", kind);
    record_error(CS, ast_error);
    return;
  }

  if (!is_table_blob_storage(CS, table_ast)) {
    report_error(CS, blob, "CQL0457: the indicated table is not marked with @attribute(cql:blob_storage)", kind);
    record_error(CS, ast_error);
    return;
  }
  ...
  sem_verify_identical_columns(CS, dest, src, "in the cursor and the blob type");
  if (is_error(src)) {
    record_error(CS, ast_error);
    return;
  }
  ...
  ast_node *var = find_local_or_global_variable(CS, cursor->sem->name);
...
static void sem_fetch_cursor_from_blob_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_validate_cursor_blob_compat(CS, ast, cursor, blob, cursor, blob);
}
...
static void sem_set_blob_from_cursor_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // the final error is set by the helper, 'ast' will have the code regardless
  sem_validate_cursor_blob_compat(CS, ast, cursor, blob, blob, cursor);
}
...
static void sem_fetch_values_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (try_rewrite_blob_fetch_forms(CS, ast)) {
    // true means affirmative success or failure and errors logged already
    CS->sem.has_dml = 1;  // this implies return code and all that comes with it
    if (!is_error(ast)) {
      ...
      // appropriately.
      sem_set_has_row_improved(CS, cursor->sem->name);
    }
    return;
  }
  ...
  if (!(cursor->sem->sem_type & SEM_TYPE_VALUE_CURSOR)) {
    report_error(CS, cursor, "CQL0165: fetch values is only for value cursors, not for sqlite cursors", cursor->sem->name);
    record_error(CS, ast);
    return;
  }
  ...
  cursor->sem->jptr = sem_join_from_sem_struct(CS, cursor->sem->sptr);
  ...
  // Ensure that the number of values matches the number of columns.
  if (valid && check.count != cols) {
    report_error(CS, ast, "CQL0166: count of columns differs from count of values", NULL);
    valid = 0;
  }
  ...
  if (valid) {
    ...
    for (uint32_t icol = 0; icol < sptr->count; icol++) {
      ...
      if (!will_use_null && !insert_dummy_spec) {
        report_error(CS, ast, "CQL0167: required column missing in FETCH statement", sptr->names[icol]);
        valid = 0;
        break;
      }
      ...
      if (is_blob(sem_type_col) && !info.use_null) {
        report_error(CS, ast, "CQL0168: CQL has no good way to generate dummy blobs; not supported for now", NULL);
        valid = 0;
        break;
      }
  ...
  if (valid) {
    ...
    sem_struct *sptr = cursor->sem->sptr;
    for (uint32_t i = 0; i < sptr->count; i++) {
      sem_unset_notnull_improved(CS, sptr->names[i], cursor->sem->name);
    }
...
static bool_t sem_name_check(CqlState* _Nonnull CS, name_check *check) {
  ...
  for (ast_node *item = check->name_list; item; item = item->right) {
    ...
    if (is_ast_name_list(item)) {
      ...
      if (!sem_find_column_for_name(CS, name_ast, name) || is_error(name_ast)) {
        report_error(CS, name_ast, "CQL0171: name not found", name);
        record_error(CS, name_ast);
        record_error(CS, check->name_list);
        valid = false;
        break;
      }
      ...
      sem_validate_index_expr_for_jptr(CS, check->jptr, expr);
      if (is_error(expr)) {
        record_error(CS, check->name_list);
        valid = false;
        break;
      ...
      item_name = expr_as_text(CS, expr);
    ...
    if (!symtab_add(CS, check->names, item_name, NULL)) {
      report_error(CS, target, "CQL0172: name list has duplicate name", item_name);
...
static ast_node *printf_col_for_dummy(CqlState* _Nonnull CS, CSTR col, CSTR seed_name) {
  ...
  ast_node *ast_seed = new_ast_str(CS, seed_name);

  ast_node *args = new_ast_arg_list(CS, ast_string, new_ast_arg_list(CS, ast_seed, NULL));
  ast_node *call_filter_clause = new_ast_call_filter_clause(CS, NULL, NULL);
  ast_node *call_arg_list = new_ast_call_arg_list(CS, call_filter_clause, args);
  return new_ast_call(CS, ast_printf, call_arg_list);
}
...
static void sem_synthesize_dummy_value(CqlState* _Nonnull CS, dummy_info *info) {
  ...
  if (!symtab_find(scope, seed_name)) {
    ast_node *asts = new_ast_str(CS, seed_name);
    asts->sem = new_sem(CS, SEM_TYPE_INTEGER | SEM_TYPE_NOTNULL | SEM_TYPE_VARIABLE);
    asts->sem->name = seed_name;
    symtab_add(CS, scope, seed_name, asts);
  }

  // Look up the name in the current scope, and only that scope.  No locals
  // No nothing.  Just the columns in the indicated type.
  ast_node *ast_col = new_str_or_qstr(CS, info->name, info->sem_type_col);

  PUSH_JOIN_BLOCK()
  PUSH_JOIN(info_scope, info->jptr);
  bool_t found = sem_find_column_for_name(CS, ast_col, info->name);
  Invariant(found);
  ...
  if (info->use_null) {
    ...
    expr = new_ast_null(CS);
  }
  else if (is_numeric(info->sem_type_col)) {
    // numbers dummy value is seed
    expr = new_ast_str(CS, seed_name);
  }
  else if (is_blob(info->sem_type_col)) {
    ast_node *inner = printf_col_for_dummy(CS, info->name, seed_name);
    expr = new_ast_cast_expr(CS, inner, new_ast_type_blob(CS, NULL));
  }
  else {
    // strings are column_name_{seed} -- using printf
    expr = printf_col_for_dummy(CS, info->name, seed_name);
  }

  new_tail = new_ast_insert_list(CS, expr, NULL);
...
static bool_t sem_validate_compatible_table_cols_vals(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *name_list, ast_node *insert_list) {
  ...
  for (ast_node *item = name_list ; item; item = item->right, value = value->right) {
    ...
    sem_root_expr(CS, expr, SEM_EXPR_CONTEXT_WHERE); // non-aggregate context
    if (is_error(expr)) {
      return false;
    }
    ...
    sem_combine_kinds(CS, expr, col->sem->kind);
    if (is_error(expr)) {
      return false;
    }

    // otherwise the columns have to be assignment compatible
    if (!sem_verify_assignment(CS, expr, col->sem->sem_type, expr->sem->sem_type, col->sem->name)) {
      return false;
    }
  }
...
static bool_t sem_validate_compatible_table_cols_select(CqlState* _Nonnull CS, ast_node *table_ast, ast_node *name_list, ast_node *select_stmt) {
  ...
  for (ast_node *item = name_list ; item; item = item->right, icol_select++) {
    ...
    sem_combine_kinds(CS, col, sptr_select->kinds[icol_select]);
    if (is_error(col)) {
      return false;
    }

    // otherwise the columns have to be assignment compatible
    if (!sem_verify_assignment(CS, col, col->sem->sem_type, sptr_select->semtypes[icol_select], col->sem->name)) {
      return false;
    }
  }
...
static bool_t sem_validate_compatible_cols_vals(CqlState* _Nonnull CS, ast_node *name_list, ast_node *values) {
  ...
  for (ast_node *item = name_list ; item; item = item->right, value = value->right) {
    ...
    sem_combine_kinds(CS, col, expr->sem->kind);
    if (is_error(col)) {
      return false;
    }
...
static void sem_assign(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!variable) {
    report_error(CS, ast, "CQL0173: variable not found", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (is_struct(sem_type_var)) {
    report_error(CS, ast, "CQL0174: cannot set a cursor", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (!sem_verify_assignment(CS, expr, sem_type_var, expr->sem->sem_type, name)) {
    record_error(CS, name_ast);
    record_error(CS, ast);
  }

  sem_combine_kinds(CS, expr, variable->sem->kind);
  if (is_error(expr)) {
    record_error(CS, ast);
  }
  ...
  if (is_not_nullable(expr->sem->sem_type)) {
    sem_set_notnull_improved(CS, name, NULL);
  } else {
    sem_unset_notnull_improved(CS, name, NULL);
  }
...
static void sem_let_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!sem_verify_legal_variable_name(CS, variable, name)) {
    record_error(CS, ast);
    return;
  }
  ...
  if (is_null_type(expr->sem->sem_type)) {
    report_error(CS, ast, "CQL0056: NULL expression has no type to imply the declaration of variable", name);
    record_error(CS, ast);
    return;
  }
  ...
  variable->sem = ast->sem = new_sem(CS, sem_type_var);
  variable->sem->name = name;
  variable->sem->kind = expr->sem->kind;
  add_variable(CS, name, variable);
}
...
static void sem_param(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (symtab_find(CS->sem.locals, name)) {
    report_error(CS, name_ast, "CQL0175: duplicate parameter name", name);
    record_error(CS, ast);
    return;
  }
  ...
  // We set this even if `CS->sem.current_proc` is NULL (i.e., we set it for the DECLARE
  ...
  ast->sem = param_detail->sem = name_ast->sem = new_sem(CS, sem_type);

  // [name]
  ast->sem->name = name;
  ast->sem->kind = data_type->sem->kind;

  Invariant(CS->sem.current_variables == CS->sem.locals);
  add_variable(CS, name, ast);
}
...
static ast_node *sem_find_likeable_proc_args(CqlState* _Nonnull CS, ast_node *like_ast, int32_t likeable_for) {
  ...
  ast_node *proc= find_proc(CS, like_name);
  if (!proc) {
    report_error(CS, like_ast, "CQL0069: name not found", like_name);
    goto error;
  }

  if (is_error(proc)) {
    report_error(CS, like_ast, "CQL0069: name not found (proc had errors, cannot be used)", like_name);
    goto error;
  }
  ...
  if (yyget_lineno(CS->scanner) == -1) {
    ...
    result = new_ast_str(CS, like_name);
    AST_REWRITE_INFO_RESET();
  }
  else {
    result = new_ast_str(CS, like_name);
  ...
  ast_node *params = get_proc_params(CS, proc);

  if (!params) {
    report_error(CS, like_ast, "CQL0262: LIKE ... ARGUMENTS used on a procedure with no arguments", like_name);
    goto error;
  }

  for (; params; params = params->right, count++) ;

  CSTR shape_name = dup_printf(CS, "%s[arguments]", proc_name);

  sem_struct *sptr = new_sem_struct(CS, shape_name, count);

  params = get_proc_params(CS, proc);
  ...
  result->sem = new_sem(CS, SEM_TYPE_STRUCT);
  result->sem->sptr = sptr;
  return result;

error:
    record_error(CS, like_ast);
    record_error(CS, name_ast);
    return NULL;
}
...
cql_noexport ast_node *sem_find_shape_def_base(CqlState* _Nonnull CS, ast_node *like_ast, int32_t likeable_for) {
  ...
  if (like_ast->right) {
    ...
    return sem_find_likeable_proc_args(CS, like_ast, likeable_for);
  }

  EXTRACT_NAME_AST(name_ast, like_ast->left);
  EXTRACT_STRING(like_name, name_ast);

  ast_node *found_shape = find_local_or_global_variable(CS, like_name);
  if (found_shape) {
    if (!is_cursor(found_shape->sem->sem_type)) {
      report_error(CS, like_ast, "CQL0200: variable is not a cursor", like_name);
      goto error;
    }
  }
  ...
  if (!found_shape) {
    ...
    found_shape = find_arg_bundle(CS, like_name);
  }

  if (!found_shape) {
    // note: it's ok to use the LIKE construct on deleted tables too, hence even_deleted
    found_shape = find_table_or_view_even_deleted(CS, like_name);
  }

  if (!found_shape) {
    found_shape = find_proc(CS, like_name);
    if (found_shape) {
      if (!found_shape->sem->sptr) {
        report_error(CS, like_ast, "CQL0178: proc has no result", like_name);
        goto error;
      }
    }
  }
  ...
  if (!found_shape) {
    found_shape = find_interface_type(CS, like_name);
  }

  if (!found_shape || is_error(found_shape)) {
    report_error(CS, like_ast, "CQL0202: must be a cursor, proc, table, or view", like_name);
    goto error;
  }

  record_ok(CS, like_ast);
  ...
  *new_shape->sem = *found_shape->sem;
  ...
  new_shape->sem->sptr = sem_clone_struct_strip_flags(CS, new_shape->sem->sptr, SEM_TYPE_INFERRED_NOTNULL);
...
cql_noexport ast_node *sem_find_shape_def(CqlState* _Nonnull CS, ast_node *shape_def, int32_t likeable_for) {
  ...
  ast_node *base_shape = sem_find_shape_def_base(CS, like, likeable_for);
  if (!base_shape) {
    goto error;
  }
  ...
  if (shape_exprs) {
    ...
    if (!sem_verify_no_duplicate_shape_exprs(CS, shape_exprs)) {
      goto error;
    }
    ...
    while (iter) {
      ...
      if (adding != adding_item) {
        report_error(CS, iter->left, "CQL0494: mixing adding and removing columns from a shape", name);
        goto error;
      }

      int32_t iold = find_col_in_sptr(sptr_old, name);
      if (iold < 0) {
        report_error(CS, iter->left, "CQL0069: name not found", name);
        goto error;
      }
    ...
    if (count == 0) {
      report_error(CS, shape_exprs, "CQL0495: no columns were selected in the LIKE expression", NULL);
      goto error;
    }

    sem_struct *sptr_new = new_sem_struct(CS, "select", count);
    ...
    result = new_ast_shape_def(CS, NULL, NULL);
    result->sem = new_sem(CS, SEM_TYPE_STRUCT);
...
static void sem_params(CqlState* _Nonnull CS, ast_node *head, bytebuf *args_info) {
  ...
  if (args_info) {
    ...
    if (count) {
      ...
      ast_node *ast_args = new_ast_str(CS, args);
      ast_args->sem = new_sem(CS, SEM_TYPE_STRUCT | SEM_TYPE_HAS_SHAPE_STORAGE);
      ast_args->sem->name = args;
      sptr = new_sem_struct(CS, args, count);
      ast_args->sem->sptr = sptr;
      add_arg_bundle(CS, ast_args, args);
...
static void sem_validate_unique_names_struct_type(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  for (uint32_t i = 0; i < sptr->count; i++) {
    if (!symtab_add(CS, fields, sptr->names[i], NULL)) {
      report_error(CS, ast, "CQL0180: duplicate column name in result not allowed", sptr->names[i]);
      record_error(CS, ast);
      break;
    }
  }

  symtab_delete(CS, fields);
  return;
}
  ...
  void *context)
{
  ...
  for (ast_node *dummy_test_list = misc_attr_value_list->right; dummy_test_list; dummy_test_list = dummy_test_list->right) {
    ...
    ast_node *table = find_table_or_view_even_deleted(CS, table_name);
    if (!table) {
      report_dummy_test_error(CS,
        table_list->left,
    ...
    bytebuf_close(CS, &column_types);
    bytebuf_close(CS, &column_names);
  }

  record_ok(CS, misc_attr_value_list->left);
  ...
  void *context)
{
  if (misc_attr_prefix &&
    ...
    for (ast_node *list = ast_misc_attr_value_list; list; list = list->right) {
      ...
      if (is_ast_misc_attr_value_list(misc_attr_value)) {
        if (sem_autotest_dummy_test(CS, misc_attr_value, context)) {
          if (*error) {
            record_error(CS, misc_attr_value->left);
          }
        } else {
          report_dummy_test_error(CS,
...
static void sem_autotests(CqlState* _Nonnull CS, ast_node *misc_attrs) {
  ...
  find_misc_attrs(CS, misc_attrs, sem_find_ast_misc_attr_callback, &error);
  if (error) {
    record_error(CS, misc_attrs);
    return;
  }
...
static uint32_t sem_column_name_annotation(CqlState* _Nonnull CS, ast_node *misc_attrs, find_annotation_values find, CSTR target) {
  ...
  CS->sem.annotation_target = target;
  uint32_t result = find(CS, misc_attrs, sem_column_name_exist_in_result_set, misc_attrs);
  CS->sem.annotation_target = NULL;
  return result;
}
...
static void sem_one_autodrop(CqlState* _Nonnull CS, CSTR name, ast_node *misc_attr_value, void *context) {
  ...
  ast_node *temp_table = find_usable_and_not_deleted_table_or_view(CS,
    name,
  ...
  if (!is_ast_create_table_stmt(temp_table)) {
    report_error(CS, misc_attr_value->parent, "CQL0182: autodrop target is not a table", name);
    record_error(CS, misc_attrs);
    return;
  }
  ...
  if (!temp) {
    report_error(CS, misc_attr_value->parent, "CQL0183: autodrop target must be a temporary table", name);
    record_error(CS, misc_attrs);
    return;
  }
...
static void verify_identical_table_params_callback(CqlState* _Nonnull CS, void *context, CSTR name, ast_node *cte_decl) {
  ...
  if (!entry) {
    ...
    symtab_add(CS, info->names, name, cte_decl);
    return;
  }

  // existing name must be identical
  sem_verify_identical_columns(CS, (ast_node*)entry->val, cte_decl, name);
  if (is_error(cte_decl)) {
...
static void sem_shared_fragment(CqlState* _Nonnull CS, ast_node *misc_attrs, ast_node *create_proc_stmt) {
  ...
  if (stmt_list->right) {
    report_error(CS, stmt_list, "CQL0179: shared fragments must consist of exactly one top level statement", proc_name);
    record_error(CS, misc_attrs);
    record_error(CS, stmt_list);
    record_error(CS, create_proc_stmt);
    return;
  }
  ...
  if (!is_select_stmt(stmt) && !is_ast_if_stmt(stmt)) {
    report_error(CS, stmt, "CQL0441: shared fragments may only have IF, SELECT, or WITH...SELECT at the top level", proc_name);
    record_error(CS, misc_attrs);
    record_error(CS, stmt_list);
    record_error(CS, create_proc_stmt);
    return;
  ...
  if (is_ast_if_stmt(stmt)) {
    ...
    symtab_delete(CS, bind_info.names);
    ...
    if (info.missing_else) {
      ...
      AST_REWRITE_INFO_SET(ast->lineno, ast->filename);
      ast_node *select_nothing_ast = new_ast_select_nothing_stmt(CS);
      ast_node *stmt_list_ast = new_ast_stmt_list(CS, select_nothing_ast, NULL);
      if_alt->right = new_ast_else(CS, stmt_list_ast);
      AST_REWRITE_INFO_RESET();
      ...
      sem_select_nothing_stmt(CS, select_nothing_ast);
    }

    if (info.bad_statement_form) {
      report_error(CS, info.bad_statement_form, "CQL0443: shared fragments with conditionals must have exactly one SELECT, or WITH...SELECT in each statement list", proc_name);
      record_error(CS, misc_attrs);
      record_error(CS, stmt_list);
      record_error(CS, create_proc_stmt);
      return;
    ...
    if (info.non_select_stmt) {
      report_error(CS, info.non_select_stmt, "CQL0443: shared fragments with conditionals must have exactly SELECT, or WITH...SELECT in each statement list", proc_name);
      record_error(CS, misc_attrs);
      record_error(CS, stmt_list);
      record_error(CS, create_proc_stmt);
      return;
  ...
  for (ast_node *ast = params; ast; ast = ast->right) {
    ...
    if (is_out_parameter(param->sem->sem_type)) {
      report_error(CS, stmt_list,  "CQL0208: shared fragments cannot have any out or in/out parameters", param->sem->name);
      record_error(CS, misc_attrs);
      record_error(CS, stmt_list);
      record_error(CS, create_proc_stmt);
      return;
...
static void sem_validate_one_interface(CqlState* _Nonnull CS, CSTR _Nonnull interface_name, ast_node *_Nonnull attr, void *_Nullable context) {
  ...
  ast_node *name_ast = get_proc_name(CS, create_proc_stmt);
  EXTRACT_STRING(proc_name, name_ast);

  ast_node *interface = find_interface_type(CS, interface_name);
  if (!interface) {
    report_error(CS, attr, "CQL0482: interface not found", interface_name);
    record_error(CS, attr);
    goto error;
  }
  ...
  for (uint32_t i = 0; i < proc_sptr->count; i++) {
    symtab_add(CS, names, proc_sptr->names[i], (void *)(uint64_t)i);
  }
  ...
  for (uint32_t i = 0; i < interface_sptr->count; ++i) {
    ...
    if (!entry) {
      CSTR msg = dup_printf(CS, "CQL0484: procedure '%s' is missing column '%s' of interface '%s'",
        proc_name, interface_column_name, interface_name);
      report_error(CS, create_proc_stmt, msg, NULL);
      goto error;
    }
    ...
    if (
      ...
      report_sem_type_mismatch(CS, expected_type, actual_type, create_proc_stmt, error, interface_column_name);
      goto error;
    }
  }
...
static bool_t sem_validate_current_proc_params_are_initialized(CqlState* _Nonnull CS, ast_node *error_ast) {
  ...
  for (ast_node *param_item = params; param_item; param_item = param_item->right) {
    ...
    if (sem_type & SEM_TYPE_INIT_REQUIRED && !(sem_type & SEM_TYPE_INIT_COMPLETE)) {
      ...
      report_error(CS, error_ast, "CQL0439: nonnull reference OUT parameter possibly not always initialized", name);
      return false;
    }
  }
...
static void sem_inside_create_proc_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  int32_t saved_between_count = CS->sem.between_count;
  CS->sem.has_dml = 0;
  CS->sem.current_variables = CS->sem.locals = symtab_new();
  CS->sem.arg_bundles = symtab_new();
  CS->sem.between_count = 0;
  CS->sem.in_proc_savepoint = false;
  ...
  if (params) {
    bytebuf *args_info = symtab_ensure_bytebuf(CS, CS->sem.proc_arg_info, proc_name);
    sem_params(CS, params, args_info);
    if (is_error(params)) {
      goto error;
    }
  }
  ...
  if (!stmt_list) {
    if (is_proc_shared_fragment(CS, ast)) {
      report_error(CS, ast, "CQL0440: fragments may not have an empty body", proc_name);
      goto error;
    }
  } else {
    // BEGIN [stmt_list] END
    ...
    if (ast->sem && ast->sem->sptr) {
      ast->sem->sptr = new_sem_struct_strip_table_flags(CS, ast->sem->sptr);
  ...
  if (CS->sem.current_proc_contains_try_is_proc_body) {
    ...
    if (!sem_validate_current_proc_params_are_initialized(CS, ast)) {
      goto error;
    }
  }
...
cleanup:
  FLOW_POP_CONTEXT_NORMAL();
  symtab_delete(CS, CS->sem.locals);
  symtab_delete(CS, CS->sem.arg_bundles);
  CS->sem.locals = NULL;
  CS->sem.arg_bundles = NULL;
  CS->sem.current_variables = CS->sem.globals;
  CS->sem.between_count = saved_between_count;
  CS->sem.current_proc_contains_try_is_proc_body = false;
...
static void sem_validate_ok_table_scan_value(CqlState* _Nonnull CS, ast_node *misc_attrs, ast_node *ast_misc_attr_value) {
  ...
  if (!find_usable_table_or_view_even_deleted(CS, table_name, misc_attrs, "CQL0326: the table name in ok_table_scan does not exist")) {
    record_error(CS, ast_misc_attr_value);
    return;
  }

  record_ok(CS, ast_misc_attr_value);
}
...
static void sem_misc_attrs_ok_table_scan(
  ...
  if (!Strcasecmp(misc_attr_prefix, "cql") &&
    ...
    if (!is_ast_create_proc_stmt(any_stmt)) {
        report_error(CS, misc_attrs, "CQL0329: ok_table_scan attribute can only be used in a create procedure statement", NULL);
        record_error(CS, misc_attrs);
        return;
      ...
      for (ast_node *list = ast_misc_attr_values; list; list = list->right) {
        ...
        sem_validate_ok_table_scan_value(CS, misc_attrs, ast_misc_attr_value);
      ...
      sem_validate_ok_table_scan_value(CS, misc_attrs, ast_misc_attr_values);
...
static void sem_misc_attrs_no_table_scan(
  ...
  if (ast_misc_attr_values != NULL) {
    report_error(CS, ast_misc_attr_values, "CQL0327: a value should not be assigned to no_table_scan attribute", NULL);
    record_error(CS, ast_misc_attr_values);
    record_error(CS, misc_attrs);
    return;
  }
  ...
  if (is_ast_stmt_and_attr(misc_attrs->parent)) {
    ...
    if (!is_ast_create_table_stmt(stmt)) {
      report_error(CS, misc_attrs, "CQL0328: no_table_scan attribute may only be added to a create table statement", NULL);
      record_error(CS, misc_attrs);
      return;
    }
  }
...
static void sem_misc_attrs_vault_sensitive(
  ...
  if (is_ast_stmt_and_attr(misc_attrs->parent)) {
    ...
    if (!is_ast_create_proc_stmt(stmt)) {
      report_error(CS, misc_attrs, "CQL0328: vault_sensitive attribute may only be added to a create procedure statement", NULL);
      record_error(CS, misc_attrs);
      return;
    }
  }

  if (exists_attribute_str(CS, misc_attrs, "vault_sensitive")) {
    encode_info info;
...
static void sem_misc_attrs_alias_of(
  ...
  if (is_ast_stmt_and_attr(misc_attrs->parent)) {
    ...
    if (!is_ast_declare_func_stmt(stmt) && !is_ast_declare_proc_stmt(stmt)) {
      report_error(CS, misc_attrs, "CQL0499: alias_of attribute may only be added to a declare function or declare proc statement", NULL);
      record_error(CS, misc_attrs);
      return;
    }
  }
  ...
  if (!is_ast_str(ast_misc_attr_values)) {
      report_error(CS, misc_attrs, "CQL0500: alias_of attribute must be a non-empty string argument.", NULL);
      record_error(CS, misc_attrs);
      return;
  }
}
...
static void sem_misc_attrs_callback(CqlState* _Nonnull CS,
  ...
  symtab_entry *entry = symtab_find(CS->sem.misc_attributes, misc_attr_name);
  if (entry) {
...
static void sem_misc_attrs(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  find_misc_attrs(CS, ast, sem_misc_attrs_callback, ast);
}
...
static void sem_create_proc_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, name_ast, "CQL0184: stored procedures cannot be nested", name);
    goto cleanup;
  }

  CS->sem.in_shared_fragment = is_proc_shared_fragment(CS, ast);

  Invariant(!CS->sem.locals);
  Invariant(!CS->sem.local_types);

  // create local storage for named type defined in the proc
  CS->sem.local_types = symtab_new();

  CS->sem.unitary_locals = _ast_pool_new(bytebuf);
  bytebuf_open(CS->sem.unitary_locals);

  // CREATE PROC [name] ( [params] )

  if (find_func(CS, name)) {
    report_error(CS, name_ast, "CQL0185: proc name conflicts with func name", name);
    goto cleanup;
  }

  if (find_interface_type(CS, name)) {
    report_error(CS, name_ast, "CQL0481: proc name conflicts with interface name", name);
    goto cleanup;
  }

  ast_node *existing_proc = find_proc(CS, name);

  if (is_ast_create_proc_stmt(existing_proc)) {
    report_error(CS, name_ast, "CQL0186: duplicate stored proc name", name);
    goto cleanup;
  }

  if (!existing_proc) {
    // proc doesn't exist, ok to add it
    bool_t added = add_proc(CS, ast, name);
    Invariant(added);
  }
  else {
  ...
  ast_node *schema_attr = find_upgrade_proc(CS, name);
  if (schema_attr) {
    ...
    if (vers != CS->sem.schema_upgrade_version) {
      CSTR msg = dup_printf(CS, "CQL0187: @schema_upgrade_version not declared or doesn't match upgrade version %d for proc", vers);
      report_error(CS, name_ast, msg, name);
      goto cleanup;
    }

    if (params) {
      report_error(CS, params, "CQL0233: procedure previously declared as schema upgrade proc, it can have no args", name);
      goto cleanup;
    }
  }
  ...
  sem_inside_create_proc_stmt(CS, ast);
  if (is_error(ast)) {
    goto cleanup;
  }

  if (schema_attr && !CS->sem.has_dml) {
    report_error(CS, name_ast, "CQL0188: procedure is supposed to do schema migration but it doesn't have any DML", name);
    goto cleanup;
  }

  if (is_struct(ast->sem->sem_type)) {
    sem_validate_unique_names_struct_type(CS, ast);
    if (is_error(ast)) {
      goto cleanup;
    }
  }
  ...
  if (existing_proc) {
    ...
    bool_t matching = sem_validate_identical_procs(CS, existing_proc, ast);
    if (!matching) {
      report_error(CS, ast, "CQL0189: procedure declarations/definitions do not match", name);
      goto cleanup;
    }
  }
  ...
  if (misc_attrs && stmt_list) {
    ...
    bool_t out_union_proc = has_out_union_stmt_result(CS->sem.current_proc);
    ...
    CS->sem.annotation_target = "vault_sensitive";
    if (exists_attribute_str(CS, misc_attrs, CS->sem.annotation_target)) {
      encode_info info;
      info.encode_columns = NULL;
      info.encode_context_column = NULL;
      info.misc_attrs = misc_attrs;
    ...
    if (exists_attribute_str(CS, misc_attrs, "vault_sensitive") && !CS->sem.has_dml) {
      report_error(CS, misc_attrs, "CQL0364: vault_sensitive annotation can only go on a procedure that uses the database", NULL);
      record_error(CS, misc_attrs);
      goto cleanup;
    }
    ...
    uint32_t identity_count = sem_column_name_annotation(CS, misc_attrs, find_identity_columns, "procedure identity");
    if (is_error(misc_attrs)) {
      goto cleanup;
    }

    if (identity_count && !result_set_proc && !out_stmt_proc && !out_union_proc) {
      report_error(CS, misc_attrs, "CQL0240: identity annotation can only go on a procedure that returns a result set", name);
      record_error(CS, misc_attrs);
      goto cleanup;
    }

    if (is_proc_shared_fragment(CS, ast)) {
      sem_shared_fragment(CS, misc_attrs, ast);
      if (is_error(ast)) {
        goto cleanup;
      }
    }

    sem_autotests(CS, misc_attrs);
    if (is_error(misc_attrs)) {
      goto cleanup;
    }

    find_attribute_str(CS, misc_attrs, sem_validate_one_interface, ast, "implements");
    if (is_error(ast)) {
      record_error(CS, misc_attrs);
      goto cleanup;
    }

    uint32_t autodrop_count = sem_autodrops(CS, misc_attrs);
    if (is_error(misc_attrs)) {
      goto cleanup;
    }
    ...
    if (autodrop_count) {
      if (!result_set_proc && !out_stmt_proc) {
        ...
        report_error(CS, misc_attrs, "CQL0234: autodrop annotation can only go on a procedure that returns a result set", name);
        record_error(CS, misc_attrs);
        goto cleanup;
      }

      if (!CS->sem.has_dml) {
        report_error(CS, misc_attrs, "CQL0236: autodrop annotation can only go on a procedure that uses the database", name);
        record_error(CS, misc_attrs);
        goto cleanup;
      }
    }
...
cleanup:
  ...
  SYMTAB_CLEANUP(CS->sem.local_types);
  CS->sem.in_shared_fragment = false;
  BYTEBUF_CLEANUP(CS->sem.unitary_locals);
}
...
static void sem_typed_name(CqlState* _Nonnull CS, ast_node *typed_name, symtab *names) {
  ...
  if (!symtab_add(CS, names, name, typed_name)) {
    report_error(CS, name_ast, "CQL0190: duplicate column name", name);
    record_error(CS, typed_name);
    return;
  }

  sem_data_type_var(CS, typed_name->right);
...
static void sem_typed_names(CqlState* _Nonnull CS, ast_node *head) {
  ...
  for (ast_node *ast = head; ast; ast = ast->right, count++) {
    ...
    sem_typed_name(CS, typed_name, names);
  ...
  head->sem = new_sem(CS, SEM_TYPE_STRUCT);
  sem_struct *sptr = new_sem_struct(CS, "select", count);
  head->sem->sptr = sptr;
...
static void sem_declare_func_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, name_ast, "CQL0191: declared functions must be top level", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (find_proc(CS, name)) {
    report_error(CS, name_ast, "CQL0192: func name conflicts with proc name", name);
    record_error(CS, ast);
    return;
  }

  ast_node *existing_func = find_func(CS, name);
  ast_node *existing_unchecked_func = find_unchecked_func(CS, name);
  ...
  {
      report_error(CS, ast, "CQL0486: function cannot be both a normal function and an unchecked function", name);
      record_error(CS, ast);
      return;
  }
  ...
  if (no_check) {
    if (!existing_unchecked_func) {
      bool_t added = add_unchecked_func(CS, ast, name);
      Invariant(added);
    }
  } else {
    if (!existing_func) {
      bool_t added = add_func(CS, ast, name);
      Invariant(added);
    }
  }
    ...
    if (select_func) {
      for (ast_node *item = params; item; item = item->right) {
        ...
        if (is_out_parameter(sem_type)) {
          report_error(CS, param, "CQL0475: select functions cannot have out parameters", param->sem->name);
  ...
  if (is_ast_typed_names(ret_data_type)) {
    sem_typed_names(CS, ret_data_type);
  }
  else {
    sem_data_type_var(CS, ret_data_type);
  ...
  if (matching_func) {
    bool_t matching = sem_validate_identical_funcs(CS, matching_func, ast);
    if (!matching) {
      report_error(CS, name_ast, "CQL0193: duplicate function name", name);
      record_error(CS, ast);
    }
  }

  if (will_add_current_entity(CS)) {
    add_item_to_list(CS, &CS->sem.all_functions_list, ast);
  }
}
...
static void sem_declare_select_func_stmt_common(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (symtab_find(CS->sem.builtin_funcs, name)) {
    report_error(CS, name_ast, "CQL0314: select function does not require a declaration, it is a CQL built-in", name);
    record_error(CS, ast);
    return;
  }

  if (core_type_of(ret_data_type->sem->sem_type) == SEM_TYPE_OBJECT) {
    report_error(CS, name_ast, "CQL0347: select function may not return type OBJECT", name);
    record_error(CS, ast);
    return;
  }

  sem_add_flags(CS, ast, SEM_TYPE_SELECT_FUNC);

  bool_t adding_current_entity = will_add_current_entity(CS);

  if (adding_current_entity) {
    add_item_to_list(CS, &CS->sem.all_select_functions_list, ast);
  }
}
...
static void sem_replace_seen_enum_values(CqlState* _Nonnull CS, ast_node *ast, symtab *names) {
  ...
  if (!is_ast_str(ast)) {
    if (ast_has_left(ast)) {
       sem_replace_seen_enum_values(CS, ast->left, names);
    }
    if (ast_has_right(ast)) {
       sem_replace_seen_enum_values(CS, ast->right, names);
    }
    return;
  }
...
static void sem_declare_enum_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  EXTRACT_NAME_AST(name_ast, typed_name->left);
  EXTRACT_STRING(name, name_ast);
  sem_data_type_column(CS, typed_name->right);
  typed_name->sem = typed_name->right->sem;
  ...
  if (CS->sem.current_proc) {
    report_error(CS, name_ast, "CQL0358: declared enums must be top level", name);
    record_error(CS, ast);
    return;
  }

  ast_node *existing_enum = find_enum(CS, name);
  ...
  while (enum_values) {
     ...
     if (!symtab_add(CS, names, enum_name, enum_value)) {
       report_error(CS, enum_value, "CQL0354: duplicate enum member", enum_name);
       record_error(CS, ast);
       goto cleanup;
     }

     if (expr) {
       sem_replace_seen_enum_values(CS, expr, names);
       sem_root_expr(CS, expr, SEM_EXPR_CONTEXT_NONE);
       eval(CS, expr, &result);

       if (result.sem_type == SEM_TYPE_ERROR || result.sem_type == SEM_TYPE_NULL) {
         report_error(CS, enum_value, "CQL0355: evaluation failed", enum_name);
         record_error(CS, ast);
         goto cleanup;
       }
     }
     ...
     enum_name_ast->sem = new_sem(CS, sem_type_enum);
  ...
  if (existing_enum) {
    bool_t matching = sem_validate_identical_ddl(CS, ast, existing_enum);
    if (!matching) {
      report_error(CS, ast, "CQL0356: enum definitions do not match", name);
      record_error(CS, ast);
      goto cleanup;
    }
    else {
...
static void sem_declare_group_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, name_ast, "CQL0462: group declared variables must be top level", name);
    record_error(CS, ast);
    return;
  }
  ...
  ast_node *existing_variable_group = find_variable_group(CS, name);

  if (existing_variable_group) {
    bool_t matching = sem_validate_identical_ddl(CS, ast, existing_variable_group);
    if (!matching) {
      report_error(CS, ast, "CQL0463: variable definitions do not match in group", name);
      record_error(CS, ast);
      return;
    }
    else {
  ...
  symtab_add(CS, CS->sem.variable_groups, name, ast);

  while (stmt_list) {
     EXTRACT_ANY_NOTNULL(stmt, stmt_list->left);

     sem_one_stmt(CS, stmt);
     if (is_error(stmt)) {
        record_error(CS, ast);
        return;
     }
...
static void sem_emit_group_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  while (name_list) {
    ...
    if (!find_variable_group(CS, name)) {
      report_error(CS, name_ast, "CQL0464: group not found", name);
      record_error(CS, ast);
      return;
    }
...
static void sem_declare_const_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, name_ast, "CQL0358: declared constants must be top level", name);
    record_error(CS, ast);
    return;
  }

  ast_node *existing_constant_group = find_constant_group(CS, name);

  if (!existing_constant_group) {
    symtab_add(CS, CS->sem.constant_groups, name, ast);
  }
  ...
  while (const_values) {
     ...
     if (!existing_constant_group) {
       if (!symtab_add(CS, CS->sem.constants, const_name, const_value)) {
         report_error(CS, const_value, "CQL0354: duplicate constant name", const_name);
         record_error(CS, ast);
         return;
       }
     }
     ...
     CSTR expr_text = dup_expr_text_buffer(CS, &tmp, expr);

     report_error(CS, expr, "CQL0177: global constants must be either constant numeric expressions or string literals", expr_text);
     record_error(CS, expr);
     record_error(CS, ast);
     CHARBUF_CLOSE(tmp);
     return;
  ...
  if (existing_constant_group) {
    bool_t matching = sem_validate_identical_ddl(CS, ast, existing_constant_group);
    if (!matching) {
      report_error(CS, ast, "CQL0356: const definitions do not match", name);
      record_error(CS, ast);
      return;
    }
    else {
    ...
    // so that it won't show up in JSON etc.
    if (adding_current_entity) {
      add_item_to_list(CS, &CS->sem.all_constant_groups_list, ast);
    }
  }
...
static void sem_declare_proc_no_check_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (find_proc(CS, name)) {
    report_error(CS, ast, "CQL0404: procedure cannot be both a normal procedure and an unchecked procedure", name);
    record_error(CS, ast);
    return;
  }

  if (find_interface_type(CS, name)) {
    report_error(CS, ast, "CQL0481: proc name conflicts with interface name", name);
    record_error(CS, ast);
    return;
  }

  // it can be added more than once, no need to check the return code
  add_unchecked_proc(CS, ast, name);
  record_ok(CS, ast);
}
...
static void sem_declare_proc_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, name_ast, "CQL0194: declared procedures must be top level", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (find_unchecked_proc(CS, name)) {
    report_error(CS, ast, "CQL0404: procedure cannot be both a normal procedure and an unchecked procedure", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (find_func(CS, name)) {
    report_error(CS, name_ast, "CQL0195: proc name conflicts with func name", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (type & PROC_FLAG_STRUCT_TYPE) {
    ...
    sem_typed_names(CS, typed_names);
  ...
  if (type & PROC_FLAG_USES_OUT) {
    sem_add_flags(CS, ast, SEM_TYPE_USES_OUT);
  }

  if (type & PROC_FLAG_USES_DML) {
    sem_add_flags(CS, ast, SEM_TYPE_DML_PROC);
  }

  if (type & PROC_FLAG_USES_OUT_UNION) {
    sem_add_flags(CS, ast, SEM_TYPE_USES_OUT_UNION);
  }
  ...
  if (existing_proc) {
    bool_t matching = sem_validate_identical_procs(CS, existing_proc, ast);
    if (!matching) {
      report_error(CS, ast, "CQL0196: procedure declarations/definitions do not match", name);
      record_error(CS, ast);
    }
    else {
      // this will let us find the duplicate later, it marks this as not authoritative
...
static void sem_declare_interface_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, name_ast, "CQL0480: declared interface must be top level", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (find_unchecked_proc(CS, name) || find_proc(CS, name)) {
    report_error(CS, ast, "CQL0478: interface name conflicts with procedure name", name);
    record_error(CS, ast);
    return;
  }

  if (find_func(CS, name)) {
    report_error(CS, name_ast, "CQL0477: interface name conflicts with func name", name);
    record_error(CS, ast);
    return;
  }

  ast_node *existing_interface = find_interface_type(CS, name);
  if (!existing_interface) {
    bool_t added = add_interface_type(CS, ast, name);
    Invariant(added);
  }
  ...
  if (existing_interface) {
    bool_t matching = sem_validate_identical_text(CS, existing_interface, ast, gen_one_stmt, NULL);

    if (!matching) {
      report_error(CS, ast, "CQL0479: interface declarations do not match", name);
      record_error(CS, ast);
    }
    else {
      // this will let us find the duplicate later, it marks this as not authoritative
...
static bool_t sem_verify_legal_variable_name(CqlState* _Nonnull CS, ast_node *variable, CSTR name) {
  ...
  if (CS->sem.current_loop_analysis_state != LOOP_ANALYSIS_STATE_REANALYZE) {
    if (symtab_find(CS->sem.current_variables, name)) {
      report_error(CS, variable, "CQL0197: duplicate variable name in the same scope", name);
      return false;
    }
  }

  // global variables can't conflict with table names, not even deleted table names
  if (CS->sem.current_variables == CS->sem.globals && find_table_or_view_even_deleted(CS, name)) {
    report_error(CS, variable, "CQL0198: global variable hides table/view name", name);
    return false;
  }
...
static void sem_declare_vars_type(CqlState* _Nonnull CS, ast_node *declare_vars_type) {
  ...
  sem_data_type_var(CS, data_type);
  if (is_error(data_type)) {
    record_error(CS, declare_vars_type);
    return;
  }
  sem_t sem_type = data_type->sem->sem_type;
  ...
  for (ast_node *ast = name_list; ast; ast = ast->right) {
    ...
    if (!sem_verify_legal_variable_name(CS, variable, name)) {
      record_error(CS, variable);
      record_error(CS, ast);
      error = true;
      continue;
    }

    variable->sem = ast->sem = new_sem(CS, sem_type | SEM_TYPE_VARIABLE);
  ...
  if (error) {
    ...
    declare_vars_type->sem = new_sem(CS, sem_type);
...
static void sem_declare_cursor(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_select_stmt(ast->right)) {
    ...
    sem_select_rewrite_backing(CS, select_stmt);
    if (is_error(select_stmt)) {
      record_error(CS, ast);
      return;
    }
    ...
    sem_call_stmt_opt_cursor(CS, call_stmt, name);
    if (is_error(call_stmt)) {
      record_error(CS, ast);
      return;
    }
    ...
    if (!is_struct(call_stmt->sem->sem_type)) {
      report_error(CS, call_stmt, "CQL0199: cursor requires a procedure that returns a result set via select", name);
      record_error(CS, ast);
      return;
    }

    if (has_out_stmt_result(call_stmt)) {
      report_error(CS, ast, "CQL0270: use FETCH FROM for procedures that returns a cursor with OUT", name);
      record_error(CS, ast);
      return;
    }
    ...
    sem_declare_cursor_for_expr(CS, ast);
    return;
  }

  if (!sem_verify_legal_variable_name(CS, ast, name)) {
    record_error(CS, ast->left);
    record_error(CS, ast);
    return;
  }
  ...
  cursor->sem = new_sem(CS, SEM_TYPE_STRUCT | SEM_TYPE_VARIABLE | out_union_and_dml);
...
static void sem_declare_cursor_for_expr(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // the indicated type must be a valid shape name (one we could use in LIKE T)
  ast_node *like_target = sem_find_likeable_from_expr_type(CS, expr);
  if (!like_target) {
    record_error(CS, ast);
    return;
  }
  ...
  // SEM_TYPE_STRUCT | SEM_TYPE_VARIABLE <=> it's a cursor
  cursor->sem = new_sem(CS, SEM_TYPE_STRUCT | SEM_TYPE_VARIABLE | cursor_flags);
...
cql_noexport ast_node *sem_find_likeable_from_expr_type(CqlState* _Nonnull CS, ast_node *expr) {
  ...
  if (!is_object(expr->sem->sem_type) || !expr->sem->kind) {
    ...
    report_error(CS, expr, "CQL0346: expression must be of type object<T cursor> where T is a valid shape name", expr_text);
    return NULL;
  }
  ...
  if (!len_tail) {
    ...
    if (!len_tail) {
      CSTR expr_text = dup_expr_text(CS, expr);
      report_error(CS, expr, "CQL0343: variable must be of type object<T CURSOR> or object<T SET> where T is a valid shape name", expr_text);
      return NULL;
    }
  }
...
static void sem_set_from_cursor(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast_node *var = find_local_or_global_variable(CS, var_name);

  if (!var) {
    report_error(CS, ast, "CQL0173: variable not found", var_name);
    record_error(CS, ast);
    return;
  }
  ...
  // the indicated type must be a valid shape name (one we could use in LIKE T)
  ast_node *like_target = sem_find_likeable_from_expr_type(CS, var);
  ...
  if (cursor->sem->sem_type & SEM_TYPE_VALUE_CURSOR) {
    report_error(CS, cursor,
       "CQL0261: cursor did not originate from a SQLite statement, it only has values", cursor->sem->name);
    record_error(CS, ast);
    return;
  }

  sem_verify_identical_columns(CS, like_target, cursor, "in the cursor and the variable type");
  if (is_error(cursor)) {
    record_error(CS, ast);
    return;
  }
  ...
  ast_node *cursor_var = find_local_or_global_variable(CS, cursor->sem->name);
  Invariant(cursor_var);
  Invariant(is_cursor(cursor_var->sem->sem_type));
  sem_add_flags(CS, cursor_var, SEM_TYPE_BOXED);
...
static void sem_declare_cursor_like_name(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!sem_verify_legal_variable_name(CS, ast, new_cursor_name)) {
    record_error(CS, new_cursor_ast);
    record_error(CS, ast);
    return;
  }
  ...
  new_cursor_ast->sem = new_sem(CS, SEM_TYPE_STRUCT | SEM_TYPE_VARIABLE | SEM_TYPE_VALUE_CURSOR | SEM_TYPE_HAS_SHAPE_STORAGE);
  new_cursor_ast->sem->sptr = found_shape->sem->sptr;
  new_cursor_ast->sem->name = new_cursor_name;
  ast->sem = new_cursor_ast->sem;

  add_variable(CS, new_cursor_name, new_cursor_ast);
}
...
static void sem_declare_cursor_like_select(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // DECLARE [name] CURSOR LIKE [select_stmt]
  {
    ...
    bool_t has_dml_saved = CS->sem.has_dml;
    sem_select(CS, select_stmt);
    CS->sem.has_dml = has_dml_saved;
  }
  ...
  // SEM_TYPE_STRUCT | SEM_TYPE_VARIABLE <=> it's a cursor
  cursor->sem = new_sem(CS, SEM_TYPE_STRUCT | SEM_TYPE_VARIABLE | SEM_TYPE_VALUE_CURSOR | SEM_TYPE_HAS_SHAPE_STORAGE);
...
static void sem_call_stmt_has_out_stmt_result_for_cursor(CqlState* _Nonnull CS, ast_node *call_stmt, CSTR name) {
  ...
  if (!has_out_stmt_result(call_stmt)) {
    report_error(CS, call_stmt, "CQL0203: cursor requires a procedure that returns a cursor with OUT", name);
    record_error(CS, call_stmt);
    return;
  }
}
...
static void sem_declare_value_cursor(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_call_stmt_has_out_stmt_result_for_cursor(CS, call_stmt, name);
  if (is_error(call_stmt)) {
    record_error(CS, ast);
    return;
  }
...
static ast_node *sem_synthesize_current_locals(CqlState* _Nonnull CS) {
  ...
  AST_REWRITE_INFO_SET(CS->sem.current_proc->lineno, CS->sem.current_proc->filename);
  CSTR locals_name = "LOCALS";

  uint32_t usable_locals = CS->sem.unitary_locals->used / sizeof(ast_node *);
  ast_node **l_syms = (ast_node **)CS->sem.unitary_locals->ptr;

  sem_struct *sptr = new_sem_struct(CS, locals_name, usable_locals);

  ast_node *ast_locals = new_ast_str(CS, locals_name);
  ast_locals->sem = new_sem(CS, SEM_TYPE_STRUCT | SEM_TYPE_HAS_SHAPE_STORAGE);
...
cql_noexport void sem_any_shape(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!strcmp("ARGUMENTS", name)) {
    if (CS->sem.current_proc) {
      EXTRACT_STRING(proc_name, get_proc_name(CS, CS->sem.current_proc));
      report_error(CS, ast, "CQL0340: FROM ARGUMENTS used in a procedure with no arguments", proc_name);
    }
    else {
      report_error(CS, ast, "CQL0163: FROM ARGUMENTS construct is only valid inside a procedure", NULL);
    }
    record_error(CS, ast);
    return;
  }
...
cql_noexport void sem_cursor(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_resolve_id(CS, ast, name, NULL);
  if (is_error(ast)) {
    return;
  }

  if (!is_cursor(ast->sem->sem_type)) {
    report_error(CS, ast, "CQL0205: not a cursor", name);
    record_error(CS, ast);
    return;
  }
}
...
static void sem_switch_expr_list(CqlState* _Nonnull CS, ast_node *ast, sem_t core_type, bytebuf *case_data) {
  ...
  while (ast) {
    ...
    if (core_type_expr > core_type) {
      report_error(CS, expr, "CQL0382: type of a WHEN expression is bigger than the type of the SWITCH expression", NULL);
      record_error(CS, head);
      return;
    }
    ...
    if (result.sem_type == SEM_TYPE_ERROR) {
      report_error(CS, expr, "CQL0380: WHEN expression cannot be evaluated to a constant", NULL);
      record_error(CS, head);
      return;
    }
...
static void sem_check_all_values_condition(CqlState* _Nonnull CS, ast_node *expr, bytebuf *case_buffer) {
  ...
  if (!kind || !(enum_stmt = find_enum(CS, kind))) {
    report_error(CS, expr, "CQL0386: SWITCH ... ALL VALUES is used but the switch expression is not an enum type", NULL);
    record_error(CS, expr);
    goto cleanup;
  }
  ...
  while (i < case_count && i < enum_count) {
    if (case_vals[i].value < enum_vals[i].value) {
      CSTR errant = dup_printf(CS, "%lld", (llint_t)case_vals[i].value);
      report_error(CS, case_vals[i].source, "CQL0388: a value exists in the switch that is not present in the enum", errant);
      record_error(CS, expr);
      goto cleanup;
    }
    ...
    if (case_vals[i].value > enum_vals[i].value) {
      ...
      report_error(CS, expr, "CQL0387: a value exists in the enum that is not present in the switch", enum_member);
      record_error(CS, expr);
      goto cleanup;
    }
    i++;
...
static void sem_switch_cases(CqlState* _Nonnull CS, ast_node *ast, ast_node *expr, bool_t all_values) {
  ...
  while (ast) {
    ...
    if (connector->left) {
      ...
      sem_switch_expr_list(CS, expr_list, core_type, case_buffer);
      ...
      if (all_values) {
        report_error(CS, ast, "CQL0383: switch ... ALL VALUES is useless with an ELSE clause", NULL);
        record_error(CS, head);
        branch_error = true;
      }
    }
  ...
  if (stmt_lists == 0) {
    report_error(CS, head, "CQL0384: switch statement did not have any actual statements in it", NULL);
    record_error(CS, head);
    goto cleanup;
  }
  ...
  for (int32_t i = 0; i < case_count - 1; i++) {
    if (case_vals[i].value == case_vals[i+1].value) {
      CSTR duplicate = dup_printf(CS, "%lld", (long long)case_vals[i].value);
      report_error(CS, case_vals[i].source, "CQL0385: WHEN clauses contain duplicate values", duplicate);
      record_error(CS, head);
      goto cleanup;
    }
  }
  ...
  if (all_values) {
    sem_check_all_values_condition(CS, expr, case_buffer);
    if (is_error(expr)) {
      record_error(CS, head);
      goto cleanup;
    }
    ...
    flow_set_context_branch_group_covers_all_cases(CS, has_else);
  }
...
static void sem_switch_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  bool_t in_switch_saved = CS->sem.in_switch;
  CS->sem.in_switch = true;
  ...
  if (!is_any_int(core_type) || is_nullable(expr->sem->sem_type)) {
    report_error(CS, expr, "CQL0381: case expression must be a not-null integral type", NULL);
    record_error(CS, ast);
    goto cleanup;
  }

  sem_switch_cases(CS, switch_case, expr, !!all_values);
...
cleanup:
  CS->sem.in_switch = in_switch_saved;
}
...
static void sem_while_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_numeric_expr(CS, expr, ast, "WHILE", SEM_EXPR_CONTEXT_NONE);
  ...
  if (stmt_list) {
    ...
    sem_stmt_list_within_loop(CS, stmt_list, expr);
...
static void sem_loop_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (stmt_list) {
    ...
    sem_stmt_list_within_loop(CS, stmt_list, condition);
...
static bool_t sem_verify_no_duplicate_names_func(CqlState* _Nonnull CS, ast_node *list, name_func func) {
  ...
  for (ast_node *a1 =list; a1; a1 = a1->right) {
    ...
    for (ast_node *a2 = a1->right; a2; a2 = a2->right) {
      ...
      if (!Strcasecmp(n1, n2)) {
        report_error(CS, a2->left, "CQL0206: duplicate name in list", n2);
        return false;
      }
    }
  }
...
static void sem_validate_arg_is_name_of_existing_variable(CqlState* _Nonnull CS, ast_node *arg, CSTR param_name) {
  ...
  sem_t *type = NULL;
  sem_resolve_id_with_type(CS, arg, name, scope, &type);
  if (is_error(arg)) {
    return;
  }
...
error:
  report_error(CS, arg, "CQL0207: expected a variable name for OUT or INOUT argument", param_name);
  record_error(CS, arg);
}
...
static void sem_arg_for_out_param(CqlState* _Nonnull CS, ast_node *arg, ast_node *param) {
  ...
  sem_validate_arg_is_name_of_existing_variable(CS, arg, param->sem->name);
  if (is_error(arg)) {
    return;
  }
  ...
  if (is_in_parameter(param->sem->sem_type)) {
    ...
    sem_validate_variable_referenced_is_initialized_if_required(CS, arg);
    if (is_error(arg)) {
      return;
    }
  }
  ...
  // this.
  sem_unset_notnull_improved(CS, name, scope);
  ...
  sem_set_initialization_improved(CS, name, scope);

  ast_node *variable = find_local_or_global_variable(CS, arg->sem->name);
  if (variable) {
    variable->sem->sem_type |= SEM_TYPE_WAS_SET;
  }
}
...
static bool_t sem_validate_arg_vs_formal(CqlState* _Nonnull CS, ast_node *arg, ast_node *param) {
  ...
  if (is_cursor_formal(sem_type_param)) {
    ...
    if (!is_auto_cursor(arg->sem->sem_type)) {
      report_error(CS, arg, "CQL0067: cursor was not used with 'fetch [cursor]'", arg->sem->name);
      record_error(CS, arg);
      return false;
    }

    ast_node *var = find_local_or_global_variable(CS, arg->sem->name);
    Invariant(var); // we know the cursor exists and is unique already
    var->sem->sem_type |= SEM_TYPE_SERIALIZE;
    return true;
  } else if (is_out_parameter(sem_type_param)) {
    sem_arg_for_out_param(CS, arg, param);
    if (is_error(arg)) {
      return false;
    }
  } else {
    // In the case of an IN-only parameter, we allow an expression.
    sem_arg_expr(CS, arg, false);
    if (is_error(arg)) {
      return false;
    }
  }
  ...
  if (is_in_parameter(sem_type_param)) {
    ...
    if (!sem_verify_assignment(CS, arg, sem_type_param, sem_type_arg, param->sem->name)) {
      return false;
    }
  }

  // the formal and the argument must match object types as well (if present)
  sem_combine_kinds(CS, arg, param->sem->kind);
  if (is_error(arg)) {
    return false;
  }
  ...
  if (is_out_parameter(sem_type_param)) {
    ...
    if (!sem_verify_assignment(CS, arg, sem_type_arg, sem_type_param, arg->sem->name)) {
      return false;
    }
    ...
    if (core_type_of(sem_type_param) != core_type_of(sem_type_arg)) {
      ...
      report_sem_type_mismatch(CS, sem_type_param, sem_type_arg, arg, error_message, arg->sem->name);
      return false;
    }
  ...
  for (uint32_t i = 1; i < ids_count; i++) {
    ...
    if (is_id_out_tagged(ids[i - 1]) || is_id_out_tagged(ids[i])) {
      ...
      if (!strcmp(previous_id_name, id_name)) {
        ...
        report_error(CS, previous_id, msg, id_name);
        return false;
      }
    }
  }
...
static void sem_validate_args_vs_formals(CqlState* _Nonnull CS, ast_node *ast, CSTR name, ast_node *arg_list, ast_node *params, bool_t proc_as_func) {
  ...
  for (; arg_item && param_item; arg_item = arg_item->right, param_item = param_item->right) {
    ...
    if (!sem_validate_arg_vs_formal(CS, arg, param)) {
      record_error(CS, ast);
      return;
    }
    ...
    if (proc_as_func && is_out_parameter(param->sem->sem_type)) {
      if (is_in_parameter(param->sem->sem_type)) {
        report_error(CS, ast, "CQL0424: procedure with INOUT parameter used as function", name);
        record_error(CS, ast);
        return;
      }
      if (param_item->right) {
        report_error(CS, ast, "CQL0425: procedure with non-trailing OUT parameter used as function", name);
        record_error(CS, ast);
        return;
      }
    }
  ...
  if (proc_as_func && !arg_item && param_item && !param_item->right) {
    ...
    if (!is_out_parameter(sem_type_param) || is_in_parameter(sem_type_param)) {
      report_error(CS, param, "CQL0211: procedure without trailing OUT parameter used as function", name);
      record_error(CS, ast);
      return;
    }

    ast->sem = new_sem(CS, core_type_of(sem_type_param) | not_nullable_flag(sem_type_param) | sensitive_flag(sem_type_param));
    ast->sem->kind = param->sem->kind;
    return;
  }

  if (param_item) {
    report_error(CS, ast, "CQL0212: too few arguments provided to procedure", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (arg_item || proc_as_func) {
    report_error(CS, ast, "CQL0235: too many arguments provided to procedure", name);
    record_error(CS, ast);
    return;
  }

  // Finally, we check whether or not any OUT or INOUT arguments are aliased.
  if (!sem_validate_out_args_are_unique(CS, arg_list, params)) {
    record_error(CS, ast);
    return;
  }
...
static void sem_call_stmt_opt_cursor(CqlState* _Nonnull CS, ast_node *ast, CSTR cursor_name) {
  ...
  if (!proc_stmt && !find_unchecked_proc(CS, name)) {
    report_error(CS, ast, "CQL0323: calls to undeclared procedures are forbidden; declaration missing or typo", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (proc_stmt && is_error(proc_stmt)) {
    report_error(CS, ast, "CQL0213: procedure had errors, can't call", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (proc_stmt &&
    ...
    is_proc_shared_fragment(CS, proc_stmt) &&
    !is_ast_shared_cte(ast->parent)) {
      report_error(CS, ast, "CQL0434: shared fragments may not be called outside of a SQL statement", name);
      record_error(CS, ast);
      return;
  }

  if (proc_stmt && is_struct(proc_stmt->sem->sem_type)) {
    if (!cursor_name && !CS->sem.current_proc && !CS->sem.in_shared_fragment_call) {
      report_error(CS, ast, "CQL0214: procedures with results can only be called using a cursor in global context", name);
      record_error(CS, ast);
      return;
    }
  }
  ...
  if (proc_stmt) {
    ...
    CS->sem.has_dml |= is_dml_proc(proc_stmt->sem->sem_type);
    ...
    sem_validate_args(CS, ast, arg_list);
    if (is_error(ast)) {
      return;
    }
  }
...
static void sem_fetch_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (cursor->sem->sem_type & SEM_TYPE_VALUE_CURSOR) {
    report_error(CS, cursor, "CQL0215: value cursors are not used with FETCH C, or FETCH C INTO", cursor->sem->name);
    record_error(CS, ast);
    return;
  }

  if (!name_list) {
    sem_validate_unique_names_struct_type(CS, cursor);
    if (is_error(cursor)) {
      record_error(CS, ast);
      return;
    }

    sem_verify_no_anon_no_null_columns(CS, cursor);
    if (is_error(cursor)) {
      record_error(CS, ast);
      return;
    }
    ...
    Invariant(cursor_var);
    Invariant(is_cursor(cursor_var->sem->sem_type));
    sem_add_flags(CS, cursor_var, SEM_TYPE_HAS_SHAPE_STORAGE);
    ...
    sem_add_flags(CS, cursor, SEM_TYPE_HAS_SHAPE_STORAGE);
    ast->sem = cursor->sem;
    ...
    sem_unset_has_row_improved(CS, cursor->sem->name);
  ...
  for (item = name_list; item && icol < cols; item = item->right, icol++) {
    ...
    ast_node *variable = find_local_or_global_variable(CS, name);
    if (!variable) {
      report_error(CS, var_name_ast, "CQL0216: FETCH variable not found", name);
      record_error(CS, ast);
      return;
    }
    ...
    if (!sem_verify_assignment(CS, var_name_ast, sem_type_variable, sem_type_cursor, name)) {
      record_error(CS, ast);
      return;
    }
  ...
  if (icol != cols || item) {
    report_error(CS, ast, "CQL0217: number of variables did not match count of columns in cursor", cursor->sem->name);
    record_error(CS, ast);
    return;
  }
  ...
  Invariant(cursor_var);
  Invariant(is_cursor(cursor_var->sem->sem_type));
  sem_add_flags(CS, cursor_var, SEM_TYPE_FETCH_INTO);
...
static void sem_fetch_call_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_call_stmt_opt_cursor(CS, call_stmt, cursor_name);
  if (is_error(call_stmt)) {
    record_error(CS, ast);
    return;
  }

  sem_call_stmt_has_out_stmt_result_for_cursor(CS, call_stmt, cursor_name);
  if (is_error(call_stmt)) {
    record_error(CS, ast);
    return;
  }
  ...
  sem_verify_identical_columns(CS, cursor, call_stmt, "receiving cursor from call");
  if (is_error(call_stmt)) {
    record_error(CS, ast);
    return;
  }
  ...
  sem_unset_has_row_improved(CS, cursor_name);
...
static void sem_continue_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // CONTINUE
  if (CS->sem.loop_depth == 0) {
    report_error(CS, ast, "CQL0218: continue must be inside of a 'loop' or 'while' statement", NULL);
    record_error(CS, ast);
    return;
  }
...
static void sem_leave_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // LEAVE
  if (CS->sem.loop_depth == 0 && !CS->sem.in_switch) {
    report_error(CS, ast, "CQL0219: leave must be inside of a 'loop', 'while', or 'switch' statement", NULL);
    record_error(CS, ast);
    return;
  }
...
static void sem_return_common(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  // RETURN
  if (CS->sem.sem_stmt_level <= 1) {
    report_error(CS, ast, "CQL0307: return statement should be in a procedure and not at the top level", NULL);
    record_error(CS, ast);
    return;
  }
...
static void sem_commit_return_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!CS->sem.in_proc_savepoint) {
    report_error(CS, ast, "CQL0350: statement must appear inside of a PROC SAVEPOINT block", NULL);
    record_error(CS, ast);
    return;
  }
...
static void sem_proc_savepoint_stmt(CqlState* _Nonnull CS, ast_node *ast)
{
  ...
  if (!CS->sem.current_proc || CS->sem.sem_stmt_level != 1 ) {
    report_error(CS, ast, "CQL0351: statement should be in a procedure and at the top level", NULL);
    record_error(CS, ast);
    return;
  }

  Invariant(!CS->sem.in_proc_savepoint);

  if (stmt_list) {
   CS->sem.in_proc_savepoint = true;
  ...
  void *_Nullable context)
{
  ...
  if (ast_misc_attr_value_list) {
    report_error(CS, ast_misc_attr_value_list, "CQL0445: @attribute(cql:try_is_proc_body) accepts no values", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  CS->sem.current_proc_contains_try_is_proc_body = true;
  ...
  if (!sem_validate_current_proc_params_are_initialized(CS, try_list ? try_list : ast)) {
    record_error(CS, ast);
    return;
  }
...
static void sem_trycatch_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (try_list) {
    sem_stmt_list_in_current_flow_context(CS, try_list);
  ...
  if (!error) {
    ...
    if (misc_attrs) {
      ...
      find_misc_attrs(CS, misc_attrs, sem_find_ast_misc_attr_trycatch_is_proc_body_callback, ast);
      error = is_error(ast);
    }
  }
...
static void sem_rollback_trans_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  EXTRACT_STRING(name, ast->left);
  if (!symtab_find(CS->sem.savepoints, name)) {
    report_error(CS, ast, "CQL0220: savepoint has not been mentioned yet, probably wrong", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_savepoint_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  symtab_add(CS, CS->sem.savepoints, name, ast);  // if already exits, no problem.
...
static void sem_release_savepoint_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!symtab_find(CS->sem.savepoints, name)) {
    report_error(CS, ast, "CQL0221: savepoint has not been mentioned yet, probably wrong", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_close_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (cursor->sem->sem_type & SEM_TYPE_BOXED) {
    EXTRACT_STRING(name, cursor);
    report_error(CS, ast, "CQL0391: CLOSE cannot be used on a boxed cursor", name);
    record_error(CS, ast);
    return;
  }
...
static void sem_out_any(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!CS->sem.current_proc) {
    report_error(CS, ast, "CQL0222: out cursor statement only makes sense inside of a procedure", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  if (!is_auto_cursor(cursor->sem->sem_type)) {
    report_error(CS, ast, "CQL0223: cursor was not fetched with the auto-fetch syntax 'fetch [cursor]'", cursor->sem->name);
    record_error(CS, ast);
    return;
  }
...
static void sem_previous_schema_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, ast, "CQL0225: switching to previous schema validation mode must be outside of any proc", NULL);
    record_error(CS, ast);
    return;
  }

  if (CS->sem.schema_upgrade_version > 0) {
    report_error(CS, ast, "CQL0254: switching to previous schema validation mode not allowed if @schema_upgrade_version was used", NULL);
    record_error(CS, ast);
    return;
  }

  CS->sem.validating_previous_schema = true;
  reset_enforcements(CS);
  CS->sem.enforcement.strict_cast = false;  // this is normally on by default, we want no strict in previous schema
  ...
  // for regions.
  CS->sem.new_regions = CS->sem.schema_regions;
  CS->sem.new_enums = CS->sem.enums;

  // this is all it takes to start fresh...
  CS->sem.schema_regions = symtab_new();
  CS->sem.enums = symtab_new();

  CS->sem.deployable_validations = _ast_pool_new(bytebuf);
  bytebuf_open(CS->sem.deployable_validations);

  reverse_list(&CS->sem.all_subscriptions_list);
  CS->sem.next_subscription = CS->sem.all_subscriptions_list;
  CS->sem.found_subscription_error = false;
...
static void sem_schema_upgrade_script_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (CS->sem.current_proc) {
    report_error(CS, ast, "CQL0226: schema upgrade declaration must be outside of any proc", NULL);
    record_error(CS, ast);
    return;
  }

  if (CS->sem.tables->count) {
    report_error(CS, ast, "CQL0227: schema upgrade declaration must come before any tables are declared", NULL);
    record_error(CS, ast);
    return;
  }

  CS->sem.schema_upgrade_script = 1;
  record_ok(CS, ast);
}
...
static void sem_schema_upgrade_version_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (vers <= 0) {
    report_error(CS, ast, "CQL0228: schema upgrade version must be a positive integer", NULL);
    record_error(CS, ast);
    return;
  }

  if (CS->sem.schema_upgrade_version > 0) {
    report_error(CS, ast, "CQL0229: schema upgrade version declaration may only appear once", NULL);
    record_error(CS, ast);
    return;
  }

  if (CS->sem.current_proc) {
    report_error(CS, ast, "CQL0230: schema upgrade version declaration must be outside of any proc", NULL);
    record_error(CS, ast);
    return;
  }

  if (CS->sem.tables->count) {
    report_error(CS, ast, "CQL0231: schema upgrade version declaration must come before any tables are declared", NULL);
    record_error(CS, ast);
    return;
  }

  CS->sem.schema_upgrade_version = vers;
...
static void sem_declare_out_call_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!proc_stmt) {
    report_error(CS, ast, "CQL0389: DECLARE OUT requires that the procedure be already declared", name);
    record_error(CS, ast);
    return;
  }
  ...
  for (; params && arg_list; params = params->right, arg_list = arg_list->right) {
    ...
    if (!is_id(arg)) {
      report_error(CS, arg, "CQL0207: expected a variable name for OUT or INOUT argument", param->sem->name);
      record_error(CS, ast);
      return;
    }
    ...
    if (arg->sem && arg->sem->sem_type & SEM_TYPE_IMPLICIT) {
      ...
      Invariant(CS->sem.current_loop_analysis_state == LOOP_ANALYSIS_STATE_REANALYZE);
      // We also must have already made a variable for this argument.
      symtab_entry *entry = symtab_find(CS->sem.current_variables, var_name);
      Invariant(entry);
    ...
    if (!symtab_find(CS->sem.current_variables, var_name)) {
      ...
      ast_node *variable = new_ast_str(CS, var_name);
      variable->sem = ast->sem = new_sem(CS, sem_type_var);
      variable->sem->name = var_name;
      variable->sem->kind = param->sem->kind;
      add_variable(CS, var_name, variable);
      AST_REWRITE_INFO_RESET();
    }
  }

  if (out_args == 0) {
    report_error(CS, name_ast, "CQL0390: DECLARE OUT CALL used on a procedure with no missing OUT arguments", name);
    record_error(CS, ast);
    return;
  }

  sem_call_stmt(CS, call_stmt);
  if (is_error(call_stmt)) {
    record_error(CS, ast);
    return;
  }
...
cql_noexport void sem_one_stmt(CqlState* _Nonnull CS, ast_node *stmt) {
  ...
  if (is_ast_stmt_and_attr(stmt->parent)) {
    ...
    sem_misc_attrs_basic(CS, misc_attrs);
    if (is_error(misc_attrs)) {
      record_error(CS, stmt_and_attr);
      record_error(CS, stmt);
      error = true;
    }
    else {
      sem_misc_attrs(CS, misc_attrs);
      if (is_error(misc_attrs)) {
        record_error(CS, stmt_and_attr);
        record_error(CS, stmt);
        error = true;
  ...
  if (!error) {
    symtab_entry *entry = symtab_find(CS->sem.non_sql_stmts, stmt->type);
    if (entry) {
      ...
      CS->sem.has_dml = 1;
      entry = symtab_find(CS->sem.sql_stmts, stmt->type);
  ...
  // of a cql statement.
  if (stmt_and_attr) {
    error ? record_error(CS, stmt_and_attr) : record_ok(CS, stmt_and_attr);
  }

  if (capture_now) {
    cql_attach_captured_errors(CS, stmt);
    CS->sem.error_capture = NULL;
  }

  if (is_control_stmt(stmt)) {
    flow_set_context_always_jumps(CS, true);
  }
...
static void sem_misc_attr_value(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (is_ast_misc_attr_value_list(ast)) {
    for (ast_node *item = ast; item; item = item->right) {
      sem_misc_attr_value(CS, item->left);
      if (is_error(item->left)) {
        record_error(CS, item);
        record_error(CS, ast);
        return;
      }
    }
    ...
    sem_root_expr(CS, ast, SEM_EXPR_CONTEXT_NONE);
    if (is_error(ast)) {
...
static void sem_misc_attrs_basic(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  while (ast) {
    ...
    if (misc_attr_value) {
      sem_misc_attr_value(CS, misc_attr_value);
...
static void sem_stmt_list(CqlState* _Nonnull CS, ast_node *head) {
  ...
  sem_stmt_list_in_current_flow_context(CS, head);
...
recurse:
  switch (CS->sem.current_loop_analysis_state) {
    case LOOP_ANALYSIS_STATE_NONE:
      is_top_level_loop = true;
      CS->sem.current_loop_analysis_state = LOOP_ANALYSIS_STATE_ANALYZE;
    ...
    case LOOP_ANALYSIS_STATE_ANALYZE: {
      ...
      FLOW_PUSH_CONTEXT_JUMP();
      if (true_expr) {
        sem_set_improvements_for_true_condition(CS, true_expr);
      }
      sem_stmt_list_in_current_flow_context(CS, stmt_list);
      FLOW_POP_CONTEXT_JUMP();
      if (is_error(stmt_list)) {
      ...
      if (is_top_level_loop) {
        CS->sem.current_loop_analysis_state = LOOP_ANALYSIS_STATE_REANALYZE;
        goto recurse;
      }
      break;
    }
...
cleanup:
  CS->sem.current_loop_analysis_state = saved_loop_analysis_state;
}
...
static void sem_expr_proclit(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  CSTR name = process_proclit(CS, ast, "@proc");
  if (!name) {
    return;
  }

  // replace with a standard string literal
  CSTR strlit = dup_printf(CS, "'%s'", name);
  ((str_ast_node*)ast)->value = strlit;

  ast->sem = new_sem(CS, SEM_TYPE_TEXT | SEM_TYPE_NOTNULL);
}
...
static void sem_expr_at_rc(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast->sem->name = "@rc";
  CS->sem.has_dml = 1; // use of result code implies DML proc
}
...
static void sem_expr_num(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  switch (num_type) {
  case NUM_BOOL:
    ast->sem = new_sem(CS, SEM_TYPE_BOOL | SEM_TYPE_NOTNULL);
    break;

  case NUM_INT:
    ast->sem = new_sem(CS, SEM_TYPE_INTEGER | SEM_TYPE_NOTNULL);
    break;

  case NUM_LONG:
    ast->sem = new_sem(CS, SEM_TYPE_LONG_INTEGER | SEM_TYPE_NOTNULL);
    break;

  default:
    // this is all that's left
    Contract(num_type == NUM_REAL);
    ast->sem = new_sem(CS, SEM_TYPE_REAL | SEM_TYPE_NOTNULL);
    break;
  }
}
...
static void sem_expr_blob(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (CURRENT_EXPR_CONTEXT_IS(SEM_EXPR_CONTEXT_NONE)) {
    report_error(CS, ast, "CQL0313: blob literals may only appear in the context of a SQL statement", NULL);
    record_error(CS, ast);
    return;
  }

  // blob literal
  ast->sem = new_sem(CS, SEM_TYPE_BLOB | SEM_TYPE_NOTNULL);
}
...
static void sem_expr_str(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (is_strlit(ast)) {
    ...
    sem_expr_proclit(CS, ast);
  }
  else if (is_at_rc(ast)) {
    sem_expr_at_rc(CS, ast);
  }
  else {
    sem_resolve_id_expr(CS, ast, str, NULL);
  }
}
...
static void sem_validate_dot_transform(CqlState* _Nonnull CS, ast_node *ast, CSTR choice1, CSTR choice2) {
  ...
  if (!expr->sem->kind) {
    report_error(CS, expr, "CQL0470: operation is only available for types with a declared type kind like object<something>", ".");
    record_error(CS, expr);
    record_error(CS, ast);
    return;
  }
  ...
  CHARBUF_OPEN(sym);
  bool_t found = try_find_possible_dot_overload(CS, &sym, expr, name, choice1, choice2);
  if (found) {
    CSTR new_name = Strdup(CS, sym.ptr);
    rewrite_dot_as_call(CS, ast, new_name);
    record_ok(CS, ast);
  }
  else {
    EXTRACT_STRING(dot_name, ast->right);
    report_error(CS, ast, "CQL0069: name not found", dot_name);
    record_error(CS, ast);
  }
  CHARBUF_CLOSE(sym);
}
...
static void sem_expr_dot(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (is_id(expr)) {
    ...
    sem_resolve_id_expr(CS, ast, name, scope);
  ...
  sem_validate_dot_transform(CS, ast, "get", "get_from");
  if (!is_error(ast)) {
    sem_expr(CS, ast);
  }
}
...
static bool_t sem_select_expr_must_return_a_row(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (
    ...
    return sem_check_aggregate_select_expr_must_return_a_row(CS, ast, select_where);;
  }
...
static void sem_expr_select(CqlState* _Nonnull CS, ast_node *ast, CSTR cstr) {
  ...
  if (CS->sem.current_expr_context == SEM_EXPR_CONTEXT_CONSTRAINT) {
    report_error(CS, ast, "CQL0394: nested select expressions may not appear inside of a constraint expression", NULL);
    record_error(CS, ast);
    return;
  }

  // (select ...)
  sem_select_rewrite_backing(CS, ast);
  if (is_error(ast)) {
    return;
  }
  ...
  bool_t invalid_select  =
    CS->sem.enforcement.strict_if_nothing &&
    CS->sem.current_expr_context == SEM_EXPR_CONTEXT_NONE &&
  ...
  Invariant(sptr);
  if (sptr->count != 1) {
    report_error(CS, ast, "CQL0232: nested select expression must return exactly one column", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  else if (sem_select_expr_must_return_a_row(CS, ast)) {
  ...
  if (sem_type == SEM_TYPE_NULL) {
     report_error(CS, ast, "CQL0374: SELECT expression is equivalent to NULL", NULL);
     record_error(CS, ast);
     return;
  }
...
static void sem_expr_select_if_nothing(CqlState* _Nonnull CS, ast_node *ast, CSTR op) {
  ...
  if (!sem_verify_compat(CS, ast, ast->left->sem->sem_type, ast->right->sem->sem_type, op)) {
    return;
  }
...
static void sem_validate_all_tables_not_in_previous(CqlState* _Nonnull CS, ast_node *root) {
  ...
  for (list_item *item = CS->sem.all_tables_list; item; item = item->next) {
    ...
    if (!is_error(ast) && !is_backed(ast->sem->sem_type)) {
      ...
      sem_validate_old_object_or_marked_create(CS, root, ast, err_msg.ptr, name);
    }
  }
...
static void sem_validate_old_object_or_marked_create(CqlState* _Nonnull CS, ast_node *root, ast_node *ast, CSTR err_msg, CSTR name) {
  ...
  // then it's good.
  if (ast->sem->create_version >= CS->sem.max_previous_schema_version) {
    return;
  }
  ...
  report_and_capture_error(CS, root, ast, err_msg, name);
}
...
static void sem_validate_all_prev_recreate_tables(CqlState* _Nonnull CS, ast_node *root) {
  ...
  for (list_item *item = CS->sem.all_prev_recreate_tables; item; item = item->next) {
    ast_node *ast = item->ast;
    ...
    sem_validate_marked_create_or_delete(CS, root, ast, err_msg.ptr, name);
  }
...
static void sem_validate_all_columns_not_in_previous(CqlState* _Nonnull CS, ast_node *root) {
  ...
  for (list_item *item = CS->sem.created_columns; item; item = item->next) {
    ...
    sem_validate_old_object_or_marked_create(CS, root, def, err_msg.ptr, name);
  }
  CHARBUF_CLOSE(err_msg);
}
...
static void sem_enforcement_options(CqlState* _Nonnull CS, ast_node *ast, bool_t strict) {
  ...
  switch (option) {
    case ENFORCE_STRICT_JOIN:
      CS->sem.enforcement.strict_join = strict;
      break;

    case ENFORCE_FK_ON_UPDATE:
      CS->sem.enforcement.strict_fk_update = strict;
      break;

    case ENFORCE_UPSERT_STMT:
      CS->sem.enforcement.strict_upsert_stmt = strict;
      break;

    case ENFORCE_WINDOW_FUNC:
      CS->sem.enforcement.strict_window_func = strict;
      break;

    case ENFORCE_CAST:
      CS->sem.enforcement.strict_cast = strict;
      break;

    case ENFORCE_WITHOUT_ROWID:
      CS->sem.enforcement.strict_without_rowid = strict;
      break;

    case ENFORCE_TRANSACTION:
      CS->sem.enforcement.strict_transaction = strict;
      break;

    case ENFORCE_SELECT_IF_NOTHING:
      CS->sem.enforcement.strict_if_nothing = strict;
      break;

    case ENFORCE_INSERT_SELECT:
      CS->sem.enforcement.strict_insert_select = strict;
      break;

    case ENFORCE_TABLE_FUNCTION:
      CS->sem.enforcement.strict_table_function = strict;
      break;

    case ENFORCE_ENCODE_CONTEXT_COLUMN:
      CS->sem.enforcement.strict_encode_context = strict;
      break;

    case ENFORCE_ENCODE_CONTEXT_TYPE_INTEGER:
      CS->sem.enforcement.strict_encode_context_type = strict;
      CS->sem.encode_context_type = SEM_TYPE_INTEGER;
      break;

    case ENFORCE_ENCODE_CONTEXT_TYPE_LONG_INTEGER:
      CS->sem.enforcement.strict_encode_context_type = strict;
      CS->sem.encode_context_type = SEM_TYPE_LONG_INTEGER;
      break;

    case ENFORCE_ENCODE_CONTEXT_TYPE_REAL:
      CS->sem.enforcement.strict_encode_context_type = strict;
      CS->sem.encode_context_type = SEM_TYPE_REAL;
      break;

    case ENFORCE_ENCODE_CONTEXT_TYPE_BOOL:
      CS->sem.enforcement.strict_encode_context_type = strict;
      CS->sem.encode_context_type = SEM_TYPE_BOOL;
      break;

    case ENFORCE_ENCODE_CONTEXT_TYPE_TEXT:
      CS->sem.enforcement.strict_encode_context_type = strict;
      CS->sem.encode_context_type = SEM_TYPE_TEXT;
      break;

    case ENFORCE_ENCODE_CONTEXT_TYPE_BLOB:
      CS->sem.enforcement.strict_encode_context_type = strict;
      CS->sem.encode_context_type = SEM_TYPE_BLOB;
      break;

    case ENFORCE_IS_TRUE:
      CS->sem.enforcement.strict_is_true = strict;
      break;

    case ENFORCE_SIGN_FUNCTION:
      CS->sem.enforcement.strict_sign_function = strict;
      break;

    case ENFORCE_CURSOR_HAS_ROW:
      CS->sem.enforcement.strict_cursor_has_row = strict;
      break;

    case ENFORCE_UPDATE_FROM:
      CS->sem.enforcement.strict_update_from = strict;
      break;

    case ENFORCE_AND_OR_NOT_NULL_CHECK:
      CS->sem.enforcement.strict_and_or_not_null_check = strict;
      break;

    default:
      // this is all that's left
      Contract(option == ENFORCE_FK_ON_DELETE);
      CS->sem.enforcement.strict_fk_delete = strict;
      break;
  }
}
...
static void sem_enforce_push_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  item->options = CS->sem.enforcement;
  item->next = enforcement_stack_rv;
  enforcement_stack_lv = item;
  record_ok(CS, ast);
}
...
static void sem_enforce_pop_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!item) {
    report_error(CS, ast, "CQL0365: @enforce_pop used but there is nothing to pop", NULL);
    record_error(CS, ast);
    return;
  }

  CS->sem.enforcement = item->options;
  enforcement_stack_lv = item->next;
  record_ok(CS, ast);
}
...
static void sem_validate_region_links(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  for (ast_node *item = region_list; item; item = item->right) {
    ...
    ast_node *region = find_region(CS, item_name);
    ...
    if (!is_error(region) && region->sem->region && !(region->sem->sem_type & SEM_TYPE_DEPLOYABLE)) {
       ...
       bprintf(&msg, "CQL0291: region links into the middle of a deployable region;"
                     ...
                     region->sem->region);
       report_error(CS, ast, msg.ptr, name);
...
static void sem_declare_schema_region_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!verify_schema_region_out_of_proc(CS, ast)) {
    record_error(CS, ast);
    return;
  }
  ...
  if (region_list) {
    if (!sem_verify_no_duplicate_regions(CS, region_list)) {
      record_error(CS, ast);
      return;
    }
    ...
    for (ast_node *item = region_list; item; item = item->right) {
      ...
      EXTRACT_NOTNULL(region_spec, item->left);
      EXTRACT_STRING(item_name, region_spec->left);
      if (!find_region(CS, item_name)) {
        report_error(CS, item, "CQL0244: unknown schema region", item_name);
        record_error(CS, ast);
        return;
      }
    }
  ...
  if (!add_region(CS, ast, name)) {
    report_error(CS, ast, "CQL0245: schema region already defined", name);
    record_error(CS, ast);
    return;
  }
  ...
  if (adding_current_entity) {
    add_item_to_list(CS, &CS->sem.all_regions_list, ast);
  }

  sem_validate_region_links(CS, ast);
}
...
static void sem_mark_deployment_subgraph(CqlState* _Nonnull CS, CSTR current, CSTR owner) {
  ...
  for (ast_node *item =region_list; item; item = item->right) {
    ...
    EXTRACT_NOTNULL(region_spec, item->left);
    EXTRACT_STRING(item_name, region_spec->left);
    sem_mark_deployment_subgraph(CS, item_name, owner);
  }
}
...
static void sem_declare_deployable_region_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  sem_declare_schema_region_stmt(CS, ast);
  if (is_error(ast)) {
    return;
  }

  // I am the new owner of the entire subgraph
  sem_mark_deployment_subgraph(CS, ast->sem->name, ast->sem->name);
  ...
  for (list_item *item = CS->sem.all_regions_list; item; item = item->next) {
    ...
    if (!region_ast->sem->region) {
      ...
      sem_validate_region_links(CS, region_ast);
      if (is_error(region_ast)) {
...
static void sem_walk_regions(CqlState* _Nonnull CS, region_walk *acc, CSTR name) {
  ...
  // two different ways.
  if (!symtab_add(CS, acc->regions, name, NULL)) {
    return;
  }

  // Every name we encounter has already been validated!
  ast_node *region = find_region(CS, name);
  Invariant(region);
  ...
  for (ast_node *item = region_list; item; item = item->right) {
    ...
    if (name == acc->root_name || !acc->honor_private || !is_private) {
      sem_walk_regions(CS, acc, item_name);
    }
  }
}
...
static symtab *sem_accumulate_regions(CqlState* _Nonnull CS, uint32_t count, char **regions) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (!find_region(CS, region)) {
      cql_error(CS, "invalid region specified '%s'\n", region);
      symtab_delete(CS, result);
      cql_cleanup_and_exit(CS, 1);
    }

    sem_accumulate_full_region_image(CS, result, region);
  }
...
static void sem_setup_region_filters(CqlState* _Nonnull CS) {
  ...
  if (CS->options.include_regions_count) {
    CS->sem.included_regions = sem_accumulate_regions(CS,
                          CS->options.include_regions_count,
                          CS->options.include_regions);
  }

  if (CS->options.exclude_regions_count) {
    CS->sem.excluded_regions = sem_accumulate_regions(CS,
                          CS->options.exclude_regions_count,
                          CS->options.exclude_regions);
  }
}
...
static void sem_begin_schema_region_stmt(CqlState* _Nonnull CS, ast_node * ast) {
  ...
  if (CS->sem.current_region) {
    report_error(CS, ast, "CQL0246: schema regions do not nest; end the current region before starting a new one", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  ast_node *region = find_region(CS, name);
  if (!region) {
    report_error(CS, ast->left, "CQL0244: unknown schema region", name);
    record_error(CS, ast);
    return;
  }
  ...
  Invariant(!CS->sem.current_region_image);
  CS->sem.current_region_image = symtab_new();
  sem_accumulate_public_region_image(CS, CS->sem.current_region_image, region_name);

  // this is the one and only text pointer value for this region
  CS->sem.current_region = region_name;
  record_ok(CS, ast);
}
...
static void sem_end_schema_region_stmt(CqlState* _Nonnull CS, ast_node * ast) {
  ...
  if (!CS->sem.current_region) {
    report_error(CS, ast, "CQL0247: you must begin a schema region before you can end one", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  Invariant(CS->sem.current_region_image);
  symtab_delete(CS, CS->sem.current_region_image);
  CS->sem.current_region_image = NULL;
  CS->sem.current_region = NULL;
  record_ok(CS, ast);
}
...
static void sem_validate_previous_ad_hoc(CqlState* _Nonnull CS, ast_node *prev, CSTR name, int32_t version) {
  ...
  ast_node *ast = find_ad_hoc_migrate(CS, name);
  if (!ast) {
    report_error(CS, prev, "CQL0286: ad hoc schema migration directive was removed; this is not allowed", name);
    record_error(CS, prev);
    return;
  }
  ...
  if (vers != version) {
    report_error(CS, ast, "CQL0285: ad hoc schema migration directive version number changed", name);
    record_error(CS, ast);
    record_error(CS, prev);
    return;
  }

  enqueue_pending_region_validation(CS, prev, ast, name);
...
static void sem_schema_unsub_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  ast_node *target = find_usable_table_or_view_even_deleted(CS,
  ...
  if (target->sem->delete_version > 0) {
    report_error(CS, ast, "CQL0469: table/view is already deleted", name);
    record_error(CS, ast);
    return;
  }

  if (target->sem->unsubscribed) {
    report_error(CS, ast, "CQL0472: table/view is already unsubscribed", name);
    record_error(CS, ast);
    return;
  }

  if (is_ast_create_table_stmt(target) && is_table_not_physical(CS, target)) {
    report_error(CS, ast, "CQL0449: unsubscribe does not make sense on non-physical tables", name);
    record_error(CS, ast);
    return;
  }

  bytebuf *buf = symtab_ensure_bytebuf(CS, CS->sem.ref_sources_for_target_table, name);
  ...
  for (uint32_t i = 0; i < ref_count; i++) {
    ...
    if (!is_deleted(src_ast)) {
      ...
      if (Strcasecmp(name, src_name)) {
        report_error(CS, ast, "CQL0473: @unsub is invalid because the table/view is still used by", src_name);
        record_error(CS, ast);
      }
    }
  }
  ...
  add_item_to_list(CS, &CS->sem.all_subscriptions_list, ast);
  ...
  if (is_table && !is_recreate) {
    record_schema_annotation(CS, 1, target, name, SCHEMA_ANNOTATION_UNSUB, NULL, version_annotation, 0);
  }
}
...
static void sem_schema_ad_hoc_migration_stmt_for_version(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (!sem_validate_version(CS, SCHEMA_ANNOTATION_AD_HOC, ast, &version, &name)) {
    record_error(CS, ast);
    return;
  }

  if (!name) {
    report_error(CS, ast, "CQL0284: ad hoc schema migration directive must provide a procedure to run", NULL);
    record_error(CS, ast);
    return;
  }
  ...
  if (CS->sem.validating_previous_schema) {
    sem_validate_previous_ad_hoc(CS, ast, name, version);
  }
  else if (adding_current_entity) {
    add_item_to_list(CS, &CS->sem.all_ad_hoc_list, ast);
    symtab_add(CS, CS->sem.ad_hoc_migrates, name, ast);
    record_schema_annotation(CS, version, ast, name, SCHEMA_ANNOTATION_AD_HOC, NULL, version_annotation, 0);
  }
}
...
static void sem_schema_ad_hoc_migration_stmt_for_recreate(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (adding_current_entity) {
    if (!symtab_add(CS, CS->sem.ad_hoc_recreate_actions, group, ast)) {
       report_error(CS, ast, "CQL0176: indicated procedure or group already has a recreate action", group);
       record_error(CS, ast);
       return;
    }
  }

  if (!sem_create_migration_proc_prototype(CS, ast, proc)) {
    record_error(CS, ast);
    return;
  }
...
static void sem_schema_ad_hoc_migration_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  if (ast->right) {
    ...
    sem_schema_ad_hoc_migration_stmt_for_version(CS, ast);
  }
}
...
static void enqueue_pending_region_validation(CqlState* _Nonnull CS, ast_node *prev, ast_node *cur, CSTR name) {
  ...
  deployable_validation *v = bytebuf_new(CS->sem.deployable_validations, deployable_validation);

  v->prev = prev;
  v->cur = cur;
  v->prev_region = CS->sem.current_region;  // recall we are processing previous items (see above)
...
static void sem_validate_previous_deployable_region(CqlState* _Nonnull CS, ast_node *root, deployable_validation *v) {
  ...
  ast_node *prev_reg = find_region(CS, prev_region);
  ...
  if (prev_reg->sem->region) {
    ...
    if (cur->sem->region) {
      ast_node *cur_reg = find_cur_region(CS, cur->sem->region);
    ...
    if (strcmp(cur_deployment_region, prev_deployment_region)) {
      ...
      report_and_capture_error(CS, root, cur, msg.ptr, name);
      record_error(CS, prev);
      CHARBUF_CLOSE(msg);
    }
  }
...
static void sem_validate_all_deployable_regions(CqlState* _Nonnull CS, ast_node *root) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (!is_error(v->prev) && !is_error(v->cur)) {
      sem_validate_previous_deployable_region(CS, root, v);
    }
  }
}
...
static void sem_emit_enums_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  while (name_list) {
    ...
    if (!find_enum(CS, name)) {
      report_error(CS, name_ast, "CQL0169: enum not found", name);
      record_error(CS, ast);
      return;
    }
...
static void sem_emit_constants_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  while (name_list) {
    ...
    if (!find_constant_group(CS, name)) {
      report_error(CS, name_ast, "CQL0169: constant group not found", name);
      record_error(CS, ast);
      return;
    }
...
static void sem_keep_table_name_in_aliases_stmt(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  CS->sem.keep_table_name_in_aliases = 1;
}
...
static void insert_table_alias_string_overide(CqlState* _Nonnull CS, ast_node *_Nonnull ast, CSTR _Nonnull table_name) {
  ...
  if (qid) {
    CHARBUF_OPEN(tmp);
    bprintf(&tmp, "[TABLE ");
    cg_unquote_encoded_qstr(CS, &tmp, table_name);
    bprintf(&tmp, " AS %s]", original_alias);
    result = Strdup(CS, tmp.ptr);
    CHARBUF_CLOSE(tmp);
  }
  else {
    result = dup_printf(CS, "[TABLE %s AS %s]", table_name, original_alias);
  }


  // Allow existing alias to be reformatted as something like "TABLE table_name AS some_alias".
  sem_node *new_alias_sem = new_sem(CS, SEM_TYPE_OK);
...
#undef STMT_INIT
#define STMT_INIT(x) symtab_add_GenOneStmt(CS, syms, k_ast_ ## x, sem_ ## x)
...
#define FUNC_INIT(x) symtab_add_sem_FuncIit(CS, CS->sem.builtin_funcs, #x, sem_func_ ## x)
...
#define MISC_ATTR_INIT(x) symtab_add_SemMiscAttributes(CS, CS->sem.misc_attributes, #x, sem_misc_attrs_ ## x)
...
cql_noexport void sem_main(CqlState* _Nonnull CS, ast_node *ast) {
  ...
  CS->sem.exprs = symtab_new();
  CS->sem.builtin_funcs = symtab_new();
  CS->sem.builtin_special_funcs = symtab_new();
  CS->sem.funcs = symtab_new();
  CS->sem.interfaces = symtab_new();
  CS->sem.unchecked_funcs = symtab_new();
  CS->sem.procs = symtab_new();
  CS->sem.unchecked_procs = symtab_new();
  CS->sem.proc_arg_info = symtab_new();
  CS->sem.enums = symtab_new();
  CS->sem.triggers = symtab_new();
  CS->sem.upgrade_procs = symtab_new();
  CS->sem.ad_hoc_migrates = symtab_new();
  CS->sem.tables = symtab_new();
  CS->sem.table_default_values = symtab_new();
  CS->sem.indices = symtab_new();
  CS->sem.globals = symtab_new();
  CS->sem.constant_groups = symtab_new();
  CS->sem.variable_groups = symtab_new();
  CS->sem.constants = symtab_new();
  CS->sem.current_variables = CS->sem.globals;
  CS->sem.savepoints = symtab_new();
  CS->sem.schema_regions = symtab_new();
  CS->sem.non_sql_stmts = symtab_new();
  CS->sem.sql_stmts = symtab_new();
  CS->sem.ref_sources_for_target_table = symtab_new();
  CS->sem.ref_targets_for_source_table = symtab_new();
  CS->sem.builtin_aggregated_funcs = symtab_new();
  CS->sem.global_types = symtab_new();
  CS->sem.misc_attributes = symtab_new();
  CS->sem.ad_hoc_recreate_actions = symtab_new();
  CS->sem.recreate_group_deps = symtab_new();

  CS->sem.schema_annotations = _ast_pool_new(bytebuf);
  CS->sem.recreate_annotations = _ast_pool_new(bytebuf);
  bytebuf_open(CS->sem.schema_annotations);
  bytebuf_open(CS->sem.recreate_annotations);

  Invariant(CS->sem.cte_cur == NULL);

  symtab *syms = CS->sem.non_sql_stmts;
  ...
  if (ast) {
    sem_stmt_list(CS, ast);
  }

  Invariant(CS->sem.cte_cur == NULL);

  // put tables/views/etc into the natural order (the order declared)
  reverse_list(&CS->sem.all_tables_list);
  reverse_list(&CS->sem.all_functions_list);
  reverse_list(&CS->sem.all_views_list);
  reverse_list(&CS->sem.all_indices_list);
  reverse_list(&CS->sem.all_triggers_list);
  reverse_list(&CS->sem.all_regions_list);
  reverse_list(&CS->sem.all_ad_hoc_list);
  reverse_list(&CS->sem.all_enums_list);
  reverse_list(&CS->sem.all_select_functions_list);
  ...
  if (CS->sem.validating_previous_schema) {
    reverse_list(&CS->sem.all_prev_recreate_tables);
    sem_validate_all_tables_not_in_previous(CS, ast);
    sem_validate_all_columns_not_in_previous(CS, ast);
    sem_validate_all_prev_recreate_tables(CS, ast);
    sem_validate_all_ad_hoc_not_in_previous(CS, ast);
    sem_validate_all_deployable_regions(CS, ast);
  }
  ...
  if (CS->sem.validating_previous_schema) {
    ...
    // to the normal  one.
    symtab_delete(CS, CS->sem.schema_regions);  // the regions during previous schema processing
    CS->sem.schema_regions = CS->sem.new_regions;   // the original regions
    CS->sem.new_regions = NULL;             // nobody should be looking here anymore

    symtab_delete(CS, CS->sem.enums);  // the enums during previous schema processing
    CS->sem.enums = CS->sem.new_enums;     // the original enums
    CS->sem.new_enums = NULL;      // nobody should be looking here anymore
  }
...
cql_noexport void sem_cleanup(CqlState* _Nonnull CS) {
  ...
  BYTEBUF_CLEANUP(CS->sem.recreate_annotations);
  BYTEBUF_CLEANUP(CS->sem.schema_annotations);
  BYTEBUF_CLEANUP(CS->sem.unitary_locals);

  SYMTAB_CLEANUP(CS->sem.ad_hoc_migrates);
  SYMTAB_CLEANUP(CS->sem.ref_sources_for_target_table);
  SYMTAB_CLEANUP(CS->sem.ref_targets_for_source_table);
  SYMTAB_CLEANUP(CS->sem.builtin_funcs);
  SYMTAB_CLEANUP(CS->sem.builtin_special_funcs)
  SYMTAB_CLEANUP(CS->sem.current_region_image);
  SYMTAB_CLEANUP(CS->sem.exprs);
  SYMTAB_CLEANUP(CS->sem.funcs);
  SYMTAB_CLEANUP(CS->sem.unchecked_funcs);
  SYMTAB_CLEANUP(CS->sem.globals);
  SYMTAB_CLEANUP(CS->sem.indices);
  SYMTAB_CLEANUP(CS->sem.locals);
  SYMTAB_CLEANUP(CS->sem.constant_groups);
  SYMTAB_CLEANUP(CS->sem.variable_groups);
  SYMTAB_CLEANUP(CS->sem.constants);
  SYMTAB_CLEANUP(CS->sem.monitor_symtab );
  SYMTAB_CLEANUP(CS->sem.new_regions);
  SYMTAB_CLEANUP(CS->sem.new_enums);
  SYMTAB_CLEANUP(CS->sem.non_sql_stmts);
  SYMTAB_CLEANUP(CS->sem.procs);
  SYMTAB_CLEANUP(CS->sem.unchecked_procs);
  SYMTAB_CLEANUP(CS->sem.interfaces);
  SYMTAB_CLEANUP(CS->sem.proc_arg_info);
  SYMTAB_CLEANUP(CS->sem.enums);
  SYMTAB_CLEANUP(CS->sem.schema_regions);
  SYMTAB_CLEANUP(CS->sem.savepoints);
  SYMTAB_CLEANUP(CS->sem.sql_stmts);
  SYMTAB_CLEANUP(CS->sem.table_items);
  SYMTAB_CLEANUP(CS->sem.tables);
  SYMTAB_CLEANUP(CS->sem.triggers);
  SYMTAB_CLEANUP(CS->sem.upgrade_procs);
  SYMTAB_CLEANUP(CS->sem.builtin_aggregated_funcs);
  SYMTAB_CLEANUP(CS->sem.included_regions);
  SYMTAB_CLEANUP(CS->sem.excluded_regions);
  SYMTAB_CLEANUP(CS->sem.global_types);
  SYMTAB_CLEANUP(CS->sem.misc_attributes);
  SYMTAB_CLEANUP(CS->sem.ad_hoc_recreate_actions);
  SYMTAB_CLEANUP(CS->sem.table_default_values);
  SYMTAB_CLEANUP(CS->sem.recreate_group_deps);

  // these are getting zeroed so that leaksanitizer will not count those objects as reachable from a global root.

  CS->sem.all_ad_hoc_list = NULL;
  CS->sem.all_functions_list = NULL;
  CS->sem.all_indices_list = NULL;
  CS->sem.all_prev_recreate_tables = NULL;
  CS->sem.all_regions_list = NULL;
  CS->sem.all_select_functions_list = NULL;
  CS->sem.all_tables_list = NULL;
  CS->sem.all_subscriptions_list = NULL;
  CS->sem.next_subscription = NULL;
  CS->sem.found_subscription_error = false;
  CS->sem.all_triggers_list = NULL;
  CS->sem.all_views_list = NULL;
  CS->sem.created_columns = NULL;
  CS->sem.cte_cur = NULL;
  CS->sem.current_explain_stmt = NULL;
  CS->sem.current_expr_context = SEM_EXPR_CONTEXT_NONE;
  current_joinscope_lv = NULL;
  CS->sem.current_region = NULL;
  CS->sem.current_upsert_table_ast = NULL;
  CS->sem.current_variables = NULL;  // this is either locals or globals, freed above
  CS->sem.has_dml = false;
  CS->sem.in_shared_fragment_call = false;
  CS->sem.in_trigger = false;
  CS->sem.in_trigger_when_expr = false;
  CS->sem.in_switch = false;
  CS->sem.in_upsert = false;
  CS->sem.in_upsert_rewrite = false;
  CS->sem.keep_table_name_in_aliases = false;
  CS->sem.loop_depth = 0;
  CS->sem.in_proc_savepoint = false;
  CS->sem.max_previous_schema_version = -1;
  reset_enforcements(CS);
  enforcement_stack_lv = NULL; // all the stack entries are in the ast pool, nothing to free
  CS->sem.monitor_jptr = NULL;
  CS->sem.recreates = 0;
  CS->sem.schema_upgrade_script = false;
  CS->sem.schema_upgrade_version = -1;
  CS->sem.select_level = 0;
  CS->sem.sem_stmt_level = -1;
  CS->sem.sem_ok = NULL;
  CS->sem.validating_previous_schema = false;
  CS->sem.between_count = 0;
  pending_table_validations_head_lv = NULL;
  CS->sem.current_table_name = NULL;
  CS->sem.current_table_ast = NULL;
  CS->sem.local_types = NULL;
  CS->sem.is_analyzing_notnull_rewrite = false;
  global_notnull_improvements_lv = NULL;
  CS->sem.current_loop_analysis_state = LOOP_ANALYSIS_STATE_NONE;
  CS->sem.current_proc_contains_try_is_proc_body = false;
  CS->sem.in_backing_rewrite = false;
  CS->sem.backed_tables_list = NULL;
}
...
//cql_data_defn( list_item *a_defn( list_item *CS->sem.all_tables_list );S->sem.all_subscriptions_list );
...
//cql_data_defn( CSTR _Nullable Cencode_context_column );
