...
static void print_dot(struct ast_node *node) {
  ...
  if (is_ast_num(node)) {
    cql_output("\n    %s%" PRIu64 " [label = \"%s\" shape=plaintext]", node->type, id, ((struct num_ast_node*)node)->value);
  } else if (is_ast_str(node)) {
    EXTRACT_STRING(str, node);
    if (is_id(node)) {
      // unescaped name, clean to emit
      cql_output("\n    %s%" PRIu64 " [label = \"%s\" shape=plaintext]", node->type, id, str);
    } else {
      ...
      // ready to use!
      cql_output("\n    %s%" PRIu64 " [label = %s shape=plaintext]", node->type, id, double_encoding.ptr);
      CHARBUF_CLOSE(double_encoding);
      CHARBUF_CLOSE(encoding);
      CHARBUF_CLOSE(plaintext);
    }
  } else {
    cql_output("\n    %s%" PRIu64 " [label = \"%s\" shape=plaintext]", node->type, id, node->type);
    primitive = false;
  }
  ...
  if (ast_has_left(node)) {
    cql_output("\n    %s%" PRIu64 " -> %s%" PRIu64 ";", node->type, id, node->left->type, next_id);
    print_dot(node->left);
  } else {
    cql_output("\n    _%" PRIu64 " [label = \"%s\" shape=plaintext]", id, ground_symbol);
    cql_output("\n    %s%" PRIu64 " -> _%" PRIu64 ";", node->type, id, id);
  }

  if (ast_has_right(node)) {
    cql_output("\n %s%" PRIu64 " -> %s%" PRIu64 ";", node->type, id, node->right->type, next_id);
