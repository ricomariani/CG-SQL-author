...
for source in $sources; do
    ...
    # Swap in two generated files
    if [[ "$source" = *02_grammar.md ]]; then
       source="$SCRIPT_DIR_RELATIVE/../.generated/cql_grammar.md"
       debug "using computed CQL grammar file $source"
    elif [[ "$source" = *05_json_schema_grammar.md ]]; then
       source="$SCRIPT_DIR_RELATIVE/../.generated/json_grammar.md"
       debug "using computed JSON grammar file $source"
    fi
