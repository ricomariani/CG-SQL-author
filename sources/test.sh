...
building() {
  ...
  if ! do_make all >"$O/build.out" 2>"$O/build.err"
  then
  ...
  if grep "^State.*conflicts:" "$O/cql.y.output" >"$O/build.err"
  then
  ...
  if ! do_make amalgam >"$O/build.out" 2>"$O/build.err"
  then
  ...
  if ! do_make amalgam_test >"$O/build.out" 2>"$O/build.err"
  then
  ...
  if ! (do_make cql-verify) 2>"$O/build.err"
  then
  ...
  if ! (do_make cql-linetest) 2>"$O/build.err"
  then
  ...
  if ! (do_make json-test) 2>"$O/build.err"
  then
...
basic_test() {
  ...
  if ! ${CQL} --echo --dev --include_paths "$T"  < "$T/test.sql" >"$O/test.out"
  then
  ...
  if ! ${CQL} --echo --dev --include_paths "$T" --in "$T/test.sql" --exp >"$O/test_exp.out"
  then
  ...
  if ${CQL} --in "$T/test_include_file_not_found.sql" 2>"$O/include_not_found.err"
  then
  ...
  if ${CQL} --in "$T/include_files_infinite_nesting.sql" --include_paths test 2>"$O/include_nesting.err"
  then
    echo "error code should have indicated failure"
    failed
  fi
...
macro_test() {
  ...
  if ! ${CQL} --test --exp --ast --hide_builtins --in "$T/macro_test.sql" >"$O/macro_test.out" 2>"$O/macro_test.err"
  then
  ...
  if ${CQL} --exp --in "$T/macro_exp_errors.sql" >"$O/macro_exp_errors.out" 2>"$O/macro_test.err.out"
  then
  ...
  if ${CQL} --exp --in "$T/macro_test_dup_arg.sql" >"$O/macro_exp_errors.out" 2>"$O/macro_test_dup.err.out"
  then
     echo "CQL macro error test returned unexpected error code"
...
semantic_test() {
  ...
  if ! sem_check --sem --ast --hide_builtins --dev --in "$T/sem_test.sql" >"$O/sem_test.out" 2>"$O/sem_test.err"
  then
  ...
  if ! sem_check --sem --ast --in "$T/sem_test_dev.sql" >"$O/sem_test_dev.out" 2>"$O/sem_test_dev.err"
  then
     echo "CQL semantic analysis returned unexpected error code"
...
code_gen_c_test() {
  ...
  if ! ${CQL} --dev --test --cg "$O/cg_test_c.h" "$O/cg_test_c.c" "$O/cg_test_exports.out" --in "$T/cg_test.sql" --global_proc cql_startup --generate_exports 2>"$O/cg_test_c.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_c.err"
    failed
  ...
  if ! ${CQL} --cg "$O/cg_test_c_globals.h" "$O/cg_test_c_globals.c" --in "$T/cg_test_c_globals.sql" 2>"$O/cg_test_c.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_c.err"
    failed
  ...
  if ! ${CQL} --test --cg "$O/cg_test_c_globals.h" "$O/cg_test_c_globals.c" --in "$T/cg_test_c_globals.sql" 2>"$O/cg_test_c.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_c.err"
    failed
  ...
  if ! ${CQL} --dev --test --cg "$O/cg_test_c_with_namespace.h" "$O/cg_test_c_with_namespace.c" "$O/cg_test_imports_with_namespace.ref" --in "$T/cg_test.sq"l --global_proc cql_startup --c_include_namespace test_namespace --generate_exports 2>"$O/cg_test_c.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_c.err"
    failed
  ...
  if ! ${CQL} --dev --test --cg "$O/cg_test_c_with_header.h" "$O/cg_test_c_with_header.c" --in "$T/cg_test.sql" --global_proc cql_startup --c_include_path "somewhere/something.h" 2>"$O/cg_test_c.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_c.err"
    failed
...
code_gen_objc_test() {
  ...
  echo running codegen test
  if ! ${CQL} --dev --test --cg "$O/cg_test_objc.out" --objc_c_include_path Test/TestFile.h --in "$T/cg_test.sql" --rt objc 2>"$O/cg_test_objc.err"
  then
    echo "ERROR:"
  ...
  if ! grep "<Test/TestFile.h>" "$O/cg_test_objc.out"
  then
...
# the output path doesn't exist, should cause an error
  ...
  if ${CQL} --in /xx/yy/zz 2>"$O/badpath.err"
  then
...
# the output file is not writeable, should cause an error
  ...
  if ${CQL} --dev --cg /xx/yy/zz /xx/yy/zzz --in "$T/cg_test.sql" --global_proc xx 2>"$O/unwriteable.err"
  then
...
# wrong number of args specified in --cg (for objc)
  ...
  if ${CQL} --dev --cg "$O/__temp" "$O/__temp2" --in "$T/cg_test.sql" --rt objc 2>"$O/cg_1_2.err"
  then
...
# semantic errors should abort output (we'll not try to write)
  ...
  if ${CQL} --cg "$O/__temp" /xx/yy/zz --in "$T/semantic_error.sql" 2>"$O/sem_abort.err"
  then
...
# no result sets in the input for objc should result in empty output, not errors
  ...
  if ! ${CQL} --cg "$O/__temp" --in "$T/noresult.sql" --objc_c_include_path dummy --rt objc 2>"$O/objc_no_results.err"
  then
...
# bogus arg should report error
  ...
  if ${CQL} --garbonzo!! 2>"$O/invalid_arg.err"
  then
...
# --cg did not have any following args, should force an error
  ...
  if ${CQL} --cg 2>"$O/cg_requires_file.err"
  then
...
# --generate_file_type did not specify a file type
  ...
  if ${CQL} --generate_file_type 2>"$O/generate_file_type.err"
  then
...
# --generate_file_type specified invalid file type (should cause an error)
  ...
  if ${CQL} --generate_file_type foo 2>"$O/generate_file_file.err"
  then
...
# --rt specified with no arg following it
  ...
  if ${CQL} --rt 2>"$O/rt_arg_missing.err"
  then
...
# invalid result type specified with --rt, should force an error
  ...
  if ${CQL} --rt foo 2>"$O/rt_arg_bogus.err"
  then
...
# --cqlrt specified but no file name present, should force an error
  ...
  if ${CQL} --cqlrt 2>"$O/cqlrt_arg_missing.err"
  then
...
# --global_proc has no proc name
  ...
  if ${CQL} --global_proc 2>"$O/global_proc_missing.err"
  then
...
# objc_c_include_path had no path
  ...
  if ${CQL} --objc_c_include_path 2>"$O/objc_include_missing.err"
  then
...
# --in arg missing
  ...
  if ${CQL} --in 2>"$O/in_arg_missing.err"
  then
...
# no c_include_namespace arg
  ...
  if ${CQL} --c_include_namespace 2>"$O/c_include_namespace_missing.err"
  then
...
schema_migration_test() {
  ...
  if ! sem_check --sem --ast --in "$T/sem_test_migrate.sql" >"$O/sem_test_migrate.out" 2>"$O/sem_test_migrate.err"
  then
     echo "CQL semantic analysis returned unexpected error code"
  ...
  if ! sem_check --sem --in "$T/schema_version_error.sql" --ast >"$O/schema_version_error.out" 2>"$O/schema_version_error.err.out"
  then
     echo "CQL semantic analysis returned unexpected error code"
  ...
  if ! sem_check --sem --ast --exclude_regions high_numbered_thing --in "$T/sem_test_prev.sql" >"$O/sem_test_prev.out" 2>"$O/sem_test_prev.err"
  then
     echo "CQL semantic analysis returned unexpected error code"
  ...
  if ! ${CQL} --cg "$O/cg_test_schema_upgrade.out" --in "$T/cg_test_schema_upgrade.sql" --global_proc test --rt schema_upgrade 2>"$O/cg_test_schema_upgrade.err"
  then
    echo "ERROR:"
  ...
  if ! ${CQL} --cg "$O/cg_test_schema_upgrade.h" "$O/cg_test_schema_upgrade.c" --in "$O/cg_test_schema_upgrade.out"
  then
  ...
  if ! ${CQL} --cg "$O/cg_test_schema_prev.out" --in "$T/cg_test_schema_upgrade.sql" --rt schema 2>"$O/cg_test_schema_prev.err"
  then
    echo "ERROR:"
  ...
  if ! ${CQL} --cg "$O/cg_test_schema_sqlite.out" --in "$T/cg_test_schema_upgrade.sql" --rt schema_sqlite 2>"$O/cg_test_schema_sqlite.err"
  then
    echo "ERROR:"
  ...
  if ! ${CQL} --cg "$O/prev_twice.out" --in "$O/prev_loop.out" --rt schema 2>"$O/cg_test_schema_prev_twice.err"
  then
    echo "ERROR:"
  ...
  if ! ${CQL} --cg "$O/prev_thrice.out" --in "$O/prev_twice.out" --rt schema 2>"$O/cg_test_schema_prev_thrice.err"
  then
    echo "ERROR:"
  ...
  if ! ${CQL} --cg "$O/cg_test_schema_partial_upgrade.out" --in "$T/cg_test_schema_upgrade.sql" --global_proc test --rt schema_upgrade --include_regions extra --exclude_regions shared 2>"$O/cg_test_schema_partial_upgrade.err"
  then
    echo "ERROR:"
  ...
  echo "  compiling the upgrade script with CQL"
  if ! ${CQL} --cg "$O/cg_test_schema_partial_upgrade.h" "$O/cg_test_schema_partial_upgrade.c" --in "$O/cg_test_schema_partial_upgrade.out"
  then
    echo CQL compilation failed
    failed;
  fi
  ...
  if ! ${CQL} --cg "$O/cg_test_schema_min_version_upgrade.out" --in "$T/cg_test_schema_upgrade.sql" --global_proc test --rt schema_upgrade --min_schema_version 3 2>"$O/cg_test_schema_min_version_upgrade.err"
  then
    echo "ERROR:"
...
misc_cases() {
  ...
  if ! ${CQL} >"$O/usage.out" 2>"$O/usage.err"
  then
  ...
  if ${CQL} --in "$T/error.sql" >"$O/error.out" 2>"$O/simple_error.err"
  then
  ...
  if ${CQL} --cg "$O/__temp.h" "$O/__temp.c" --in "$T/cg_test_prev_invalid.sql" 2>"$O/prev_and_codegen_incompat.err"
  then
  ...
  if ! ${CQL} --cg "$O/__temp.h" "$O/__temp.c" --in "$T/bigquote.sql" --global_proc x >/dev/null 2>"$O/bigquote.err"
  then
  ...
  if ! ${CQL} --dev --cg "$O/__temp.h" "$O/__temp.c" --in "$T/cg_test.sql" --global_proc x --cqlrt alternate_cqlrt.h 2>"$O/alt_cqlrt.err"
  then
    echo alternate cqlrt test failed
    failed
  fi

  if ! grep alternate_cqlrt.h "$O/__temp.h" >/dev/null
  then
  ...
  if ${CQL} --dev --cg "$O/__temp.c" "$O/__temp.h" --in "$T/cg_test.sql" --global_proc x --generate_exports 2>"$O/gen_exports_args.err"
  then
  ...
  if ${CQL} --cg "$O/cg_test_schema_partial_upgrade.out" --in "$T/cg_test_schema_upgrade.sql" --global_proc test --rt schema_upgrade --include_regions bogus --exclude_regions shared 2>"$O/inc_invalid_regions.err"
  then
  ...
  if ${CQL} --cg "$O/cg_test_schema_partial_upgrade.out" --in "$T/cg_test_schema_upgrade.sql" --global_proc test --rt schema_upgrade --include_regions extra --exclude_regions bogus 2>"$O/excl_invalid_regions.err"
  then
  ...
  if ${CQL} --cg "$O/__temp.c" "$O/__temp.h" --in "$T/bigquote.sql" 2>"$O/global_proc_needed.err"
  then
  ...
  if ${CQL} --dev --cg "$O/unwritable.h".out "$O/unwritable.c".out --in "$T/cg_test.sql" --rt c --global_proc cql_startup 2>"$O/write_fail.err"
  then
  ...
  if ! ${CQL} --cg "$O/cg_test_generated_from.h" "$O/cg_test_generated_from.c" "$O/cg_test_generated_from.out" --in "$T/cg_test_generated_from.sql" 2>"$O/cg_test_generated_from.err"
  then
  ...
  if ${CQL} --test --cg "$O/cg_test_objc.out" --in "$T/cg_test.sql" --rt objc 2>"$O/c_include_needed.err"
  then
  ...
  if ! grep "Generated from test/cg_test_generated_from.sql:21" "$O/cg_test_generated_from.h" >/dev/null
  then
    echo Generated from text did not appear in the header output.
    failed
  fi
  if ! grep "Generated from test/cg_test_generated_from.sql:21" "$O/cg_test_generated_from.c" >/dev/null
  then
  ...
  if ${CQL} --in "$T/parse_test_fetch_from_call_columns.sql" 2>"$O/parse_test_fetch_from_call_columns.err"
  then
  ...
  if ${CQL} --in "$T/parse_test_cql_inferred_notnull.sql" 2>"$O/parse_test_cql_inferred_notnull.err"
  then
...
json_validate() {
  ...
  if  ! ${CQL} --cg "$O/__temp.out" --in "${sql_file}" --rt json_schema 2>"$O/cg_test_json_schema.err"
  then
  ...
  if ! common/json_check.py <"$O/__temp.out" >/dev/null
  then
  ...
  if ! out/json_test <"$O/__temp.out" >"$O/json_errors.txt"
  then
...
json_schema_test() {
  ...
  if ! ${CQL} --test --cg "$O/cg_test_json_schema.out" --in "$T/cg_test_json_schema.sql" --rt json_schema 2>"$O/cg_test_json_schema.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_json_schema.err"
    failed
...
test_helpers_test() {
  ...
  if ! ${CQL} --test --cg "$O/cg_test_test_helpers.out" --in "$O/__temp" --rt test_helpers 2>"$O/cg_test_test_helpers.err"
  then
    echo "ERROR:"
  ...
  if ! ${CQL} --sem --ast --in "$O/cg_test_test_helpers.out" >/dev/null 2>"$O/cg_test_test_helpers.err"
  then
     echo "CQL semantic analysis returned unexpected error code"
     cat "$O/cg_test_test_helpers.err"
     failed
  ...
  if ! ${CQL} --test --dev --cg "$O/test_helpers.h" "$O/test_helpers.c" --in "$O/cg_test_test_helpers.out" 2>"$O/cg_test_test_helpers.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_test_helpers.err"
    failed
  ...
  if ! "./$O/test_helpers_test" >/dev/null 2>"$O/cg_test_test_helpers.err"
  then
...
run_test() {
  ...
  if ! ${CQL} --nolines --cg "$O/run_test.h" "$O/run_test.c" --in "$T/run_test.sql" --global_proc cql_startup --rt c
  then
    echo codegen failed.
    failed
  fi
  ...
  if ! (echo "  executing tests"; "./$O/run_test")
  then
    echo tests failed
    failed
  fi

  if ! ${CQL} --compress --cg "$O/run_test_compressed.h" "$O/run_test_compressed.c" --in "$T/run_test.sql" --global_proc cql_startup --rt c
  then
  ...
  if ! (echo "  executing tests (compressed version)"; "./$O/run_test_compressed")
  then
    echo tests failed
    failed
  fi
...
query_plan_test() {
  ...
  if ! ${CQL} --sem --ast --dev --in "$O/cg_test_query_plan2.sql" >"$O/__temp" 2>"$O/cg_test_query_plan.err"
  then
     echo "CQL semantic analysis returned unexpected error code"
  ...
  if ! ${CQL} --test --dev --cg "$O/cg_test_query_plan.out" --in "$O/cg_test_query_plan2.sql" --rt query_plan 2>"$O/cg_test_query_plan.err"
  then
    echo "ERROR:"
    cat "$O/cg_test_query_plan.err"
    failed
  ...
  echo semantic analysis
  if ! ${CQL} --sem --ast --dev --test --in "$O/cg_test_query_plan.out" >"$O/__temp" 2>"$O/cg_test_query_plan.err"
  then
     echo "CQL semantic analysis returned unexpected error code"
     cat "$O/cg_test_query_plan.err"
     failed
  ...
  if ! ${CQL} --test --dev --cg "$O/query_plan.h" "$O/query_plan.c" --in "$O/cg_test_query_plan.out" 2>"$O/query_plan_print.err"
  then
    echo "ERROR:"
  ...
  if ! "./$O/query_plan_test" >"$O/cg_test_query_plan_view.out" 2>"$O/cg_test_query_plan_view.err"
  then
  ...
  if ! common/json_check.py <"$O/cg_test_query_plan_view.out" >"$O/cg_test_query_plan_js.out" 2>"$O/cg_test_query_plan_js.err"
  then
  ...
  if ! ${CQL} --test --dev --cg "$O/cg_test_query_plan_empty.out" --in "$T/cg_test_query_plan_empty.sql" --rt query_plan 2>"$O/cg_test_query_plan_empty.err"
  then
    echo "ERROR:"
  ...
  if ! ${CQL} --sem --ast --dev --test --in "$O/cg_test_query_plan_empty.out" >"$O/__temp" 2>"$O/cg_test_query_plan_empty.err"
  then
     echo "CQL semantic analysis returned unexpected error code"
     cat "$O/cg_test_query_plan_empty.err"
     failed
  ...
  if ! ${CQL} --test --dev --cg "$O/query_plan.h" "$O/query_plan.c" --in "$O/cg_test_query_plan_empty.out" 2>"$O/query_plan_print.err"
  then
    echo "ERROR:"
    cat "$O/query_plan_print.err"
    failed
...
line_number_test() {
  ...
  if ! grep "^#line " "$O/cg_test_generated_from.c" >/dev/null
  then
    echo "# line directives not emitted. See" "$O/cg_test_generated_from.c"
    failed
  fi

  if ! grep "/\* A comment \*/" "$O/cg_test_generated_from.c" >/dev/null
  then
  ...
  if ! ${CQL} --nolines --cg "$O/cg_test_generated_from.h" "$O/cg_test_generated_from.c" "$O/cg_test_generated_from.out" --in "$T/cg_test_generated_from.sql" 2>"$O/cg_test_generated_from.err"
  then
    cat "$O/cg_test_generated_from.err"
    echo 'ERROR: Compilation failed.'
    failed
  ...
  if ! ${CQL} --in "$T/linetest.sql" --cg "$O/linetest.h" "$O/linetest.c" 2>"$O/linetest.err"
  then
    cat "$O/linetest.err"
    echo 'ERROR: Compilation failed.'
    failed
  ...
  if ! $O/cql-linetest "$T/linetest.expected" "$O/linetest.c"
  then
...
stats_test() {
  ...
  if ! ${CQL} --cg "$O/stats.csv" --in "$T/stats_test.sql" --rt stats 2>"$O/stats_test.err"
  then
    echo "ERROR:"
...
amalgam_test() {
  ...
  if ! ("./$O/amalgam_test" "$T/cql_amalgam_test_success.sql" "$T/cql_amalgam_test_semantic_error.sql" "$T/cql_amalgam_test_syntax_error.sql" >"$O/cql_amalgam_test.out" 2>"$O/cql_amalgam_test.err")
  then
...
code_gen_lua_test() {
  ...
  echo running codegen test
  if ! ${CQL} --dev --test --cg "$O/cg_test_lua.lua" --in "$T/cg_test_lua.sql" --global_proc cql_startup --rt lua 2>"$O/cg_test_lua.err"
  then
    echo "ERROR:"
...
dot_test() {
  ...
  if ! ${CQL} --dot --hide_builtins --in "$T/dottest.sql" >"$O/dottest.out"
  then
...
signatures_test() {
  ...
  if grep "$AT_GENERATED_TAG SignedSource" $T/*.ref
  then
